...
#### 四路審查未修項（Session 32 提出，Session 33／34 續清；剩餘 2 條）

Session 32 修掉「線上壞掉」與「靜默毀資料」兩層後留下 25 條，Session 33
清掉 8 條，Session 34 再清掉 15 條（含 1 條判定為不宜照做），只剩
兩條需要專案外的輸入：一條要產品決策，一條要實機。詳細說明見 WORKLOG「未來待辦」。
...
- **A24 待產品決策** — 棋子→八卦分佈不均：`仕/士` 與 `兵/卒` 共用同一組
  錯卦（艮/兌），造成艮兌各 7 子、乾坤各 1 子。兩顆棋時乾為天／坤為地
  各約 1/1024，兌為澤／艮為山約 49/1024，相差 49 倍。抽棋本身均勻、
  無模除偏差，這是紅黑錯卦配對的副作用而非 bug。要不要調整取決於
  「忠於棋盤實際子數」與「卦象機率均衡」哪個優先——見 WORKLOG。
