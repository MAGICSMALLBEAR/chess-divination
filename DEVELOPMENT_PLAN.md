...
技術面剩一項（`/settings 可正常載入` 的 e2e 競態，Session 68 收尾時抓到，
編號 #39）；其餘皆卡在外部資源（帳號／裝置／付費）或待排程的功能候選，
完整清單見 `WORKLOG.md` 的「未來待辦」。

> **這句話在 S56 之後被推翻過一次又一次**（S57 首頁分享入口、S58 文案寫死的
> 數字、S59 首頁空欄位、S62 占驗提醒只導到統計頁，S68 又加上這條 e2e 競態），
> 每一次都是在寫下「待辦清空」之後換一種找法才找到的。所以它真正的意思是
> **「上一種找法已經找不到了」**，不是「沒有了」。

> Session 47 註：這句話每一輪都要重新確認，不可沿用。上一次寫下它時，
> 成就系統還不認靈棋、AI 提示詞還看不到棋盤——**新模式接完主流程不等於
> 做完，周邊系統（歷史／收藏／分享／統計／成就／AI 提示詞／圖鑑）要逐一
> 走過**。這兩處都不會壞畫面、不會紅測試。
...
- iOS/Android 實機測試（Expo Go，31 項清單）→ EAS Build → 上架
  （App Store $99/年、Google Play $25）→ 自訂域名。實機時一併確認
  分享圖片有內容（A25 防線的現場驗證）
- **功能候選路線圖**（Session 61 提出，Session 62 續排定順序）：七項依
  **「卡在什麼」**排序，完整表格見 `WORKLOG.md` 的「未來待辦 › 功能候選」。
  **這些是待排程的產品決策，不是待修的缺陷，兩者不可混列。**
  ~~1~~ ✅ **S63 完成**（占驗回訪的首頁提示：指名最近滿期的那一筆、可按進去
  回填；統計頁那行也改成可按）——剩下的提醒策略仍待你決定；
  ~~2~~ ✅ **S64 完成**（統計洞察的樣本門檻：`MIN_INSIGHT_SAMPLES` 成為唯一
  真相來源，排序認門檻、樣本不足的列標示並附說明）；
  3 **只差你動手一次**（雲端同步）；4–5 **先拍板規則**（牌陣擴充、匯出報告）；
  6–7 **需要外部人力**（多語校閱、匿名回饋）。
- **雲端同步：在 Vercel Marketplace 接上 Upstash Redis 並 redeploy。**
  程式早就寫完，線上實測 `GET /api/sync` 帶合法配對碼回 **501**
  ——伺服器端從未接上資料庫。與 `DEEPSEEK_API_KEY` 同一類：
  **不是程式問題，是環境沒設**（且設完必須 redeploy 才會生效）。
...
### Session 46 — 兩軍對壘陣上線 + 靈棋規則式深度解讀（8/31）

#### 兩軍對壘陣（Session 42 構想，本次結案）

Session 42 提出、掛在「未排期」的構想：紅黑雙方各在己方半場布三子，
看楚河漢界兩側的強弱消長。動工前的兩個決策本次定案：

- **起卦結構**採「上卦＝紅方陣、下卦＝黑方陣、動爻＝雙方子力差」。
  半場本身有方位意義（紅為我方、黑為對方），各自成卦才讀得出對立；
  子力差有符號（紅減黑），動爻取模 6、0 視為 6，與既有路徑同一套餘數。
- **`maxPieces` 改為依牌陣可變**：`SpreadDefinition.maxPieces` +
  `getSpreadMaxPieces(id, fallback)`，只有此陣回 6。
- **落子半場限制在棋盤元件裡守住**：`formationHalfFull(row)` 滿三子的
  半場不再出現落子點，點擊與拖曳兩條路徑都走同一個判斷，不靠頁面記得。
  半場計數不放棋盤上（任何固定標籤都可能被棋子壓住），放牌陣導覽區。
- **揭曉頁一行都沒改**：兩軍對比文字（紅方子力較盛／黑方較盛／
  勢均力敵）併入 `positionSummary`，揭曉頁與歷史記錄照舊印它。
- 允許重複棋子沿用既有開關；此陣無固定格位（`slots: []`），角色是
  「哪一半場」而不是「哪一格」。

#### 靈棋深度解讀（Session 43–45 三度擱置的產品決策，本次定案：規則式）

不接 AI——現有提示詞整套繞著六十四卦與六爻盤寫，靈棋要接得另設一套。
規則式解讀（`services/lingqiInterpretation.ts`）的結構：

- 三才（天＝上、人＝中、地＝下）各以朝上子數分「全伏／中平／盛」三檔；
- 主軸取獨盛或並盛（兩才同數）或三才均衡，依主軸改寫第二步行動建議；
- 依問事類別給一段類別視角；收尾三步行動計畫。
- **不生成原典沒有的吉凶等級**——與 Session 43 的誠實原則同源。

過程重點：檔次文案是**函式不是字串**——`localizeProse` 的 zh 回傳不插值，
zh fallback 要把數字烘進去，en／ja 才走 `{n}` 佔位（liuyao／wenwang 慣例）。

#### 順帶修掉的既有缺陷

1. 靈棋 e2e 三條隱性 flaky：`/卦$/` 比對在 5 個不以「卦」結尾的卦目
   （抑災勢、救助教、鬼災勢、歲登勢、人事勢）上必紅；改以記錄的
   `poemTitle` 為準。`象曰`／`詩曰` 比對加 `{ exact: true }`。
2. 字型子集缺 4 字（壘／鏡／棚／絡），照既有流程重建。
3. e2e 匹配到 expo-router 疊棧裡隱藏背景頁的文字（Session 37 之後第二次）：
   改比帶冒號的摘要句（`/兩軍對壘：/`）——只有 positionSummary 這樣寫。
