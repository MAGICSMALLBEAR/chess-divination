...
技術面只剩兩項，其餘全卡在外部資源（帳號／裝置／付費），完整清單見
`WORKLOG.md` 的「未來待辦」。

- **`summarizeReading()` 是死碼**（Session 38 發現）。註解寫「供分享與離線解讀使用」，
  但全專案只有測試呼叫它，它產出的 6 條中文樣板因此也沒翻。要接上分享流程讓註解成真、
  還是連同 4 則測試一起刪，需先確認當初是否真的打算用在分享文字。
- **棋盤頁的棋子池位置**（Session 37 提出）。桌面上挑棋子與落子之間要上下移動視線；
  解法是把棋子池移到棋盤右側，但拖曳定位邏輯（`screenToGrid`）在 `ChessBoard` 內，
  屬於重構而非版面切換，要另立範圍。
