...
| B5 | 移除 `SpaceMono` 英文字體及其阻塞 App 載入的行為；原生端改為系統字體後備 |
...
- **寬螢幕雙欄佈局（3.4）未做**。目前限寬置中已能應付桌面閱讀；雙欄需要重新設計資訊層級，不是單純的版面切換。
- ~~棋盤模式因不允許重複選子，2 顆棋時仍無法組出乾為天／坤為地~~ ✅ 已於 Session 24 加「允許重複棋子」開關解決。
...
#### Session 32 四路審查未修項（2026-08-24，共 25 條）

Session 32 修掉了「線上壞掉」與「靜默毀資料」兩層；以下為同一批審查確認
存在、經原始碼核對但當輪未動手的項目。詳細說明見 WORKLOG「未來待辦」。

🔴 高（僅 web）：

- **A1** web 巢狀 Touchable 連鎖觸發——按分享同時跳到抽棋頁、按愛心同時開啟記錄（`(tabs)/index.tsx:90` 等 3 處）
...
- **A5** `recordUsage` 與 `syncAchievements` 競態，連續天數可能無故中斷
- **A6** 同步失敗一律顯示「尚未設定雲端同步伺服器」，與實情不符
- **A7** 淺色主題對比度不足（2.88:1、3.6:1），未達 WCAG AA
- **A8** `InkSplashOverlay` 不理會 reducedMotion（1.7 秒全螢幕墨滴）
- **A9** 純圖示按鈕無無障礙標籤，觸控目標低於 44pt
- **A10** `pieceCountPreset` 設定存得進卻從不被讀；孿生問題：`drawAnimationSpeed` 有人讀卻無 UI
- **A22** 用神兩現取最先出現者而非發動者，回頭生剋與進退神少計

🟢 低：

- **A11** 空名稱／空白名稱靜默無反應、姓名不 trim 也不限長
- **A12** `+html.tsx` 的 `lang` 寫死 zh-TW、`theme-color` 固定深色（僅 web）
- **A13** 圖鑑「隨機籤詩」展開的卡片多半在畫面外，按了像沒反應
- **A14** 首頁中平與下下配色相同，視覺層級不辨
- **A15** 收藏頁搜尋仍只比對原始中文（圖鑑同型缺陷已於 S32 修掉）
- **A16** 儲存失敗時的未處理 rejection——抽棋永遠停在「解讀中…」
- **A17** 統計「本週／本月」是滾動視窗，與標籤不符；未來時間戳永遠被計入
- **A18** `all_levels` 成就可被無法辨識的等級字串解鎖
- **A19** `medianVerifyDelay` 時鐘變動時可能顯示「-3 天」
- **A20** `buildBackup` 遇單一壞鍵整份失敗，與讀取端的降級策略不一致
- **A21** ErrorBoundary 蓋住整個 App，「重試」必炸時沒有任何出路
- **A23** 日干支用裝置本地日期而非台北時間（目標客群 UTC+8，實務無礙）
- **A24** 棋子→八卦分佈嚴重不均（乾/坤 1/1024 vs 兌/艮 49/1024，紅黑錯卦配對的副作用）

❓ 待實機確認：

- **A25** 分享卡以 `opacity: 0` 離屏渲染截圖，iOS 有產生空白 PNG 的回報，需實機驗證

#### 外部資源／產品決策（與 WORKLOG 同步）

- Vercel 加 `DEEPSEEK_API_KEY`（AI 解讀最後一步；前端已會降級）
- iOS/Android 實機測試（Expo Go）→ EAS Build → 上架（App Store $99/年、Google Play $25）→ 自訂域名
