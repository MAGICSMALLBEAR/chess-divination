...
- iOS/Android 實機測試（Expo Go，26 項清單）→ EAS Build → 上架
  ...
  分享圖片有內容（A25 防線的現場驗證）
- **功能候選路線圖**（Session 61 提出，尚未排優先序）：占驗回訪流程、
  記錄匯出閱讀報告、雲端同步正式化、個人化統計洞察、牌陣擴充包、
  多語內容校閱、匿名產品回饋——七項的價值與「開始前需決定什麼」逐項
  列在 `WORKLOG.md` 的「未來待辦 › 功能候選」。**這些是待排程的產品決策，
  不是待修的缺陷，兩者不可混列。** 其中「占驗回訪流程」的第一段
  （點提醒直接打開那一筆）已於 Session 62 完成，其餘仍待你決定提醒策略。
