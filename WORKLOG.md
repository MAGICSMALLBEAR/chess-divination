...
**維護待辦一項，不是零**——但也不是產品缺陷，是測試自己的競態（#39）。
「沒有已知程式缺陷等待修復」那半句仍然成立；這裡不再寫「為零」，是因為
**那句話每一次寫下來之後都被推翻過**（S57／S58／S59／S62 各一次，
理由見 `DEVELOPMENT_PLAN.md` 的引言），而這次至少還有一件已知沒做的事。
下列「功能候選」是下一輪產品路線圖，須先選定優先級才進入實作，
不可與缺陷待修混列。

| # | 待辦 | 卡在什麼 | 為什麼還沒做 |
|---|------|---------|------------|
| 39 | **`/settings 可正常載入` 的 strict 衝突** | **什麼都不缺** | 它是競態——單獨跑 3 次全過，跟整份套件一起跑才重現（228 條裡 1 flaky）。修法要動 e2e 的定位方式（給頁目標題一個 testID，或改用 `getByRole`）並重跑全套驗證，收尾時不動它。診斷見 Session 68 |
