...
- **A24 待產品決策**：`仕/士` 與 `兵/卒` 共用同一組錯卦（艮/兌），
  造成艮兌各 7 子、乾坤各 1 子，兩顆棋時乾為天／坤為地各約 1/1024、
  兌為澤／艮為山約 49/1024。抽棋本身均勻無模除偏差，這是紅黑錯卦配對
  的副作用而非 bug。若要調整，把 `仕/士` 併到 `乾/坤`、`兵/卒` 獨佔
  `艮/兌` 可讓分佈從 1–7 收斂到 3–5（機率差距 49 倍 → 約 2.8 倍），
  代價是「仕守宮不出」配 `艮山` 的意象要換成 `乾天`。取捨在於
  「忠於棋盤實際子數」與「卦象機率均衡」哪個優先，屬產品決策。
...
處理掉 8 條，Session 34 再清掉 15 條（含 1 條判定為不宜照做），
**剩餘 2 條列於下表**：一條要產品決策，一條要實機。
