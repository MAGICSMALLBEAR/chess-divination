...
| 項目 | 數值 |
|------|------|
| 原始碼檔案 | 113 個（`src` 的 .ts/.tsx，不含測試） |
| Git Commits | 166 次 ※ |
| Jest 測試 | 1362 個 ※ · 64 套件 · 全部通過 |
| E2E 測試 | 310 個 ※ · Playwright · mobile + desktop |
| TypeScript | 零錯誤 |
| 頁面 | 17 個 |
| 元件 | 29 個 |
| Hooks | 12 個 |
...
| # | 待辦 | 你要做的 | 之後我能接手 | 成本 |
|---|------|---------|------------|------|
| 3 | **iOS/Android 實機測試** | `npx expo start --go`，手機掃碼；依 `NATIVE_TESTING.md` 驗 32 項 | 依你回報的現象修 | 免費 |
...
完整清單見 `NATIVE_TESTING.md` 第一階段（32 項，已標明 Expo Go 驗得到與驗不到）。
...
| 15 | **節氣與農曆** | **什麼都不缺** — 節氣近似式與月建早已在 `date.ts`；農曆用平台曆法 | 首頁看得到今天的農曆、節氣、月建與當令五行，而且與卦盤用的是同一份答案 | 已定：只列曆法事實，不做擇日宜忌 |
...
**目前候選裡「我能自己往前推」的是空的**：1／2／4／5／8／9／10／11／12／13 已結案，
