...
| 原始碼檔案 | 147 個 |
| Git Commits | 113 次 |
| Jest 測試 | 1019 個 · 51 套件 · 全部通過 |
| E2E 測試 | 162 個 · Playwright · mobile + desktop |
| TypeScript | 零錯誤 |
| 頁面 | 15 個 |
| 元件 | 22 個 |
| Hooks | 12 個 |
...
- **《靈棋經》125 卦目要不要進圖鑑？** 圖鑑目前只收 64 籤詩，靈棋擲出來的
  卦目只能在歷史與收藏裡回頭看，沒有可瀏覽的目錄。做起來不難（原典已
  入庫、`poemList.ts` 也已認得靈棋記錄），但它是產品決策不是缺陷：
  圖鑑的定位是「籤詩圖鑑」還是「所有占卜文本」，決定了要不要分頁籤。
