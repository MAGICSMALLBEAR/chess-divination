...
| 項目 | 數值 |
|------|------|
| 原始碼檔案 | 106 個（`src` 的 .ts/.tsx，不含測試） |
| Git Commits | 156 次 ※ |
| Jest 測試 | 1273 個 ※ · 61 套件 · 全部通過 |
| E2E 測試 | 266 個 ※ · Playwright · mobile + desktop |
| TypeScript | 零錯誤 |
| 頁面 | 16 個 |
| 元件 | 26 個 |
| Hooks | 12 個 |
...
| # | 待辦 | 你要做的 | 之後我能接手 | 成本 |
|---|------|---------|------------|------|
| 3 | **iOS/Android 實機測試** | `npx expo start --go`，手機掃碼；依 `NATIVE_TESTING.md` 驗 29 項 | 依你回報的現象修 | 免費 |
...
完整清單見 `NATIVE_TESTING.md` 第一階段（29 項，已標明 Expo Go 驗得到與驗不到）。
...
| 11 | **同問題複占比對** | **需要先拍板判定規則** — 「同一件事」怎麼認：精確文字比對／同類別＋時間相近／使用者手動確認（傾向後者，見 Session 72） | 六爻「一事不二占」是真實命理禁忌，App 目前對重複問同一件事完全沒提示，也能對照兩次卦象異同 | 判定規則、比對後呈現方式（只給連結 vs 並列六爻差異） |
...
**目前候選裡「我能自己往前推」的是空的**：1／2／4／5／8／9／10／12 已結案，3 卡在你的
Vercel 設定，6／7 需要母語者與後端／法務，11 卡在你對判定規則的拍板（「同一件事」怎麼認）。
下一個動作是回覆 #11 的開放式問題，或先處理 #3 雲端同步接線。
