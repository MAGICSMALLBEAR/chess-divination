...
| 優先 | 候選功能 | 使用者價值 | 開始前需決定 |
|---|---|---|---|
| P0 | **占驗回訪流程** | 兩週後從通知／首頁直接回填結果，提高應驗率資料完整度 | 提醒頻率、是否允許關閉 |
| P0 | **記錄匯出閱讀報告** | 將單筆占卜、六爻與回填結果輸出成可分享／保存的 PDF | 報告版型、隱私遮罩規則 |
| P1 | **跨裝置雲端同步正式化** | 將既有配對碼同步做成可理解的啟用、衝突處理與復原流程 | 是否採 Upstash、資料保留與費用上限 |
| P1 | **個人化統計洞察** | 依問事類別、時間與占卜模式提供「哪些情境較準」的樣本門檻提示 | 最小樣本數與避免過度解讀的文案 |
| P1 | **牌陣擴充包** | 為感情、抉擇、職涯提供專用位置與解讀脈絡 | 首批牌陣清單與命理解讀規則審核 |
| P2 | **多語內容校閱** | 將英／日文的命理術語、典故與行動建議提升到可對外宣傳品質 | 母語校閱者與術語表 |
| P2 | **匿名產品回饋** | 讓使用者回報解讀品質或問題，作為後續優化依據 | 蒐集資料範圍、隱私政策更新與後端服務 |
