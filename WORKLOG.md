...
#### 等你拍板（Session 54 提出，四項）

| # | 項目 | 要決定的 |
|---|------|---------|
| 24 | `shareToFacebook()` 沒有呼叫端 | 做一個真的分享選單（LINE／FB／複製），還是照 S49 的 `playClickSound` 刪掉？分享的降級路徑是二選一確認框，塞不下第三個選項，而 `dialog.web.ts` 刻意不自製 Modal |
| 25 | `stats.tsx` 的 `pct` 算了沒印 | 吉凶分佈只印次數。是想印百分比而沒印完（那就接上去），還是不該印（那就刪）？ |
| 26 | 兩個沒有呼叫端的參數 | `HexagramLines.showLabels`（爻名）、`SplitReading.singleMaxWidth`（註解說「棋盤頁需要覆寫成 720」，但棋盤頁根本沒用 `SplitReading`）。接上去或刪掉 |
| 27 | `問事類別記憶` 三條 e2e 偶發 flaky | 預渲染出按鈕、React 掛上處理器之前的點擊會落空，retry 會過。修法是把「點擊 → 讀設定」整段包進 `expect.poll` |

`ChessBoard.tsx` 的 `isAvailable`／`hasPiece` 兩個死區域變數與 #26 同類，
但純屬內部工具、沒有產品含義，可直接刪，不列為待決。
