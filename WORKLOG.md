...
（上一次盤點正是這樣才挖出財運/健康/學業/出行四陣，見 #4）。

**目前沒有可由我啟動的程式迭代**：八項候選裡 1／2／4／5／8 已結案，
3 卡在你的 Vercel 設定，6／7 需要母語者與後端／法務。下一個動作是上面那段
（雲端同步接線與複驗），不在我這邊。

還懸著的小決策只剩**占驗提醒策略**（目前固定 14 天一次、不能關、不能改天數）
——它不擋任何事，提醒已經在跑，首頁提示（S63）也不需要它先拍板。
