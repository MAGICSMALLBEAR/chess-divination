...
> 這張表曾經連續四個 session 沒人更新（寫著 1064 測試／178 E2E，實際已是
> 1103／204），因為沒有任何東西會在它過期時出聲。現在數得出來的那幾列由
> `overviewTable.test.ts` 對著檔案系統核對，改錯或忘了改都會紅。
> 標「※」的三列沒有辦法在單元測試裡數（測試數要跑完才知道、commit 數要問
> git），收尾時跟著驗證鏈的輸出一起更新。

| 項目 | 數值 |
|------|------|
| 原始碼檔案 | 117 個（`src` 的 .ts/.tsx，不含測試） |
| Git Commits | 170 次 ※ |
| Jest 測試 | 1430 個 ※ · 66 套件 · 全部通過 |
| E2E 測試 | 336 個 ※ · Playwright · mobile + desktop |
| TypeScript | 零錯誤 |
| 頁面 | 17 個 |
| 元件 | 30 個 |
| Hooks | 12 個 |
...
## 今日（9/4）— Sessions 53–56：掃「沒有呼叫端的東西」，從資料夾清到分享鏈

一天五個 session，主題是一條線：**改用「有沒有人呼叫」這把掃描器找缺陷**——
走功能清單問的是「這個功能有沒有傳到那裡」，掃資料流問的是「寫好的東西
有沒有人用」。前者走不到的地方，後者用兩支 grep 就找得到：孤兒匯出與
沒被呼叫過的 `useState` setter。各段細節在下方各自的 session 條目。

| Session | 做了什麼 | 順帶修掉的既有缺陷 |
|---------|----------|--------------------|
| 53 | 資料夾兩半（`setSelectedFolderId` 從未被呼叫 → 點不開；`removeFromFolder` 沒有呼叫端 → 只進不出）——卡片只印三筆預覽、歸錯沒有退路 | — |
| 54 | 資料夾修完露出來的三件事：兩則筆記搜不到、靈棋卡片空棋子格、刪記錄不清資料夾 | 空棋子格是「靈棋沒有的欄位照著有那條路走」的第三個位置 |
| 55 | 分享去處選單（LINE／Facebook／複製＋真的取消）、吉凶分佈百分比、爻名標籤接上、死參數刪除、flaky 修掉 | 二選一確認框沒有「都不要」，取消被迫變成第二個動作 |
| 56 | WORKLOG 總覽表守門（八列有真相來源）＋第三支孤兒掃描（區域變數） | 總覽表連續四個 session 沒人更新，因為沒有東西會在它錯時出聲 |
| 56 續 | 分享鏈兩個「按了沒反應」：取消系統分享被塞第二張選單、沒裝 LINE 按 LINE 靜默無反應 | `shareNative` 的布林同時代表「取消」與「不支援」兩件該做相反處理的事 |

**今日收尾**：tsc 零錯誤 · Jest 1115（54 套件）· E2E 208 全綠。
前三筆 commit 入庫（`bda3fa2` S53–55、`48021ad` S56、`d72472c` S56 續）。
收尾驗證全套 e2e 時抓到 3 條紅，當場修掉後全套重跑 208 全綠：

- **疊棧背景頁的第四種面貌**（S37／S46／S49 之後）：模式頁標題與首頁模式卡
  同名，背景頁留在 DOM 裡（0×0 不可見）仍會被 `getByText` 匹配。
  `[desktop] 抽棋頁顯示類別與數量選項` 是確定性的 strict 誤中；問事類別記憶
  （mobile）與面向按鈕高度（desktop）是同一個碰撞在載入時序上的競態——
  `.first()` 在背景頁掛上 DOM 前後會命中不同元素，無 filter 寫法則時紅時綠。
- 修法：這次比對的就是「可見的標題」本身，不能像 S46 那樣改比別的字串，
  六處 locator（含頁面可達性、內容可見性兩個迴圈）統一改
  `filter({ visible: true })`。先寫探針腳本取樣 0–4 秒每 250ms 的元素數與
  boundingBox，確認背景頁「從掛上 DOM 起就不可見」後才定案，不是憑推測。
- **教訓：「全綠」以全套為準**——收尾只跑子集（例如只跑新增的測試）會把
  既有的 strict 碰撞放過去，收尾驗證要跑全套。

六處修法收在 `test:` commit `3290904`。**程式待辦清空**——剩下的都是外部資源：
Expo Go 實機測試（`NATIVE_TESTING.md` 26 項）、EAS Build preview、
App Store／Google Play 上架、自訂域名。

---

## Session 53 — 資料夾只做了一半：點不開，也出不來（9/4）

**動機**：上一輪的結論是「程式功能沒有本地待辦」。照 S43 留下的規矩，
這句話要以最近一次 commit 為準重新確認，而不是沿用。這次換一種找法——
不靠回想哪個功能該接什麼，改用兩支掃描：**沒有呼叫端的匯出**（S52 的
`deadExports` 只掃六個工具模組，這次擴到全 `src` + `e2e` + `api`）
與**沒有呼叫端的 `useState` setter**。第二支只中一發，而那一發正是答案。

### 一、資料夾點不開（第四筆之後沒有任何地方到得了）

`collection.tsx` 宣告了 `selectedFolderId`，也算好了 `selectedFolder` 與
`folderRecords`——**`setSelectedFolderId` 從來沒有被呼叫過**。整個資料夾
詳細頁的資料都備妥了，就是沒有人打開它。畫面上看得到的後果是：資料夾卡片
只列得下 `recordIds.slice(0, 3)`，右上角的計數卻寫著真正的筆數。歸檔第四筆
之後，那些記錄從資料夾這一側再也點不到。

修法：資料夾卡片的標題列整條可按（`folder-open-<id>`），按下去把第三個
分頁換成該資料夾的記錄清單——沿用 `renderRecordCard`，所以收藏、刪除、
歸檔、占驗標記在資料夾裡都在，不是另做一個閹割版的清單。超過三筆時卡片
底部多一行「還有 N 筆…」，讓「還有東西沒顯示」這件事看得見。
資料夾內容也走 `sortAndFilter`：搜尋框在三個分頁都看得到，只有前兩頁會篩
等於搜尋在這一頁是壞的。

`folderRecords` 取自歷史 ∪ 收藏再去重，而不是只查 history——收藏頁的卡片
一樣歸檔得了，只查 history 會讓「歷史清掉但還在收藏」的記錄在資料夾裡
憑空消失，計數卻還算著它。

### 二、資料夾只進不出

`removeFromFolder()` 早就寫好了（含 `updateSettings` 佇列），**沒有任何
呼叫端**。歸錯資料夾之後唯一的退路是連整個資料夾一起刪，而那會把其他
記錄的歸檔一併弄丟。這是 `summarizeReading`（S39）、`playClickSound`（S49）
的同一個病，只是這次少的那一半是使用者天天會用到的動作。

修法不是再加一顆按鈕，而是把既有的歸檔清單改成**切換**：已歸檔的資料夾
名稱轉金並打勾，再點一次就移出。一個機制、一個入口，在歷史／收藏／資料夾
三個地方都是同一套操作。記錄卡上的資料夾圖示同時改為「已歸檔就轉金」——
否則使用者無從得知這筆進過哪裡，也就不會想到可以點開來移出。

`collection.addToFolder` 與 `a11y.addToFolder` 兩個鍵的字面意思已經不對，
改名為 `collection.folderPick` 與 `a11y.manageFolders`（舊鍵刪除，
翻譯鍵反向覆蓋守門會擋沒人用的鍵）。

### 守門

- 新增「畫面沒有從未被呼叫的 setState」——與 `deadExports` 同一個檔案，
  因為是同一個病的兩半：一半長在服務層的匯出，一半長在畫面層的區域變數。
  **型別檢查對後者永遠沒有意見**：setter 是解構出來的，用不用都合法，
  `noUnusedLocals` 也管不到解構的元素。以臨時檔注入一個沒人呼叫的 setter
  驗過會紅。
- E2E 一條走完「建立 → 歸檔 → 打開 → 移出 → 返回」。**兩次注入驗過會紅**：
  把移出改回只加入（紅在最後的空狀態）、把開啟鈕改成 no-op（紅在打開那一步）。
...
**「功能有沒有做完」問得到的地方，比「功能該不該做」多。** 前五十幾個
session 找漏接靠的是走清單（歷史／收藏／分享／統計／成就／AI／圖鑑），
那是問「這個功能有沒有傳到那裡」；這次找到的兩處走清單走不到——資料夾
在清單上是「有」，它有分頁、有卡片、有計數，只是按下去沒有反應。
**能問得更死的是資料流本身：寫好的東西有沒有人呼叫。** 匯出是一種，
`useState` 的 setter 是另一種，兩種都 grep 得到，都不必記得任何事。

第二個：**孤兒匯出與孤兒 setter 常常是同一個半成品的兩半。**
`removeFromFolder` 沒有呼叫端、`setSelectedFolderId` 沒有呼叫端，
看起來是兩條，其實是「資料夾詳細頁沒做完」這一件事。修的時候要一起看，
否則會只補上其中一半，然後在下一個 session 再找到另一半。

---

## Session 54 — 資料夾修完之後露出來的三件事（9/4）

**動機**：S53 用「掃沒有呼叫端的東西」找到資料夾那一半。這一輪把同一套
掃描往外推（元件 props、元件內算了沒用的區域變數），並補上一個以前沒問過
的問題——**新加的欄位，有沒有回頭接上舊的那層**。三處都不會壞畫面、
不會紅測試，其中一處是 S53 自己把它從看不見變成看得見的。

### 一、使用者自己寫的兩則筆記，搜不到

`recordMatchesSearch` 比對籤詩標題、詩句、棋子與 `questionText`——那段
註解還特地寫了為什麼要收 `questionText`：「它是使用者回頭找某一次占卜時
最好用的線索」。照同一個道理，**使用者自己動手打下的字更該搜得到**，
但 `note`（自由筆記）與 `outcome.note`（占驗自述）兩則都不在比對裡。

成因不是判斷失誤，是時序：寫那段比對時只有 `questionText`，兩則筆記是
後來才加的欄位，加的時候沒有回頭看誰在比對記錄。**S51 的教訓（新增一層
分類時要找所有拿舊那層的值去比對的地方）反過來也成立：新增一個欄位時，
要找的是所有在比對記錄的地方。**

### 二、靈棋記錄的卡片上掛著一格沒有字的方塊

靈棋擲的是卦目、不落子，`drawnPieceChars` 是空陣列。收藏卡照樣畫出
`piecesMini` 那個圓角色塊，於是每一筆靈棋記錄左側都有一格空方塊；
資料夾的預覽列則印成「 · 大通卦」，前面掛著一個沒有左操作元的分隔點。

這是 S44（分享卡的空等級標籤）、S45（模式標籤預設值把靈棋標成棋盤）的
第三個位置。**同一個病灶連續在三個地方出現，共通點是「靈棋沒有的欄位，
畫面照著有的那條路走」**——修法也一樣：沒有內容就整格省略，而不是留一個
空殼在那裡。

### 三、刪掉的記錄永遠留在資料夾裡

`removeHistory` 清歷史、清收藏、記墓碑，**就是沒有清資料夾**。資料夾存的
是 id 不是記錄，於是刪過的 id 一直留著：卡片的筆數把它們算進去，那串 id
還跟著備份與雲端同步一路複製，只增不減。`clearHistory` 同理——清完之後
每個資料夾都還抱著一整份指不到東西的清單。

這個缺陷從資料夾功能上線就在，但**在 S53 之前看不出來**：以前資料夾只印
前三筆預覽，數字對不上沒有地方可以對照。S53 加了詳細頁之後，卡片說「5 筆」
、打開只有 1 筆，同一個資料夾的兩個地方講不同的數字。

修法分兩層，缺一不可：
- **來源**：`removeHistory` 與 `clearHistory` 都呼叫 `pruneFromFolders()`，
  走 `updateSettings` 的佇列（與 `deleteFolder` 同一套），沒有要刪的 id
  時直接返回、不多寫一次設定。
- **既有資料**：修來源治不了已經存下的那些 id。畫面上的筆數與預覽改成
  數「還指得到記錄的」，卡片與詳細頁因此永遠講同一個數字。

資料夾本身在 `clearHistory` 之後保留——那是使用者建的分類，不在「清除
所有歷史」這個確認的範圍內。

### 守門

- Jest：兩則筆記各自命中、沒寫筆記的記錄不誤命中、占驗回填但沒寫自述時
  不出錯；刪記錄／清空歷史後資料夾不留死 id、刪一筆沒歸檔的記錄不動到
  資料夾、收藏過的記錄刪掉時也清得掉。
- E2E：以筆記搜得到記錄、靈棋卡片只有兩個棋子格（三筆記錄）、資料夾筆數
  不把死 id 算進去且與詳細頁一致。
- **四處注入各自驗過會紅**：拿掉 note 比對（搜尋回 0 筆）、讓棋子格無條件
  渲染（3 格而非 2 格）、拿掉 `pruneFromFolders`（3 條 Jest 紅）、
  筆數改回 `recordIds.length`（印出「舊夾2 筆」）。
...
- **`shareToFacebook()` 沒有任何呼叫端**。分享的降級路徑是一個二選一的
  確認框（確認＝LINE、取消＝複製），塞不下第三個選項；而 `dialog.web.ts`
  刻意用瀏覽器原生 `confirm` 而非自製 Modal（理由寫在檔頭：那一層失效
  就是靜默資料遺失）。要嘛做一個真的分享選單，要嘛照 S49 的 `playClickSound`
  刪掉。**這是產品決策，不是缺陷**。
- **三個算了沒用的區域變數**：`stats.tsx` 的 `pct`（吉凶分佈算了百分比卻
  只印次數——是想印而沒印完，還是不該印？）、`ChessBoard.tsx` 的
  `isAvailable` 與 `hasPiece`。
- **兩個沒有呼叫端的參數**：`HexagramLines.showLabels`、
  `SplitReading.singleMaxWidth`（後者的註解寫著「棋盤頁需要覆寫成 720」，
  但棋盤頁根本沒有用 `SplitReading`）。
- **`問事類別記憶` 的三條 e2e 偶發 flaky**：靜態站台預渲染出按鈕之後、
  React 掛上處理器之前的那一下點擊會落空，retry 會過。要修的話是把
  「點擊 → 讀設定」整段包進 `expect.poll`。

### 教訓

**功能做完的那一刻，往往就是舊缺陷第一次看得見的時候。** 資料夾的死 id
存在很久了，但只有在詳細頁把「筆數」和「內容」放在一起之後才成為矛盾。
所以每補完一個功能，值得再問一次：**這個新畫面現在暴露了什麼以前藏得住的
東西？** 這比「還有什麼沒做」更好答，因為答案就在剛改過的那一頁上。

---

## Session 55 — 清掉 S54 列出的四項待決（9/4）

使用者拍板：Facebook **做成真的分享選單**、兩個算好沒顯示的值**都接上去**。
另外兩項（死參數、flaky e2e）我原本列成「待決」是列錯了——它們不需要任何
產品決策，這一輪一併做掉。

### 一、分享去處選單（#24）

原本的降級路徑是一個二選一確認框：**確認＝LINE、取消＝複製**。兩個毛病：
- `shareToFacebook()` 寫好了卻永遠沒有入口（S54 掃出來的孤兒匯出）；
- **「取消」其實是一個動作而不是取消**——想「算了」的人按下去，剪貼簿
  就被悄悄覆寫了。二選一的框裡根本沒有「什麼都不做」這個狀態。

改成 `ShareTargetSheet`：LINE／Facebook／複製文字，外加一個真的取消。

**為什麼可以自製 Modal**：`dialog.web.ts` 檔頭寫著刻意只用瀏覽器原生
`confirm`，理由是刪除／還原／覆蓋這類確認一旦靜默失效就是資料遺失。
分享不是那種操作——沒掛上就是沒分享出去，使用者看得見，也沒有東西不見。
**上一輪我把那段理由套得太寬，這裡更正**。而原生 confirm 只有兩顆按鈕，
本來也塞不下三個去處。

**Facebook 要先複製一份**：`sharer.php` 只認 `u` 參數，`quote` 早已被 Meta
忽略。不複製的話，使用者選了 Facebook 只會得到一個光禿禿的連結，而他要
分享的那首籤詩不見了。故先寫進剪貼簿再開 sharer，並告知「貼上即可」；
沒帶 url 時補上 App 網址，否則是一則完全空白的貼文。

分派邏輯放服務層的 `shareToTarget()`，回傳**訊息鍵**而非直接 notify——
服務層不碰 i18n 與對話框，畫面才決定得了要不要說。揭曉頁與靈棋頁的降級
路徑一字不差，分開寫遲早會有一邊漏掉 Facebook 那段的剪貼簿處理。

### 二、兩個算好卻沒顯示的值（#25、#26 的一半）

**`stats.tsx` 的 `pct`**：吉凶分佈只印次數，而長條的長度是相對「最多的
那一級」，看不出佔全體幾成——那正是這一節該回答的問題。接上去的同時修掉
一個潛在錯誤：**原本的 `pct` 分母是 `total`（全部記錄），但 `levelCounts`
只數有等級的記錄**。靈棋沒有等級，照原式印出來五個百分比會加不到 100%，
而畫面上完全看不出少掉的是誰。分母改為 `leveled.length`。

**`HexagramLines.showLabels`**：接上去，並把線寬 52 收到 44 讓三卦並列後
仍容得下標籤。截圖確認過——爻名讓卦例圖與底下的文字段落對得起來：
圖上的「九二 ●」、「動爻在 九二（第 2 爻）」、動爻專讀的「九二：渙奔其机」、
納甲盤的列首「九二」，本來是四個地方講同一件事而圖上沒有名字。

### 三、`SplitReading.singleMaxWidth`（#26 的另一半）——直接刪

我上一輪列成「待決」是列錯了。它的註解寫著「棋盤頁需要覆寫成 720」，
但棋盤頁用的是自己樣式表裡的 `maxWidth: 720`（board.tsx），而且
「棋盤頁刻意不分欄」是已經釘死並有 e2e 守住的決策。那個呼叫端不存在，
也不會出現。**註解描述的呼叫端不存在時，該查的是它到底有沒有來過。**

### 四、`問事類別記憶` 三條 flaky（#27）——直接修

同樣不需要決策。成因：測的是 `expo export` 出來的靜態站台，按鈕先由預渲染
的 HTML 畫出來，React 掛上 `onPress` 是之後的事；落在這兩者之間的點擊什麼
都不會發生。**Playwright 的可操作性檢查（可見、穩定、可接收事件）看不出
「處理器還沒掛上」**，所以改成重點到設定真的出現值為止（選同一個類別是
冪等的）。

### 順帶：`scrim` 進主題，theming 白名單短一條

新元件的遮罩 `rgba(0,0,0,0.6)` 被 `theming.test.ts` 當場攔下。沒有把新檔案
加進白名單（那份清單只該變短），改為在 `ThemeColors` 補 `scrim` token
（暗色 0.6、淺色 0.45——淺色主題壓 0.6 會像關燈），並把唯一另一個用它的
`CustomCategoriesSection` 一起遷過去，**從白名單移除**。

字型子集因兩個新字（濃、禿，都在註解裡）重跑，+936 bytes。

### 守門

- Jest：`shareToTarget` 六條（LINE 不動剪貼簿、Facebook 先複製再開 sharer、
  沒帶 url 補 App 網址、呼叫端帶了就不覆寫、剪貼簿被拒仍開 sharer 但不謊稱
  已複製、複製成功與失敗各回一個訊息鍵）。
- E2E：三個去處都在（Facebook 也在）、取消真的什麼都不做、選複製才寫剪貼簿。
  剪貼簿改用 `addInitScript` 攔截而非讀真的剪貼簿——`writeText` 需要使用者
  手勢，測試沒辦法先塞一筆進去當對照；攔下來還能直接數「寫了幾次」。
- **兩處注入驗過會紅**：拿掉 Facebook 那一列（第一條紅）、把 onDismiss 接成
  複製（第二條紅）。
- 既有的靈棋與兩軍對壘陣分享 e2e 一併改走選單，順帶變簡單了：原本要分辨
  confirm 與 alert 兩種對話框的常駐 handler，現在只剩一個 alert。
...
**「這需要你決定」也會列錯。** 上一輪我把四項並列成待決，實際上只有一項
（要不要 Facebook 這個出口）是產品決策；另外兩項——死參數、flaky 測試——
我是拿「這是別人寫的」「這跟既有決策衝突」當理由停下來的，兩個理由查一下
就站不住：那個呼叫端從來不存在，而 `dialog.web.ts` 的理由只涵蓋破壞性確認。
**把事情列給對方決定之前，先確認它真的有兩條路。**

另一條：**二選一的確認框不該用來問「你要哪一個動作」。** 它沒有「都不要」
這個狀態，於是取消被迫變成第二個動作——使用者按下去，某件他沒要求的事
就發生了。選項超過一個時要的是選單，不是 confirm。

---

## Session 56 — 日誌自己的第一張表過期了四個 session（9/4）

**動機**：使用者指著 WORKLOG 開頭那張「專案總覽」表的一行。那行寫著
**E2E 178 個**，實際是 204。整張表都在飄：Jest 1064（實際 1103）、
原始碼檔案 149（實際 98）、commits 127（實際 134）。

### 一、沒有真相來源的數字，一定會過期

這與 S51 引導頁那條是同一件事——**數量寫死在文案裡就要有真相來源**——
只是這次過期的是日誌自己，而且是整份文件第一眼看到的東西。它連續四個
session 沒被更新，因為沒有任何東西會在它出錯時出聲。

新增 `overviewTable.test.ts`：解析開頭那張表，對著檔案系統核對。
守得住的八列：原始碼檔案、頁面、元件、Hooks、服務、Jest 套件數、
籤詩 64、靈棋 125。

**守不住的三列標「※」**：Jest 測試數要整套跑完才知道、E2E 數要跑
Playwright、commit 數要問 git，三者都不是單元測試該做的事。守一半好過
守零；標記讓讀的人知道哪幾個數字是快照。順帶把「原始碼檔案」那一列的
**定義寫進表格裡**（`src` 的 .ts/.tsx，不含測試）——沒有寫明定義的數字
本來就守不住，這一列原本就是這樣飄掉的。

新守門以「把頁面數改成 16」注入驗過會紅。它抓到的第一件事是自己：
新增這個測試檔讓套件數 53 → 54。

### 二、順手清掉 S54 列出的最後一項

`ChessBoard.tsx` 的 `isAvailable` 與 `hasPiece` 兩個死區域變數刪除。

同時把找到它們的那支掃描做成守門：**「沒有算出來卻沒人用的區域變數」**。
至此這一家族有三支——孤兒匯出（S52）、孤兒 setState（S53）、孤兒區域變數
（S56）——分別對應「服務層寫好沒接」「畫面層狀態沒接」「函式內算了沒用」。
只看 `const x =` 不看解構（解構出一半不用是常見且無害的寫法），實測全專案
只中那三個，沒有雜訊。以臨時檔注入一個沒人用的區域變數驗過會紅。

三支掃描合起來這兩輪的戰績：`removeFromFolder`、`setSelectedFolderId`、
`shareToFacebook`、`SplitReading.singleMaxWidth`、`HexagramLines.showLabels`、
`stats.pct`、`isAvailable`、`hasPiece`——**其中三個是「功能只做了一半」
留下的線索，不是單純的死碼**。
...
**文件裡的數字和程式裡的常數是同一種東西。** 這個專案已經替引導頁的
模式數、翻譯鍵、字型字元、音效檔案都建了真相來源，卻讓自己的工作日誌
用手抄的數字撐了四個 session。**會過期的東西不分程式或文件，判準只有一個：
它有沒有辦法在錯的時候出聲。**

**守一半好過守零。** 十一列裡有三列在單元測試裡數不出來，那不構成整張表
不守的理由——把數得出來的守起來，數不出來的標記清楚，讀的人就知道哪幾個
數字要自己再確認一次。

---

## Session 56（續）— 分享鏈的兩個「按了沒反應」（9/4）

補完總覽表之後，照慣例回頭走一次剛上線的東西：**S55 的分享去處選單有沒有
在原生也講得通**。兩個問題，都不是選單本身的錯，是它踩到了底下早就在的坑。

### 一、取消系統分享選單，會被塞第二張選單

`shareNative()` 用一個布林回報結果，而 `false` 同時代表兩件相反的事：

| 情況 | 舊回傳 | 該做的 |
|---|---|---|
| 使用者按了取消 | `false` | **什麼都別做**——他剛說了不要 |
| 這台裝置沒有分享功能 | `false` | 端出降級的去處選單 |

於是手機瀏覽器（有 `navigator.share`）的使用者叫出系統分享選單、按取消，
**馬上被塞第二張選單**。首頁的每日運勢更糟：取消之後剪貼簿被靜靜覆寫，
那是他沒有要求過的事。

這個缺陷在 S55 之前就在（那時彈出的是「分享到 LINE？」確認框），是**選單
讓它變得更刺眼**才被看見——與 S54 的資料夾死 id 是同一種發現方式。

改成三態 `'shared' | 'dismissed' | 'unavailable'`。分辨方式兩個平台不同：
原生的取消是 resolve 出 `dismissedAction`，Web 的取消是 reject 一個
`AbortError`，而「沒有分享功能」是 reject 一般的 `Error`（react-native-web
的 `Share.share` 原始碼裡寫著 `'Share is not supported in this browser'`）。

**型別檢查對這次改動完全沒有意見**——三個呼叫端都寫成 `if (nativeOk)`，
字串在布林位置永遠是真，改完之後選單會變成永遠不出現。三處都逐一改成
明確比對 `'unavailable'`。**回傳型別從布林換成聯集時，要去看每一個把它
當條件用的地方，編譯器不會幫你。**

### 二、沒裝 LINE 的人按「LINE」，什麼都不會發生

`shareToLine()` 在原生走 `Linking.canOpenURL('line://…')`，為否時
**什麼都不做**。使用者的視角是：按下 LINE，選單關掉，沒有 LINE、沒有訊息、
沒有任何跡象——就是按鈕壞了。

改為退到網頁版網址（`https://line.me/R/msg/text/?…`）：裝了 LINE 會被
universal link 接走直接開 App，沒裝則落到 LINE 的網頁。能不能完成分享交給
LINE 決定，但「按了有反應」必須成立。

**這一條的舊測試斷言的是 `expect(openURL).not.toHaveBeenCalled()`**——
又一次把缺陷本身當成規格記了下來（`copyToClipboard` 那兩條的前科，
測試檔裡還留著當時的註解）。已改寫並寫明原因。

### 三、實機清單 21 → 25 項

S53 之後上線的東西一項都不在上面（**這是第四次犯同一個錯**：S49 補 14→20、
S50 補 20→21，這次補 21→25）。新增：取消系統分享選單的行為（原生與 Web
是兩條完全不同的路徑，e2e 只驗得到 Web 那條）、沒裝 LINE 時按 LINE、
Facebook 這個去處在原生的實際行為（**去看它合不合理，不是去確認它會過**）、
資料夾詳細頁與歸檔切換。

### 守門

- Jest：`shareNative` 四種輸入各對一個結果（完成／原生取消／Web 取消／
  沒有分享功能）；`shareToLine` 未安裝與 `canOpenURL` 拋錯兩條都要開網頁版。
- E2E：把 `navigator.share` 裝成「取消」時選單不該出現（**等 1.5 秒才斷言**，
  馬上斷言不存在的話任何延遲都會讓它假綠）、裝成「不支援」時選單一定要出現。
- 注入驗過會紅：拿掉 `AbortError` 那一行判斷（＝修復前的行為），取消那條 e2e
  當場紅。
...
**一個布林回傳值同時表示兩件該做相反處理的事，就是缺陷。** 這次是「取消」
與「不支援」，上一次是 `copyToClipboard` 的「分享失敗」與「該複製」。
兩者都不會壞畫面、不會紅測試，症狀是使用者做了一個動作卻得到另一個動作。

**把布林換成聯集型別時，編譯器保護不了你**——`if (someString)` 永遠是真。
改回傳型別要當成改呼叫端，一個一個看過去。

---

## Session 57 — 畫面對使用者說錯話的兩處（9/5）

**動機**：上一輪的結論是「程式待辦清空」。照 S43 的規矩，這句話要以最近一次
commit 重新確認。前一天用的掃描器是「有沒有人呼叫」；那把掃描器問的是
**程式之間**的關係，找不到「程式接好了，但它對使用者說的話是錯的」。
這次換一個問法：**使用者按下去、或看到的那一句，說對了嗎？**

### 一、首頁的分享是第三個入口，兩輪都只修到別的頁

`(tabs)/index.tsx` 的每日運勢分享，在沒有 `navigator.share` 的環境
（桌面瀏覽器全部算在內）：

```
const outcome = await shareNative(...);
if (outcome === 'unavailable') await copyToClipboard(text);   // 修前
```

**按下分享，畫面上什麼都沒發生。** 內容確實被複製了，但沒有任何提示，
而 LINE 與 Facebook 這兩個去處根本到不了——S55 加去處選單時接了揭曉頁與
靈棋頁，這裡沒接；S56 修 `shareNative` 三態時，這裡只改到「取消不再偷偷
覆寫剪貼簿」那一半。**同一條分享鏈，同一個入口，連續兩輪都被跳過。**

修法：接上與另外兩頁一字不差的那一套——`setPendingShareText` → 
`ShareTargetSheet` → `shareToTarget` → `notify(t(messageKey))`。

### 二、收藏頁把「搜尋沒命中」說成「你還沒有任何記錄」

搜尋框在歷史、收藏、資料夾詳細三個分頁都看得到，三份資料也都走
`sortAndFilter`，但空狀態一律說「尚無占卜記錄」＋「開始占卜後記錄將顯示
於此」。對一個存了兩百筆、只是打錯關鍵字的人，這句話不是事實，附的指示
也解決不了他遇到的事。

**資料夾詳細頁最露餡**：標題那行印著「5 筆」，下面同時寫「這個資料夾還沒有
記錄」——兩句話在同一個畫面上互相打臉。與 S54 修資料夾死 id 是同一個判準
（筆數與內容必須對得起來），那次修的是資料，這次是說詞。

圖鑑早就分得清楚（`library.notFound`／`library.notFoundLingqi`），
收藏頁沒有跟上。修法：三個空狀態統一走 `renderEmpty()`，依 `search.trim()`
決定說哪一組；新增 `collection.noMatch`／`noMatchDesc` 三語。
資料夾清單本身**刻意不改**——資料夾不受搜尋影響，「尚無資料夾」始終是實話。

### 守門

兩支都是來源掃描，因為兩處缺陷的共同點是「服務層各自都對，缺的是畫面
有沒有把它們接起來」——型別與單元測試對這種缺口永遠沒有意見。

- `socialShare.test.ts` 新增「分享入口的接線」：掃 `src/app` 底下所有呼叫
  `shareNative(` 的畫面（窮舉出首頁、揭曉頁、靈棋頁三個，數量本身也是斷言，
  避免掃描範圍壞掉時零個檔案全過），每一個都必須分得出 `'unavailable'`、
  端得出 `ShareTargetSheet`、並把 `shareToTarget` 的訊息鍵 `notify` 出來。
- `emptyStateWiring.test.ts`（新檔）：掃收藏頁裡每一個 `xxxData.length === 0`
  的分支都走 `renderEmpty`，且 `renderEmpty` 依 `search.trim()` 分岔；
  另釘住「資料夾清單不走這條」的理由，免得日後有人順手一併改掉。

**注入迴歸**：兩支守門與四條新 e2e 都把畫面還原成 HEAD 的缺陷版本重跑過
——守門紅在該紅的那一個檔（`index.tsx` 紅、另外兩頁維持綠），
e2e 4 條 × 桌面／手機共 8 條全紅。
...
E2E +10（5 條 × 兩專案）：首頁去處選單出得來、首頁取消不寫剪貼簿、
搜尋沒命中說「找不到符合的記錄」、清空搜尋後記錄回來、資料夾裡搜尋沒命中
不會一邊說「1 筆」一邊說「還沒有記錄」。

### 教訓

**「掃有沒有人呼叫」找不到「說錯話」。** 前一天那三支孤兒掃描問的是程式
之間的關係；這兩處的程式全都接上了、也都有人呼叫，錯的是**接上之後對
使用者說的那一句**。這類缺陷只有一種找法：把自己當成正在用它的人，
按下每一個按鈕，讀出畫面上的每一句話，問「這句話現在是真的嗎」。

**同一個功能的第三個入口最容易被跳過。** 分享鏈連續兩輪都只修揭曉頁與
靈棋頁——不是忘了首頁，是每次都從「上次改過的檔案」開始找。
所以這次的守門不是釘住某一頁，而是**窮舉所有入口再逐一斷言**：
新增第四個分享入口而沒接完整條鏈，測試會直接說出檔名。

---

## Session 58 — 文案裡寫死的數字：一個現在就在說謊，其餘等著說謊（9/5）

**動機**：延續上一輪那把掃描器（「使用者看到的那一句，說對了嗎」）。
這次挑一種**掃得到**的說錯話：文案裡寫死的數字。它的特徵是讀起來完全通順，
只有把它跟真相來源擺在一起才看得出不對。

### 一、首頁「快速抽一籤」的副標，兩件事都不成立

`home.quickDrawDesc` 寫的是「直接抽取 2 顆棋子獲得指引」：

1. **沒有「直接抽取」**——那顆按鈕只是 `router.push('/draw')`，
   到了抽棋頁還要選問事面向、選棋數，按下去才抽。
2. **「2」是寫死的**，而抽棋數量是使用者在設定裡選的（1／2／3）。
   把預設設成 3 的人，抽棋頁會把 3 標成「建議」（S34 接的線），
   首頁卻告訴他會抽 2 顆——**兩個畫面同時開著，講的是不同的數字**。

修的是文案不是行為：S34 已經寫下「抽棋是按下去就直接開始，沒有確認那一步，
所以『預設』能有的誠實作用就是把偏好的那顆標出來，而不是替使用者按下去」。
按那個決策，正確的一句是描述它真正會做的事——
「選好面向與棋數，立刻起卦」，**不帶數字**（帶了就會跟設定對不上）。

### 二、真相來源存在，但真正用它的那一段沒在用

`verification.ts` 匯出 `VERIFY_REMINDER_DAYS = 14`，自稱是「建議回填的等待
天數」的真相來源——可是只有統計那一側在用它。**真正決定何時發提醒的
`scheduleVerificationReminder()` 自己寫死 `14 * 86_400_000`**，通知文案
（`notify.verifyBody`「已過 14 天」）又寫死一次。三份同一個事實，
兩份不認得那個常數。今天三個都是 14，所以看不出問題；哪天有人改常數，
統計改了、提醒不改，而且不會有任何跡象。已改為 import 那個常數。

### 守門

新增 `copyNumbers.test.ts`：掃**整張翻譯表**，把文案裡的數字對回真相來源
——籤詩首數（`ALL_POEMS.length`）、靈棋卦目數（`LINGQI_ORACLES.length`）、
棋子池顆數（`ALL_PIECES.length`）、占驗提醒天數（`VERIFY_REMINDER_DAYS`），
**三種語言各自都要掃得到**（否則會變成「只有中文被守住」而 en／ja 靜靜過期）。

兩個設計上的取捨值得記下來：

- **「棋子池有幾顆」與「這次抽幾顆」要分開。** 第一版用 `(\d+)\s*顆棋子`，
  它確實抓到了舊文案的「2 顆棋子」——但日後有人正當地寫「抽 3 顆棋子」
  也會被判成錯。收緊成「從 N 顆棋子中／from N pieces／N の駒から」，
  只認棋子池那個語意。**會亂叫的守門遲早被關掉，那比沒有守門更糟。**
- **日文的「卦」有兩個意思**：「易経64卦」是《易經》的外部事實，
  「125卦を見る」才是我們收了幾個卦目。一條 `(\d+)卦` 會把外部事實
  一起拖下水，所以逐語言列出量詞而不是共用一條正則。

首頁那句另有兩條專屬迴歸（不寫死顆數、不宣稱直接抽取）——它的數字
**不是對不上真相來源，而是根本不該存在**，這種缺陷沒有來源可以對。

**注入迴歸**：舊文案版本 → 兩條專屬迴歸紅、e2e 兩專案全紅；
把「64 首籤詩」改成「63 首」→ 籤詩首數紅，訊息直接指名
`settings.libraryDesc（zh-TW）寫 63，真相來源是 64`。
...
E2E +2（1 條 × 兩專案）：把設定的預設顆數設成 3，斷言首頁那顆按鈕內
不出現顆數、而抽棋頁把 3 標成建議——**缺陷是「兩處對不上」，
只看其中一頁都是通順的，所以測試要同時看兩頁**。
按鈕補了 `testID="quick-draw"`：斷言必須限定在那顆按鈕內，
模式卡的「從 32 顆棋子中隨機抽取」講的是棋子池，那句是對的。

### 教訓

**「有真相來源」不等於「有人在用」。** `VERIFY_REMINDER_DAYS` 存在了很久，
註解也寫明它是什麼，但真正發提醒的那一段從來沒有 import 它——
**常數的價值不在於它被定義，而在於它被使用**；沒有呼叫端的常數與寫死的
字面量完全等價。這是「掃沒有呼叫端的東西」那個家族的另一種面貌：
它有呼叫端，只是**該用它的那一個沒有用**。

---

## Session 59 — 同一個毛病的第四個位置：首頁的空欄位（9/5）

**動機**：前兩輪問的是「畫面說的話對不對」。這次換一個更笨但更有效的問法：
**同一個缺陷以前出現過幾次？還有哪裡沒掃過？** 專案裡最常復發的一族是
「空欄位照著有那個欄位的路走」——S44 空等級標籤、S45 空模式標籤、
S54 收藏卡的空棋子格，三次都在不同畫面。**還沒被掃過的畫面是首頁。**
...
```tsx
<Text style={styles.recentPieces}>{r.drawnPieceChars.join(' ')}</Text>   // 修前
<Text style={styles.recentLevel}>{r.poemLevel}</Text>                    // 修前
```

靈棋擲的是卦目、不落子（`drawnPieceChars` 是空陣列），原典也未載吉凶等級
（`poemLevel` 是空字串）。兩個欄位照畫的結果：**卦名前面空著一格
（樣式寫死 `width: 60`）、後面再空一格。** 收藏卡與資料夾預覽早就判過了
（`length > 0`／`filter(Boolean)`），首頁是唯一沒跟上的那一頁。

修法完全照既有慣例——`drawnPieceChars.length > 0 &&` 與
`recordHasLevel(r) &&`，後者本來就是 `storage.ts` 匯出的判準，
收藏卡一直在用。

### 二、原生清單少了首頁那個分享入口

`NATIVE_TESTING.md` 的 #21「分享去處選單：取消要真的是取消」驗的是揭曉頁。
Session 57 把首頁的每日運勢接上同一條分享鏈之後，**那個入口沒有進清單**
——而且它正是 S56 那個缺陷最嚴重的地方（取消之後剪貼簿被靜靜覆寫），
**只有原生走得到**：Web 的 e2e 只能攔一個假的剪貼簿。新增 #25，21 → 26 項。
（「新功能上線就要同步這份清單」已經是第五次記在這裡。）

### 守門

新增 `emptyFieldRendering.test.ts`：掃 `src/app` 與 `src/components` 底下
**所有把這兩個欄位畫出來的地方**，每一處都要先問過有沒有值
（`drawnPieceChars.length > 0`／`filter(Boolean)`／`|| undefined`；
`recordHasLevel()`／`poemLevel ?`）。守的不是某一頁，而是整個家族——
第五次再犯時會直接指名是哪個檔案。

**只認渲染、不認計算**：`stats.tsx` 拿 `poemLevel` 去分組、`collection.tsx`
拿它排序，那些不會在畫面上留下空欄位；要求它們也加判斷，只會逼人寫
沒有意義的程式碼，然後守門就開始被當成雜訊。

**注入迴歸**：把首頁還原成缺陷版本 → 3 條紅（兩條規則各一，加上首頁專屬
那條），其餘檔案維持綠；e2e 把兩個判斷換成 `true` 重建 → 兩專案全紅。
...
E2E +2（1 條 × 兩專案）：種三筆記錄（兩筆抽棋、一筆靈棋），斷言首頁的
`recent-pieces` 與 `recent-level` **各只出現兩次**，而靈棋那一列的卦名
仍看得到——不是把整列藏起來，是只藏它沒有的那兩格。

### 教訓

**修完一個缺陷，要問的下一句是「這個毛病以前犯過幾次、還有哪裡沒掃過」。**
S44／S45／S54 各修各的那一頁，三次都沒有人回頭問「還有哪個畫面在畫這個
欄位」——而那個問題 grep 得到。這與 S51 的教訓是同一句話的另一面：
**漏的不是原則，是清單**；差別在 S51 漏的是「誰在讀舊那層的值」，
這次漏的是「誰在畫這個可能為空的欄位」。

---

## Session 60 — 成就門檻：四個數字，十二份拷貝，零個真相來源（9/5）

**動機**：S58 的結論是「文案裡的數字要有真相來源」，當時掃的是介面字串。
成就那一層沒掃到，而它正是同一個問題最密集的地方。

### 找到什麼

四個成就的解鎖門檻（`ten_draws` 10、`fifty_draws` 50、`week_streak` 7、
`ten_verify` 10）各自被寫了三遍——中文說明在 `achievements.ts` 的清單裡，
en 與 ja 在 `data/translations/achievements.ts`——**十二份拷貝，而門檻本身
只是條件式裡的字面量**（`totalReadings >= 10`），沒有一份是真相來源。

**今天四個數字都是對的**，所以這不是現行缺陷，是等著發生的那種。
但它的後果特別難查：說明寫「累積 10 次占卜」，門檻若被改成 12，
使用者做到第 10 次沒解開，只會覺得成就系統壞了——**而且他是對的**，
只是壞的不是解鎖邏輯，是那句話。

### 怎麼修

抽出 `ACHIEVEMENT_THRESHOLDS` 並讓四個條件式真的用它——理由與 S58 的
`VERIFY_REMINDER_DAYS` 一字不差：**常數的價值不在於被定義，而在於被使用**。
`all_levels` 刻意不收進門檻表：它的 5 來自 `POEM_LEVELS.length`，
那本來就是真相來源了，再包一層只會多一個要同步的地方。

### 守門

`copyNumbers.test.ts` 增一段：每個門檻的數字都要出現在該成就的中／英／日
說明裡，紅的時候**逐語言指名是哪一句話錯了**。

另有一條反向的：**清單裡任何「說明帶數字」的成就都必須在門檻表裡**。
少了它，日後新增一個「累積 100 次」的成就時這份守門不會跟著長大——
與 S47 那條「手抄 id 的守門不會跟著新項目長大，一律列舉真清單」同一個道理，
只是這次列舉的是「哪些說明寫了數字」。

**注入迴歸**：門檻 10 → 12 → 三條訊息分別指出 zh／en／ja 的說明沒跟上；
把 `fifty_draws` 換成一個沒登記的 `hundred_draws` → 涵蓋那條當場紅。
...
**沒有新增 e2e**，因為這一輪沒有任何使用者看得到的行為改變——條件式換成
同值的具名常數，畫面一模一樣。**守的是文件與程式之間的一致性，
不是畫面**，寫一條 e2e 只會讓人以為它在測別的東西。

收尾那次全套 e2e 有 1 條 flaky（`[mobile] /settings 可正常載入`，重試即過）
——與記錄在案的「4 workers 對 `expo serve` 冷啟動時 mobile 的頁面載入
會逾時」同一類基礎設施 flake，不是本輪改動造成。

### 教訓

**「同一個事實被寫了幾遍」是可以直接數的。** S58 數的是介面字串，
這次數的是成就；兩次都是先數拷貝份數，再問「哪一份是真的」。
四個數字乘三種語言乘兩個檔案——**十二這個數字本身就是警訊**，
不必等它出錯才動手。

---

## Session 61 — Claude Code 交接收尾：可部署性、測試環境與下一步（9/5）

**交接結論**：工作目錄沒有未提交修改；`master` 與 GitHub 遠端同在
`879517c`。這不是「還有一半沒做完」的工作狀態，而是程式層面的待辦已清空、
需要把驗證、部署與下一輪產品決策說清楚的狀態。

### 今日完成

- 依 Expo SDK 57 文件核對目前專案版本組合：Expo 57、React Native 0.86、
  React 19.2，無需升降版。
- `npm run typecheck`、Jest **1,155** 項測試與 `npm run build:web` 均通過；
  Web 靜態匯出產生 16 條路由，Vercel 的 `api/interpret`、`api/sync` 兩支
  serverless function 均在正式部署產物中。
- GitHub 已確認 `master` 最新提交為 `879517c`；Vercel production deployment
  `dpl_2QFeA84XJCcKko8AeS6Zms1taY6H` 為 **Ready**，正式網址
  `https://chess-divination-app.vercel.app` 首頁回應 HTTP 200。
- `api/interpret` 的 GET 回 HTTP 405 是設計使然：端點只接受 POST；不是健康檢查
  失敗，也不應為了讓瀏覽器網址列好看而開放錯誤的方法。

### E2E 環境修復

第一次執行 222 項 Playwright E2E 時，所有案例在啟動前失敗，原因是本機缺少
`chromium_headless_shell`，不是 App 行為回歸。安裝 Playwright Chromium、清除
該次遺留的 Playwright／Expo 測試程序後重跑，首批涵蓋抽棋、靈棋、兩軍對壘、
拖曳棋子池與分享鏈的案例皆恢復通過。

這條記錄的重點是：**缺瀏覽器時不要把「Executable doesn't exist」計入產品
失敗數，也不要用它推論部署壞掉。** 在全新開發機執行 E2E 前，先跑
`npx playwright install chromium`。

### 尚不能由程式取代的驗證

Web 與 serverless function 已上線；但 Expo Go／原生 preview 的分享圖片、推播、
觸覺、字型與原生 API 相對網址行為，仍必須用真實 Android／iOS 裝置測試。
這些列在下方外部資源待辦，並非留下未完成的程式碼。

---

## Session 62 — 占驗提醒知道是哪一筆，卻把使用者丟在統計頁（9/12）

**動機**：Session 61 交接後隔了一週，先照 S43 的規矩把「程式待辦為零」
重新驗一遍：tsc 零錯誤、Jest 1,155、E2E 222 全綠、`build:web` 通過
——交接文件的說法屬實。接著照它建議的下一步（P0「占驗回訪流程」）往下走，
而那條路上第一段就是斷的。

### 找到什麼

占驗提醒排程時寫的是 `data: { screen: '/stats', recordId: record.id }`
——**它一直知道自己在講哪一筆**。但點擊那一側：

- `subscribeToNotificationTaps()` 只取 `screen`，`recordId` 當場丟掉；
- `stats.tsx` 從頭到尾沒有讀 `recordId` 的程式（連參數都沒接）。

於是使用者收到「『龍騰九霄』已過 14 天，記下實際結果吧」，點下去，
App 把他丟在一個通用的統計頁，**要回填哪一筆得自己去歷史裡找**。
`subscribeToNotificationTaps` 的註解甚至就寫著「提醒的用意正是**現在就去
回填那一筆**」——程式沒有做到它自己寫下的那句話。

這與 S53 的「`setSelectedFolderId` 從來沒有被呼叫過」是同一種缺陷：
**資料一路備妥，只差最後有人用它**。型別攔不住（多帶一個欄位沒人讀
完全合法），單元測試也測不到（排程那側的測試早就斷言了 `recordId` 有寫進
payload——斷的是消費那一側）。

### 怎麼修

點通知後改成打開**那一筆記錄本身**，回填介面就長在那一頁上。

要緊的是別把既有的安全性質改掉。`ROUTABLE_SCREENS` 白名單是刻意的
——「通知能帶你去哪」由程式決定，不由外部資料決定。所以：

- 通知**只說 id**，不說路徑。路由仍由 `recordLink()` 依那筆記錄自己的
  `mode` 決定（靈棋的 `poemId` 恆為 0，交給 reveal 會被顯示成籤詩 #1）。
- `targetFromNotificationData()` 同時帶出保底畫面與 id，**不是二選一**：
  提醒排在 14 天後，這期間使用者清掉歷史很正常，記錄不在了仍要有地方可去
  ——點了通知一定要有反應。
- id 仍做型別與長度檢查。它只被當成查詢鍵、永遠不會變成路徑的一部分，
  但**能擋的就擋**，而不是依賴「它目前只被這樣用」。

### 守門

- 服務層：`recordIdFromNotificationData` 的取值、修空白、非字串與過長一律
  不認；`targetFromNotificationData` 在畫面不在白名單時整個不導頁
  ——**帶了 recordId 也救不回一個不在白名單的畫面**。
- 接線（來源掃描）：root layout 必須拿 `target.recordId` 去 `getHistory()`
  查記錄，並 `router.push(recordLink(record))`。少了任何一半，提醒就又只是
  「把 App 打開」。
- **重錨**（S50 的規矩）：原本那條 `router.push(screen)` 因為回呼型別改變
  而不再成立。守的性質沒有變——點了通知一定會導頁——所以改為要求保底那條
  路徑仍在，而不是把斷言刪掉。

**注入迴歸**：把 `_layout.tsx` 還原成丟掉 `recordId` 的版本，兩條接線守門
當場紅，其餘 56 個套件維持綠。

### 為什麼沒有 e2e

這條路徑原生限定——`subscribeToNotificationTaps` 在 web 直接早退，
Playwright 連進都進不去。**測不到的東西不要假裝測得到**，改成把驗證寫進
`NATIVE_TESTING.md` #8，並把那一條原本寫著「應進入 `/stats`」的敘述改掉
（它現在是錯的），另補一個刪掉記錄後的退路情境。

### 順帶

- 字型子集守門紅了一個「閘」字，只出現在註解裡。**為一個使用者永遠看不到
  的字去長大 1.2MB 的字型子集不合理**，改用詞而非重建子集。
- 總覽表的 commit 數停在 142（交接那筆自己沒更新），一併校正。
...
**「這個功能還缺什麼」最快的問法，是去看資料流到哪裡斷掉。** 交接文件把
占驗回訪列成 P0 並說「基礎都已存在」，那句話是對的——基礎確實都在，
包括通知裡那個一直沒人讀的 `recordId`。**功能清單看不出這種缺口，
因為每一項都打勾了；看得出來的是跟著一個欄位走一遍，問「誰寫的、誰讀的」。**

---

## Session 62（續）— 為了排路線圖的優先序，去問了每一項「還缺多少」（9/12）

**動機**：路線圖七項沒有優先序。**順序不能憑感覺排——每一項「還缺多少」
差距極大，而那才是決定順序的東西。** 所以逐項去程式裡查它實際已經有什麼。

### 查出來最重要的一件事：雲端同步不是功能待做，是外部資源待接

`api/sync.ts` 的 `kv()` 在缺 `UPSTASH_REDIS_REST_URL`／`TOKEN` 時直接回 501。
拿一組合法格式的隨機配對碼打線上端點：

```
GET /api/sync  (x-sync-key: <48 hex>)  →  HTTP 501
```

**線上的雲端同步從來沒有被接上資料庫。** 但程式這一側是完整的：
`failureFromStatus()` 早就把 501 分類成 `'not-configured'`，設定頁也早就
逐種失敗給不同說明（`SYNC_FAIL_KEYS`）——**所以這不是程式缺陷，
是那條在路線圖上被列成 P1「功能」的東西，其實只差你在 Vercel 接上
Upstash Redis**。分類錯了，順序就會排錯。

### 順帶抓到的真缺陷：那句說明指向了錯的旋鈕

`settings.syncUnset` 原本寫「請設定 `EXPO_PUBLIC_CLOUD_SYNC_URL` 環境變數」。
兩處都不對：

1. **那是另一個旋鈕**，而且是選用的——web 走同源相對路徑、原生有絕對網址
   預設，端點明明回應了才會有 501。照著設一輩子也不會好。
   這是 S48 那個教訓的重演：**把「設了沒生效」歸咎到錯的那個變數上**。
2. **它是寫給開發者看的字，卻出現在使用者的對話框裡**——使用者設不了任何
   環境變數。

改成告訴他「雲端同步尚未啟用：伺服器端還沒接上資料庫。這不是你的裝置或
配對碼的問題」，然後停在那裡；開發者要的那段資訊移到 i18n 的註解裡，
留在會被讀到的地方。

**守門**：新增「使用者看得到的訊息裡不出現環境變數名稱」——連續大寫底線字
至少兩段即算。含反空轉（規則本身要抓得到那種長相）。
注入迴歸：還原舊文案 → 三種語言全被指名。
...
| 需要什麼 | 項目 |
|---|---|
| 什麼都不缺，可直接做 | 占驗回訪的首頁提示、統計洞察的樣本門檻文案 |
| 只缺你動手一次 | **雲端同步**（Vercel 接 Upstash → redeploy，程式已完成） |
| 缺你先拍板規則 | 牌陣擴充包（首批牌陣與解讀規則）、匯出報告（版型與隱私遮罩） |
| 缺外部人力 | 多語校閱（母語者）、匿名回饋（後端 + 隱私政策） |
...
**排優先序之前要先問「這一項是什麼種類的待辦」。** 路線圖把雲端同步列成
P1 功能，實際上它是一個外部資源待辦——程式早就寫完、而且會誠實地說自己
沒被啟用。**混在一起的代價是排序本身是錯的**：一件只要你點幾下的事，
被排在兩件要先開會決定規則的事後面。

---

## Session 63 — 待回填算得出來，但沒有地方按（9/12）

**動機**：路線圖重排後的第 1 項——它被排在最前面的理由是「什麼都不缺」，
而那句話值得驗一次。驗出來的結果比預期糟一點：不只首頁沒有出口，
**連統計頁那個唯一的出口也只是一個數字**。

### 找到什麼

`pendingVerification()` 很早就存在，算的是「哪些滿了 14 天還沒回填」。
但在此之前用它的只有統計頁一處，而且是這樣用的：

```tsx
{pending.length > 0 && (
  <Text>有 {n} 則兩週前的占卜還沒回填結果。</Text>   // 修前：一段死文字
)}
```

**使用者被告知有 N 筆，然後自己去歷史裡找是哪幾筆。** 占驗提醒那則通知
在 S62 之前也只送到這一頁——於是整條回訪路徑的終點，是一句沒有出口的話。

這與 S53「`setSelectedFolderId` 從來沒有被呼叫過」是同一族：
**資料備妥，只差最後有人用它**。差別在這次不是「沒人用」，是
**用了、但只用來顯示，沒有用來行動**——比完全沒用更難發現，
因為畫面上看得到東西，看起來像是做完了。

### 怎麼修

- **首頁新增待回填提示**（`pending-verify`）：標題說有幾筆，副標**指名最近
  滿期的那一筆**（「最近的是『龍騰九霄』，已過 15 天」），整張可按，
  點下去直接開那筆記錄——回填介面本來就長在那一頁上。
- **統計頁那一行也變成可按的**，去處相同。同一個病修一次就該兩處一起修，
  否則下次又會有人問「為什麼統計頁點不動」。
- 沒有待回填時整塊不顯示；未滿 14 天的記錄不會被催（`pendingVerification`
  的門檻走 `VERIFY_REMINDER_DAYS`，與提醒排程同一個常數，S58 已統一）。

**這一段不需要產品決策**。需要你拍板的是提醒策略（頻率、可否關閉），
那是另一件事——**列在路線圖上的東西不一定整條都卡在決策上**。

### 守門

- Jest（來源掃描，放在 `verification.test.ts`）：首頁必須真的呼叫
  `pendingVerification()`、必須 `router.push(recordLink(pending[0]))`、
  必須有 `pending.length > 0` 的條件。**中間那條是關鍵**——只顯示數字
  就是修前的樣子，而那也會讓前兩條以外的斷言全綠。
- E2E 兩條：種一筆 15 天前未回填的記錄 → 首頁看得到提示、**內文指名那一筆**、
  按下去 URL 帶著 `recordId=pending-1`；反過來種一筆昨天的記錄 → 不該出現提示
  （先等首頁畫完再斷言不存在，否則任何延遲都會讓這條假綠）。

**注入迴歸**：把首頁還原成沒有提示的版本 → 3 條守門與 2 條 e2e（×2 專案）
全紅，其餘維持綠。
...
**「有用到」不等於「有出口」。** 孤兒掃描問的是「這個東西有沒有人呼叫」，
`pendingVerification()` 從頭到尾都有人呼叫——所以三支孤兒掃描全都掃不到它。
真正該問的是**「算出來的結果，使用者能拿它做什麼」**：一個只被拿去
`<Text>` 的計算結果，和一個沒人呼叫的函式，對使用者來說是同一件事。

---

## Session 64 — 一筆全中不該坐在第一列（9/12）

**動機**：路線圖第 2 項「個人化統計洞察」，需決定的是「最小樣本數與避免
過度解讀的文案」。查下去發現**最小樣本數早就決定好了**，只是沒有套到
該套的地方。

### 找到什麼

`bestCategory()` 從一開始就寫著「樣本不足時回 null——三筆全中就宣告
『你問感情特別準』是拿雜訊當訊號」，門檻 5 筆。但分項應驗率的六個區塊
走的是 `breakdownBy()`，它只做兩件事：濾掉沒回填的組、**依應驗率由高到低
排序**。

於是同一個 App：

- 個人洞察那一段說「一筆不算數」（算不出來就整段不顯示）；
- 分項列表把**一筆 100% 的分組排在五筆 80% 的上面**。

**排序本身就是一種宣稱。** 畫面上「第一列」的意思就是「你在這方面最準」，
那句話不該由一次巧合說出口。每列雖然都印了樣本數（`/1`），但那是在
資料誠實的前提下讓排序去說謊——讀的人先看到順序，才看到那個小小的分母。

### 怎麼修

- 門檻抽成 `MIN_INSIGHT_SAMPLES`（原本只活在 `bestCategory` 的預設參數裡），
  `bestCategory` 改用它——**同一個判準只能有一份**。
- `AccuracyBreakdown` 多一個 `enoughSamples`，**由服務層算好**。讓畫面自己
  比 `verified >= 5` 就等於把規則再抄一份，日後改門檻會有一邊沒跟上
  ——那正是 S60 那個「十二份重複、零個真相來源」的病。
- 排序改成：樣本足夠的整批在前，各自再依應驗率、同率則樣本多者在前。
- 畫面上樣本不足的列淡化**並標上「樣本不足」**——淡化只表示「這列比較
  不重要」，說不出為什麼；使用者會以為是自己在這方面比較差。
  區塊底下補一句說明，門檻數字走 `{n}` 佔位取自常數。

### 守門抓到的兩件事

寫到一半被自己的守門攔下兩次，兩次都是真的：

1. **`stats.lowSample` 有鍵沒人用**——我加了翻譯鍵卻只做了淡化、沒做標記。
   翻譯鍵反向覆蓋守門（S36）當場紅。**這正是它存在的理由**：沒有它，
   那個鍵會安靜地躺在翻譯表裡，而畫面上少一句該說的話。
2. **字型子集少一個「拷」字**——只出現在註解裡。與 S62 同一個判準：
   為使用者永遠看不到的字去長大 1.2MB 的字型不合理，改用詞。
...
Jest +3：門檻等於 `bestCategory` 用的那一個（含恰好等於／差一筆的邊界）、
樣本足夠者排在前面即使應驗率較低、同樣不足時彼此仍依應驗率排。
E2E +2：種 5 筆 80% 與 1 筆 100%，斷言**第一列是 80% 那組**、
不足那列有「樣本不足」字樣、區塊底下有說明。

**注入迴歸**：排序拿掉門檻優先 → 單元測試紅在該紅的那條；
畫面還原成修前 → e2e 兩專案全紅。

### 教訓

**決定過的判準要找出它所有該管的地方。** 門檻在 `bestCategory` 裡躺了
很久，寫得清清楚楚、還附了理由，但它只管住一個函式。這與 S51 是同一句話
——**漏的不是原則，是清單**——差別在 S51 漏的是「誰在讀舊那層的值」，
這次漏的是**「誰在替使用者做排序」**。排序是最容易被當成中性的東西，
但它每次都在替使用者下結論。

---

## Session 65 — 牌陣擴充包：財運／健康／學業／出行四陣（9/13）

**動機**：路線圖第 4 項「牌陣擴充包」候選寫著「為感情、抉擇、職涯提供
專用位置」，但查 `spreads.ts` 才發現感情（`relationship`）、抉擇
（`choice`）、職涯（`strategy`）三個早就有牌陣了——候選清單本身的舉例
已經過期。真正對著 `POEM_FACETS` 七個主類別（`general／marriage／
career／wealth／health／study／travel`）盤點，缺的是 **wealth／health／
study／travel 四個，完全沒有專屬牌陣**，問這類問題時只能退回自由佈局。

### 拍板紀錄（先問過使用者再動工）

- 四個缺口一次做完，不分批。
- 佈局依領域客製，不沿用既有的直線三格（`spreads.ts` 已有四種不同幾何：
  時間陣是直線、抉擇陣是下凹 V、關係陣是上凸 ^、策略陣是右上斜線）：
  財運陣改用**三角形**（財源在頂點、守成／耗損分列下方兩側）、健康陣改用
  **中軸對稱的水平排列**（成因／現狀／調養之道同列，現狀置中）、學業陣用
  **左上到右下的下行對角線**（策略陣鏡像方向）、出行陣用**之字形**
  （啟程／落腳同側、途中偏另一側）。

### 怎麼做

`spreads.ts` 開頭原有的話仍然成立：牌陣不改變起卦演算法，只給每個落子
穩定的角色與座標，所以新增四個純屬 `SpreadDefinition` 資料 + 三張 i18n
對照表（名稱／說明／提示，`spreads.test.ts` 已用 `Object.values(SPREADS)`
泛化掃描，不需要另外手寫守門）。`board.tsx` 的牌陣選單本來就是
`Object.keys(SPREADS)` 動態產生，沒有任何手寫清單要改。

健康陣的說明刻意加一句「非醫療診斷或治療建議」——延續本專案一貫的誠實
邊界（AI 解讀正名、雲端同步 501 如實回報），命理文字可以談身心步調，
不能讓使用者誤以為是醫囑。

### 找到並順手修掉的兩處數字

`STORE_LISTING.md` 與 `NATIVE_TESTING.md` 都寫死「六種牌陣」（前者還逐一
唱名）——這正是 S58／S60 一再抓到的病：**寫死在文案裡的數字沒有真相
來源，功能一擴充就沒人記得回頭改**。兩處改為「十種牌陣」，
`STORE_LISTING.md` 補上四個新名稱。

### 驗證

`tsc --noEmit` 零錯誤；`jest` 57 套件 1172 全過（既有守門測試泛化掃描
`SPREADS`，新牌陣自動被涵蓋，測試數不變）；`playwright test` 228 個
（226 過＋2 個與本次改動無關的 `/settings` 頁 flaky，屬已知的
`expo serve` 冷啟動基礎設施問題，非新增迴歸）。另外重新 `expo export`
後用真瀏覽器逐一點開四個新牌陣截圖核對：名稱／說明／提示/角色鏈
（如「財源 → 守成 → 耗損」）與棋盤上的角色標籤都正確顯示，且棋盤只標示
「下一個要落子的角色」、已落子的角色回填標籤——與既有五個牌陣行為一致，
不是新缺陷。
...
| 項目 | 之前 | 之後 |
|------|------|------|
| Jest | 1172 | 1172（既有守門測試對新牌陣泛化生效，不需新增案例） |
| E2E | 228 | 228 |

### 教訓

**候選清單自己也會過期。** S61 寫下「為感情、抉擇、職涯提供專用位置」時
這三者確實還沒有牌陣；等到 S65 真正要做，`relationship`／`choice`／
`strategy` 已經在更早的 session 做完，候選清單卻沒有跟著劃掉重寫。
排優先序前不能只信任候選清單上的文字，要回頭對真相來源（這次是
`POEM_FACETS`）重新盤點一次缺口在哪。

---

## Session 66 — 記錄匯出閱讀報告：財運/健康/學業/出行陣之後的路線圖第 5 項（9/13）

**拍板紀錄**：格式選長圖（沿用 ShareCardView 的離屏截圖機制，不加 PDF 相依）；
「問題內容」與「自由筆記」預設納入、可關閉；入口單筆（揭曉頁／靈棋頁）與
收藏頁批次匯出都要做。

### 怎麼做

新增 `report.ts`（純邏輯，不碰 RN）：`buildReportSection` 依 `record.mode`
分岔——抽棋／棋盤走既有的 `buildLiuYaoReading` + `buildInterpretation`，
靈棋走 `lingqiOracleByKey` + `buildLingqiInterpretation`，隱私選項在**送進
`buildInterpretation` 之前**就擋下 `questionText`（它會被原樣嵌進解讀文字，
只清 `section.questionText` 不夠，那只擋得住畫面顯示，擋不住文字已經被
烘進解讀段落）。

`ReportCardView` 是新元件，但六爻／納甲／用神斷語**直接內嵌 `LiuYaoPanel`
本體**，不重寫一份——那裡已經有完整規則式解讀（伏神、進退神、三合局全
都在），重寫只會做出一份維護兩次的複本。取捨是 `LiuYaoPanel` 跟隨目前
介面主題色，貼在紙色報告裡不是純白底黑字的宣紙風格，但內容正確、可讀，
勝過重寫一份簡化版。

### 三個在截圖驗證時才抓到的真缺陷

寫完覺得「功能都接上了」，但這次刻意不只跑測試就收尾——照 CLAUDE.md
「UI 改動要開瀏覽器走一次」的規矩，用真瀏覽器分別跑了單筆（抽棋／靈棋）
與批次匯出三條路徑，抓到守門測試完全測不到的三個問題：

1. **`window.open(dataURI, '_blank')` 在 Chrome 會被擋成空白頁**——
   截圖驗證時開出的新分頁 URL 是空的，不是 PNG。data: URI 的導覽早被
   瀏覽器視為安全風險擋下，只是不會報錯，看起來像「好像有反應」。
   改用隱形 `<a download>` 觸發真正的下載手勢，不是導覽，不受這條限制。
   **這正是「有反應」與「有成功」的差距**——按鈕按下去確實發生了什麼，
   但使用者拿到的是一片空白。
2. **直接開網址進 `/lingqi` 會炸出 React 水合錯誤 #418**——`ReportCardView`
   原本在 render 裡印 `dateStr(Date.now())` 當「產生時間」，靜態匯出的
   伺服器預渲染與瀏覽器 hydration 是兩次不同的 `Date.now()`，字串對不上
   就是經典的 hydration mismatch。**只在直接載入路由時炸**，從 App 內用
   `router.push` 點過去的同一頁測不出來——因為那種導覽根本沒有 hydration
   這一步，是純客戶端渲染。與 `PaperTexture` 拒用 `Math.random()`（見
   ShareCardView 檔頭）同一個病因，這次資料換成了時間而不是亂數。修法是
   直接刪掉這行「匯出時間」文字——它從來不是報告的必要資訊，每筆記錄
   自己就帶著日期，不需要另外再印一次「現在幾點」。
3. **hydration 這個病只有真的「重新整理／直接貼網址」才會發作，SPA 內部
   導覽測不出來**——這條教訓比缺陷本身更值錢：往後任何新元件只要在
   render 期間讀了 `Date.now()`／`Math.random()`／`window` 相關的非
   確定性值，都要用直接 `page.goto()` 到該路由驗一次，不能只在應用內
   點過去就算測過。

### 守門測試也攔到三件事（跟之前每一輪一樣）

- `theming.test.ts`：兩個新檔各寫死一個色碼字面量（`#6B9B6B`、`#1A1210`），
  改用既有的 `ShareCardLevelColors['中吉']` 與 `theme.textInverse`。
- `fontSubset.test.ts`：日文新詞「閲読」「深層解釈」用到子集沒收的字，
  重跑 `scripts/subset-font.py`；另外我原本想用「✎」「❓」這類裝飾符號
  標問題／筆記兩欄，符號可以進排除清單但**不符合這個專案自己的路線**
  （B2 早就把 Emoji 全部換成 SVG 圖示），改用「問題：」「筆記：」的文字
  標籤，順便也不用糾結該不該為了兩個符號動字型。
- `overviewTable.test.ts`：新增 3 個服務／元件檔案，WORKLOG 開頭那張表的
  原始碼檔案數／元件數／服務數／套件數全部要跟著改——這張表能守住的原因
  正是每次都會紅一次。

### 測試增減

新增 `report.test.ts`：隱私選項真的擋住 `interpretation` 裡的問題文字
（不是只清欄位）、越界／缺漏卦象資料回傳 `reading: null` 而非拋錯、
靈棋鍵值對不上任何卦目時内容欄位保持空、批次依原順序逐筆組出。
守門測試（`spreads.test.ts` 那套「泛化掃描」手法不適用於這裡——report
不是像牌陣那樣的固定表，改成針對純邏輯函式寫案例）。

| 項目 | 之前 | 之後 |
|------|------|------|
| Jest | 1172 | 1185（58 套件） |
| E2E | 228 | 228（不變，本輪走真瀏覽器逐條截圖驗證，未新增 e2e 案例） |

### 教訓

**這次的三個真缺陷，沒有一個是測試測得出來的**——tsc／jest／playwright
全綠之後才用真瀏覽器走一次單筆與批次兩條路徑，逐一打開下載下來的圖檔
比對內容，才抓到 window.open 開出空白頁與 hydration 炸掉。**「全綠」
只代表已經寫的斷言都成立，不代表功能真的在使用者手上能用**——尤其是
新元件第一次真的離屏渲染＋截圖＋下載這種全鏈路操作，沒有既有測試可以
沿用泛化掃描，必須真的跑一次。

已更新 `NATIVE_TESTING.md` #26（報告匯出在原生的長圖是否截得完整、
分享選單是否正確彈出）與 `STORE_LISTING.md`。路線圖第 5 項完成。

---

## Session 67 — 收尾交接：路線圖現況與雲端同步下一步（9/13）

**交接結論**：工作目錄乾淨，但 `master` 領先 `origin/master` 兩筆
（`458f962` 牌陣擴充包、`51265a7` 記錄匯出閱讀報告），**尚未推送**——
已問過兩次是否要推，使用者都還沒回覆，故保持現狀，不要自己推。
...
| 1 占驗回訪流程 | ✅ S63 |
| 2 個人化統計洞察 | ✅ S64 |
| 3 跨裝置雲端同步 | 🔴 卡在外部資源——下方有接線步驟 |
| 4 牌陣擴充包 | ✅ S65 |
| 5 記錄匯出閱讀報告 | ✅ S66 |
| 6 多語內容校閱 | 🔴 需要母語者人力，我做不了 |
| 7 匿名產品回饋 | 🔴 需要後端＋法務先拍板蒐集範圍，我做不了 |

**程式面能做的路線圖項目已經全部做完**。剩下三項（3／6／7）都不是「我
繼續寫程式」就能往前推的類型，性質各不相同——不要混著問「還有什麼可以
做」，要分開問。

### #3 雲端同步的接線步驟（已給使用者，記錄下來以防之後忘記脈絡）

程式端完全準備好、`api/sync.ts` 已同時認得 `UPSTASH_REDIS_REST_URL`／
`UPSTASH_REDIS_REST_TOKEN` 與 Vercel KV 別名 `KV_REST_API_URL`／
`KV_REST_API_TOKEN`，不用手動對應命名。使用者要做的：

1. Vercel 專案 → **Storage** 分頁 → Create Database → 選 Upstash 提供的
   **Redis**（Marketplace，有免費額度）
2. Connect 到 `chess-divination-app` 專案，環境至少選 **Production**
3. 環境變數不會套用到既有部署（S48 踩過的坑）——連接完要 **Redeploy**
   一次，或推一個新 commit 觸發（今天這兩筆剛好可以順便推）

**驗證方式**：接完並重新部署後，對 `https://chess-divination-app.vercel.app
/api/sync` 打一個帶假配對碼的請求，狀態碼應從 **501**（未設定）變成
**401**（配對碼格式對但查無資料——這才是「有接上」的正常回應，不是
錯誤）。下一輪如果使用者回報已經接好，第一件事是打這支驗證，不要先假設
「接了就一定成功」。

### #6／#7 我能先做、不算數的準備工作

- #6 多語校閱：可以先把命理用語（漢字保留清單、`divinationProse.test.ts`
  逐條登記的那份）整理成一份對照表草稿，但校閱本身要靠母語者判斷道不
  道地，我判斷不了。
- #7 匿名回饋：可以先設計技術方案草稿（存哪裡、格式、要不要 rate
  limit——`api/sync.ts` 的限流模式可以直接借），但蒐集範圍與隱私政策
  要不要更新是使用者的決定，不能我先做完再問，順序反了。

### 開發環境備忘

今天用 `npx expo start --web`（非靜態匯出，帶 HMR）另外起了一個開發
伺服器供使用者手動操作，跑在 `http://localhost:8081`，與 e2e 用的
`expo serve --port 8099`（靜態匯出）是兩條不同路徑，不要混淆——
改了原始碼要看到效果得看 8081 那個，8099 那個要重新 `expo export`
才會更新（S65／S66 驗證時就踩了一次這個時序差）。

---

## Session 68 — 照 S67 交代的驗證方式打了一次，發現使用者已經動手、但接錯了（9/14）

**照著上次交接寫的做**：S67 說「下一輪如果使用者回報已經接好，第一件事是
打這支驗證，不要先假設『接了就一定成功』」。使用者這輪沒有主動回報，但
既然驗證方式已經寫在交接裡、成本又低，主動打一次比等對方想起來要划算。

### 驗證方式本身有一個坑，S67 沒寫清楚

S67 交代「打一個帶假配對碼的請求，狀態碼應從 501 變 401」，但沒說配對碼
要放在哪裡。`syncKey()`（`api/sync.ts`）讀的是 `x-sync-key` **標頭**，
不是 query string；而且格式檢查（`/^[a-f0-9]{48}$/`）發生在**呼叫
Upstash 之前**——送一個隨便格式的字串（例如 `?code=TEST1234`）無論伺服器
有沒有接資料庫都會回 401，因為根本沒走到會去問 Upstash 的那一段程式。
第一次照著「打一個假配對碼」的字面意思用 query string 送，果然拿到看似
「成功」的 401，實際上什麼都沒驗到。改用真的 48 位十六進位字串放進
`x-sync-key` 標頭，才會真的觸發 `kv()` 呼叫 Upstash。**教訓與 S48／S62
同一種**：驗證方式若沒寫清楚怎麼送，下一個照做的人會被格式檢查擋在
半路、卻误以為驗證通過了。

### 真正驗到的狀態：env 已設，但呼叫 Upstash 失敗

用正確格式重打，連續三次都拿到 **HTTP 501 `{"error":"SYNC_UNAVAILABLE"}`**
——不是 S67 記錄的「未設定」那個 501（`{"error":"SYNC_NOT_CONFIGURED"}`）。
讀 `api/sync.ts` 才發現：`kv()` 對「env 變數缺漏」與「呼叫 Upstash 本身
失敗」根本是兩條分支，但**兩者共用轉發邏輯**——沒接資料庫時我們自己判定
回 501；接了資料庫但呼叫失敗時，程式碼是把 Upstash 回應的**原始狀態碼**
原樣轉發（`{ status: response.status }`），這次剛好也是 501，兩種完全不
相干的失敗在客戶端變得無法分辨。使用者確實已經在 Vercel Storage 接了
Upstash（env 有值，否則會走「未設定」那條、回的訊息會是
`SYNC_NOT_CONFIGURED` 而非 `SYNC_UNAVAILABLE`），但接上之後對 Upstash
的呼叫本身持續失敗——原因可能是 URL／Token 沒對應到剛連的那個資料庫、
Marketplace 那端沒把它連到正確的 Vercel 專案，或環境沒有涵蓋
Production。這是外部設定問題，我看不到 Vercel／Upstash 後台，無法再往
下確認，需要使用者自己去檢查連線設定。

### 順手修的程式缺陷：不能讓兩種失敗撞成同一個狀態碼

即使這次是巧合撞上 501，**下一次也可能撞上其他號碼**——`kv()` 對
Upstash 回來的任何非 2xx 狀態都照樣直接轉發，未來換一種失敗方式一樣會
混進「未設定」的分類，使用者已經連過一次資料庫，卻繼續被導向「請去
Vercel 接資料庫」這句已經不成立的說明——與 S62「使用者看得到的訊息裡
出現環境變數名稱」同一種病：**訊息與實情脫節**。

修法比照 `api/interpret+api.ts` 早就用過的分法（`aiPrompt.ts` 對
DeepSeek 的上游錯誤一律轉成固定的 502，不透傳原始狀態碼）：`kv()` 回傳
一個帶 `kind` 的判別聯集（`'not-configured' | 'ok' | 'upstream-error'`），
GET／PUT 各自依 `kind` 決定回應，upstream-error 一律固定回 502
`SYNC_UPSTREAM_ERROR`，不論 Upstash 實際回的是什麼碼；fetch 本身丟例外
（斷線／DNS）也一併收進同一分類，不讓它變成沒接住的例外把整個 function
炸掉。client 端 `cloudSync.ts` 新增 `'upstream-error'` 到 `SyncFailure`，
`failureFromStatus` 認得 502；設定頁新增對應文案
`settings.syncUpstreamError`（三語），內容明講「資料庫已連接，但連線
失敗」，不再叫使用者去設定他已經設定過的東西。

### 測試增減

新增 `apiSync.test.ts`——`api/sync.ts` 部署快兩年（自 A4／A6 就存在）
卻從來沒有自己的端點測試，`cloudSync.test.ts` 只測 client 端「狀態碼
進來後怎麼分類」，沒人測伺服器「什麼情況吐哪個狀態碼」。覆蓋：缺標頭／
格式錯誤配對碼（401，且不呼叫 Upstash）、env 未設（501
`SYNC_NOT_CONFIGURED`）、Upstash 回非 2xx 或 fetch 丟例外（502
`SYNC_UPSTREAM_ERROR`，含「Upstash 剛好也回 501 時客戶端仍看到 502、
不是 SYNC_NOT_CONFIGURED」這條迴歸）、成功讀寫路徑、payload 過大。
`cloudSync.test.ts` 的狀態碼對照表補一列 `[502, 'upstream-error']`。

| 項目 | 之前 | 之後 |
|------|------|------|
| Jest | 1185 | 1197（59 套件，新增 `apiSync.test.ts`） |
| E2E | 228 | 228（不變，純伺服器端與文案改動，無新 UI 行為） |

### 教訓

**驗證方式本身要寫到能被照抄，不能只寫結論。** S67 寫下「應該從 501 變
401」，這句話本身沒錯，但漏了「配對碼放標頭還是查詢字串」「格式要多
精確」——這正是這個系列教訓（S48 環境變數要 redeploy、S62 訊息裡不能有
環境變數名稱）的同一個根：**別人（或未來的自己）照著做時，缺的那一步
會讓驗證看似通過、實則什麼都沒測到**。另外，**「使用者已經動手」不等於
「動手就會成功」**——這次進度是真的（env 從無到有），但下一步不會自動
發生，仍需要具體回報「呼叫 Upstash 失敗」而不是含糊的「還沒接好」。

### 收尾複驗（同日深夜）——生產端狀態又變了一次，而且不是變好

收尾時照上面那條教訓，把複驗**指令一起寫下來並實際再打一次**（照抄版見
「未來待辦」）。結果與本節稍早的記錄不同：

| 時間 | 線上回應 | 意思 |
|---|---|---|
| 今天下午 | 501 `SYNC_UNAVAILABLE` | env 有值，是**呼叫 Upstash** 失敗（舊版程式碼把 Upstash 的原始 501 直接轉發出來） |
| 今天深夜 | 501 `SYNC_NOT_CONFIGURED` | env **對這個部署不可見**——不是 URL／Token 打錯，是那組變數不在 |

同時核對生產端的靜態 bundle（`entry-39056873…js`）含有本節新增的
`settings.syncUpstreamError` 字串，**今天這筆 commit 確實已經部署上去了**
（`origin/master` 也已在 `4269d61`，S67 當時「領先兩筆未推」的狀態已經結束）。
前端與 serverless function 同屬一次部署，所以線上跑的是修完之後的程式碼；
在**新程式碼**下回 `SYNC_NOT_CONFIGURED` 只有一個意思：
`UPSTASH_REDIS_REST_URL`／`_TOKEN`（或 `KV_REST_API_*` 別名）在這個部署裡
讀不到。

**所以下午「你接上了、只是接錯」的結論現在也不成立了**，變成「那組變數
不在這個部署上」。最可能是連線之後 Storage 設定又被動過（重新連接／解除，
或變數只掛在某個環境而這次建置沒涵蓋）——這一步看不到 Vercel 後台，
需要你回去確認。**兩次複驗之間隔不到幾小時，狀態就變了，所以這條的
結論要當成有保存期限的**，下一輪動手前先重打一次。

### 收尾驗證（全套）

- `npm run typecheck` 零錯誤
- Jest **1,197 通過**（59 套件）
- `npm run build:web` 通過（靜態匯出正常產出）
- Playwright **228 條**：227 passed、1 flaky（下一條）——**這次不算全綠**

### 收尾抓到一條 flaky：`/settings 可正常載入`

`[desktop] › 頁面可達性 › /settings 可正常載入` 在整份套件裡第一次跑紅、
重試才過。四項事實：

1. 失敗原因是 strict 模式衝突：`getByText('設定').filter({ visible: true })`
   一次命中 **5 個可見元素**——分頁標籤、頁目標題、外觀設定、體驗設定、
   點擊設定。
2. `getByText(字串)` 比對是**「包含」不是「完全等於」**，所以後三個也中。
3. **`filter({ visible: true })` 在這裡救不了**（S56 之後的統一寫法）：那五個
   都是真的可見，不是疊棧背景頁那種「留在 DOM 但不可見」。
4. 單獨跑 3 次全過（`--repeat-each=3`），只有跟整份套件一起跑才重現——
   所以它是**競態**，不是壞掉。

推測（未經儀器驗證）是斷言有時落在頁面內容掛上之前的空窗，那時只有一個
「設定」可見。修法不能用 `{ exact: true }`——分頁標籤與頁目標題的文字都正好
是「設定」，縮到兩個仍會 strict 衝突；要給頁目標題一個 testID（S45 為分項
區塊加 testID 是同一種解法），或改用 `getByRole` 指到標題本身。

**這是「疊棧同名標題」家族的第五種面貌**：前四種（S37／S46／S49／S56）
都是「不可見卻被匹配到」，這條是**「可見的也不只一個」**——同一把
`filter({ visible: true })` 掃過去會以為已經處理完了。

---

## Session 69 — 問事類別擴充包：官司陣／尋物陣（9/15）

路線圖候選（#6／#7）都卡在外部人力，逐一往下挖了一層——`questionCategories.ts`
的 `POEM_FACETS` 七個主類別（感情/事業/財運/健康/學業/出行/綜合）之外，
六爻傳統另有兩個取法明確、卻完全沒接進來的問事類別：**官司／訴訟**（用神
官鬼，與事業同一取法，京房法本就通用）、**失物**（用神妻財，「物如財也」
是傳統取法）。兩者用神都不需要新的取法分支，接進既有 `useGod.ts` 的
`CANDIDATES` 表即可，不必動 `wenwang.ts` 的判定引擎。

**範圍上做了一個收斂**：原本設想的類別是「失物／尋人」，但尋人的用神
依問卜者與對象的關係而定（如同感情類別依性別而定），App 沒有收集這層
關係輸入，勉強套用會是取法不明確的猜測。比照感情類別「沒有性別就不出
用神」的既有邊界，把這個新類別收斂為單純的**失物**（遺失物品），尋人
留白不做。

### 改動範圍

- `data/poems.ts`：`JieYue` 新增 `lawsuit`／`lostItem` 兩欄，64 首籤詩
  各自撰寫對應卦象的斷語（非模板套用，逐卦扣合卦名典故），中文優先，
  en/ja 留白走既有 fallback（六爻散文 Session 38 同一慣例）
- `useGod.ts`：新增 `lawsuit`（官鬼／喜父母／忌子孫）、`lostItem`（妻財／
  喜子孫／忌兄弟）兩個候選
- `spreads.ts`：新增「官司陣」（己方立場／爭議焦點／裁決走向）與
  「尋物陣」（遺失之因／藏匿之處／尋獲契機）
- `questionCategories.ts` 的 `POEM_FACETS`、`interpretation.ts` 的
  `CATEGORY_FIELDS`、`useQuestionCategories.ts` 的 `BUILT_IN`、
  `questionPrompts.ts`、`aiPrompt.ts` 的 `CATEGORY_LABELS`（AI 深度解讀
  提示詞）、`i18n.ts`（類別/牌陣/格位名稱三語）、`PoemCard.tsx` 的
  `CATEGORIES`（籤詩詳解分頁）全部接上——沿用 Session 47/49 定下的檢查法：
  新類別要走過解讀／牌陣／AI 提示詞／籤詩詳解分頁這幾層，才不會做一半
- 新增 `Icon` 元件的 `lawsuit`（天秤）與 `search`（放大鏡）兩款水墨風圖示

### 順帶確認的既有守門測試都認得新類別

`questionPrompts.test.ts` 的「PoemCard 的分頁鍵就是這幾個面向」與
「七個詳解面向與籤詩的 jieYue 欄位一一對應」兩條測試，原本就是照
`POEM_FACETS` 動態核對，不是寫死七個字面量——這兩條完全沒改測試碼本身，
新增類別後自動要求 `PoemCard.tsx` 也跟著補上，漏補會直接紅。這正是這個
專案偏好「測試核對真相來源而非字面量」的價值所在。

### 測試與收尾

`poems.test.ts`／`useGod.test.ts`／`interpretation.test.ts`／`i18n.test.ts`
補上新欄位與新類別的斷言（未新增 `test()` 筆數，都是既有案例擴充覆蓋範圍，
故總數維持 1197）。新增的中文字元觸發字型子集守門，重跑
`python scripts/subset-font.py` 補上 10 個新字（1389 KB，仍遠低於完整字型）。
`STORE_LISTING.md`／`NATIVE_TESTING.md` 的「十種牌陣」文案同步改為
「十二種」。`tsc --noEmit` 零錯誤、`jest` 1197 全過（59 套件）。

### 收尾：修掉 #39（`/settings` strict mode 衝突）

同一天順手清掉維護待辦清單上最後一項。根源正如 S68 診斷的：`/settings`
頁面標題（`settings.tsx` 的 `t('settings.title')`）與底部導覽分頁標籤
（`tab.settings`）中文都是「設定」，兩者在頁面上同時可見——e2e 原本用
`getByText('設定').filter({ visible: true })` 判斷頁面有沒有載入成功，
遇到「可見的不只一個」就會撞 strict mode violation，跟整份套件一起跑
才會重現（單獨跑時渲染時序恰好只有一個可見）。

修法照 S68 留的兩個選項擇一：給頁面標題加 `testID="settings-title"`，
`/settings` 這條路由改用 `getByTestId` 判斷，其餘路由不動。

**踩到一個部署相關的坑，值得記一筆**：改完程式碼後第一次重跑 e2e 仍然
失敗，這次是「element(s) not found」而非 strict mode violation——`testID`
在 DOM 裡根本找不到。原因是 `playwright.config.ts` 測的是
`expo export --platform web` 的靜態產物（`dist/`），`reuseExistingServer`
本機預設為 true，會直接重用先前建置、尚未包含這次改動的舊 `dist/`。
**改完原生／web 元件的程式碼後，要跑 e2e 前得先 `npm run build:web`
重新匯出**，光改測試碼、不改元件碼的情況才可以跳過這步——這次剛好兩者
都改了，容易誤以為測試碼本身有問題。重新匯出後兩輪完整套件（228 條）
都全綠，收斂為非偶發。

至此路線圖候選裡「我能做的」已經沒有剩餘項目——3／6／7 分別卡在你的
Vercel 設定、母語校閱者、後端與法務決策。
...
- 個人洞察：哪類問事最準（要求足夠樣本）
- 待回填提醒與回填延遲中位數
- 直覺校準：占卜前選填直覺機率（五檔），回填時記下事情本身的結果；統計頁給 Brier 分數、逐檔實際如願率，並在同一批記錄上並列卦與直覺的方向命中數

### 使用者功能
- 歷史記錄 + 收藏 + 資料夾分類（資料夾可開啟瀏覽全部記錄、歸檔可切換移出；
  刪除記錄會一併從資料夾拿掉）
- 同一件事重占：手動連結、並列卦象；收藏頁卡片與報告長圖都看得到連結，雲端同步以 `relatedAt` 墓碑合併
- 記錄搜尋涵蓋籤詩標題與譯名、詩句、棋子、問題本文、自由筆記與占驗自述
- 占卜圖鑑（64 首籤詩與《靈棋經》125 卦目雙分頁，瀏覽/搜尋/展開）
- 統計儀表板（日期篩選/吉凶分佈含佔比/棋子排行）
- 每日運勢
- 圖片卡分享 + 文字分享 + 分享去處選單（LINE／Facebook／複製，系統分享不可用時才出現）
...
## Session 82 — 成就補齊周邊：路線圖 #18，做完（9/26）

使用者照提案拍板。成就 12 → 17 個，補的是 S76–S78 上線、原本沒有任何成就認得的三個功能：

| 成就 | 條件 | 門檻 |
|---|---|---|
| 初窺易理 | 學習模式第一次答對 | — |
| 八卦在心 | 八卦牌八張的複習間隔都到 4 天以上 | `trigram_mastery: 4` |
| 先問本心 | 第一次在占卜前記下直覺 | — |
| 知己知卦 | 既記了直覺、也回填了事情結果的記錄達 10 筆 | `ten_calibrated: 10` |
| 一事再問 | 第一次把兩次占卜連結成同一件事 | — |

### 幾個取法

- **門檻用天數不用格數**：學習頁對使用者說的是「依序 1、2、4、8、16 天」，從沒提過 Leitner 的「格」。
  門檻放進 `ACHIEVEMENT_THRESHOLDS`，`copyNumbers.test.ts` 照舊守三語說明裡的數字（它以成就 id 對門檻鍵，
  第一版取名 `trigram_interval` 就被它攔下）。
- **「答對過」看 `reviews > lapses`，不看格數**：第一次就答對與答錯都落在第一格，看格數分不出來。
  注入驗證：改成 `box > 1`，「第一次作答就答對也算」那條紅。
- **學習進度在另一個儲存鍵**（`learning.ts`），`syncAchievements` 要自己讀進來——漏讀的話條件永遠是假，
  單元測試與 e2e 都走真的儲存函式／真頁面。
- **校準的條件與統計頁共用同一組判斷**（`isIntuitionPct`／`isRealizedStatus`）：壞值在統計頁不算數，這裡也不算。
- **不在各個動作處另外觸發檢查**：顯示成就的只有成就頁，它一打開就重算（`syncAchievements`）；
  雲端同步合併的是已解鎖清單。所以在學習頁、回填、連結處各加一次檢查不會讓使用者多看到任何東西。

### 順帶修掉的：成就圖示會靜靜退回星星

成就原本存 emoji，成就頁再查一張 emoji→圖示對照表，**查不到就退回星星**。新增的五個 emoji 都不在表裡——
照原樣上線，五個新成就會顯示成同一顆星，而且沒有任何測試會紅。改成 `icon` 欄位直接是 `IconName` 型別，
對照表整張拿掉，由編譯器檢查。成就卡片另補 `testID` 與說得出「已解鎖／未解鎖」的 `aria-label`
（原本只靠顏色與鎖頭圖示分辨，讀屏聽不出來）。字型子集的排除清單因 emoji 移出原始碼而跟著縮短，字型檔本身不變。

### 驗證

Jest 1421 → 1430 · E2E 332 → 336（`learn.spec.ts` 加兩條：答對一題後成就頁看得到「初窺易理」解鎖；
預先放入八張間隔 4 天的進度，「八卦在心」解鎖）· tsc 零錯誤。手機寬度截圖看過成就頁的五個新卡片與各自的圖示。

---

## Session 81 — 卦辭＋大象傳：路線圖 #17，做完（9/26）

使用者拍板「連大象傳一起做」。卦典每一卦多了**卦辭**與**大象**（排在爻辭前面）；揭曉頁盤面在動爻之前
多一格「本卦卦辭（火水未濟）」＋大象。卦辭與大象也搜得到（「自強不息」→ 乾）。

### 不憑記憶寫經文：腳本產生，三道查核

比照《靈棋經》（S43 的 `build-lingqi-oracles.mjs`），新增 `scripts/build-zhouyi-texts.mjs` 自維基文庫抓取、
產生 `src/data/zhouyiTexts.ts`（標明勿手改）。卦辭取「周易/屯」這類逐卦頁——**與既有 384 條爻辭同一頁**；
大象取獨立的「周易/大象」頁（標點體例一致）。三道查核任一不過就不寫檔：

1. **逐卦頁解析出的爻辭，逐字出現在已校對的 384 條裡**——證明解析抓對了行、版本是同一個。
   第一次跑就抓到兩件事：後半部的頁面不包 `<span>`（解析漏抓），以及小象同樣以 `*#` 起首（要截在段落標題前）。
2. **兩頁大象互證**（去標點、異體歸一後一字不差）。
3. **卦辭與大象對《周易正義》（十三經注疏本）**——另一個版本；正義把王弼注與孔疏夾在經文中間，
   去注去疏去標點後只比字序。ctext 的 API 要金鑰，所以第二來源選同站的正義。

### 查核逼出來的裁定（都寫在腳本裡，而且例外沒用到就報錯）

- **異體字表只用於比對，不改字**：无／無、于／於、恆／恒、衆／眾、甯／寧、辯／辨、鄉／嚮、遁／遯、曆／歷、脩／修……
  每一組都是這次比對實際遇到的版本異文。畫面上印的是來源原字。
- **剝的大象**：逐卦頁作「山附地上」，大象頁與正義經文都作「山附於地」（正義孔疏也這樣引），取後者。
- **革的卦辭「巳日乃孚」**：正義的經文作「己日」，但同頁王弼注與孔疏都引「巳日」，逐卦頁的卦辭與彖傳也是「巳日」。
  己／巳是《易》學上著名的異文，取「巳日」並把理由寫進異體表旁。
- **履、否、同人、艮的卦名就是卦辭的一部分**（「履虎尾，不咥人，亨。」），沒有冒號——卦辭一律整句照收、含卦名。
- 已知的轉錄瑕疵登記在案：逐卦頁的否（爻名後用逗號）、豐（簡體「觌」）——384 條當初已更正；
  正義的井（一段王弼注混進經文）。**例外清單若有一條沒用到，腳本就報錯**：第一次跑就抓到睽的「多餘空白」
  已被 `clean()` 吸收、那一條例外成了會掩護別的問題的死條目，當場刪掉。

### 只列本卦卦辭，不列變卦的

一爻動的通行讀法是看本卦卦辭與動爻爻辭。兩段卦辭並列，使用者不知道該讀哪一段。變卦的卦辭在卦典查得到。
`LiuYaoPanel` 同時被報告長圖使用，所以匯出的長圖也帶著本卦卦辭——那是文字、不是可按的連結，沒有 S73 那個顧慮。

### 字型子集

經文帶進 25 個介面上從沒出現過的字（牝、綸、瀆、盥、顒、裒……）。這些是真的要印的字，不能像 S80 的「〜」
那樣改寫避開，照 S79 的規則重建子集（+10.9 KB），並比對新舊字表：只多這 25 個、沒有少任何字。

### 單元測試對的是獨立來源

產生檔的卦名來自維基、App 的卦名來自籤詩——**逐卦對得上，才證明腳本沒把卦序排錯**（把第三卦的卦辭掛到第四卦）。
比對時認得無妄／无妄、恒／恆與坎卦原文的「習坎」。另釘住乾、坤、履的全文，剝與革的兩處裁定，
以及經文裡沒有維基標記殘留。

### 沒做

- **學習模式的卦辭牌組**：候選表寫過「可多一副牌」，這一輪沒做——卦辭是整句，四選一容易變成比字面，
  跟詞典沒做成卡片是同一個理由；要做得另想題型。
- 乾坤的「用九」「用六」仍不收（爻辭表只存六爻）；彖傳、小象、文言都不收。
- 經文三語皆印原文；en／ja 的標籤沒有母語者校閱（#6）。

### 驗證

Jest 1411 → 1421 · E2E 332（既有條目加斷言：卦典卡片的卦辭與大象、揭曉頁盤面的本卦卦辭）· tsc 零錯誤。
手機寬度截圖看過揭曉頁的新格子（火水未濟）。

---

## Session 80 — 重新盤點候選（#16–18）＋卦典：路線圖 #16，做完（9/26）

S79 之後「我能自己往前推」的候選是空的，使用者要求重新盤點，確認後直接開做第一項。

### 盤點：四種找法，各找到什麼

| 找法 | 結果 |
|---|---|
| 既有取法（`POEM_FACETS`／`useGod`） | **掃乾了**——九個面向都有牌陣，有明確用神的類別都覆蓋了 |
| 沒有呼叫端的匯出 | **`TRIGRAM_OPPOSITE`（錯卦配對）寫好了從沒人用**；其餘是測試輔助或 `api/` 在用 |
| S73–S79 的「沒做」段落 | 幾乎都是刻意的邊界（宜忌、應期、詞典卡片、學習進度同步）或卡外部，不重開 |
| 新功能有沒有走過周邊系統（S47 教訓） | **成就系統不認識學習、校準、複占連結** |
| 寫好卻讀不到的內容（這次新加的找法） | **384 條爻辭全數校對過，卻只看得到動爻那一條**；圖鑑沒有地方翻 |

收斂成三項，寫進候選表 #16–18：卦典（本輪做完）、卦辭＋大象傳、成就補齊周邊。

### 卦典：圖鑑第三個分頁

64 卦依文王卦序。收合時印卦序、上下卦、卦形、卦名；展開後是**六條爻辭**與**互／錯／綜**三個相關的卦
（可點過去，目標展開並捲進畫面），以及「看這一卦的籤詩」。可以用某一句爻辭搜尋（「潛龍勿用」→ 乾）。
揭曉頁盤面下方加一個入口「在卦典讀本卦六條爻辭與互錯綜（水雷屯）」，帶著卦序過去。

- **不存新資料**：`hexagramCatalog.ts` 的每一欄都取自既有真相來源——卦名取籤詩、爻辭取 `yaoReading.ts`
  （揭曉頁讀的同一份，新增 `getYaoTexts()`）、錯卦取 `TRIGRAM_OPPOSITE`。
- **互卦只剩一份算法**：原本在 `liuyao.ts` 私有的 `buildNuclear` 算式搬到 `hexagram.ts` 的
  `nuclearTrigrams()`，盤面與卦典共用；錯（`oppositeTrigrams`）、綜（`reversedTrigrams`）放在旁邊。
- **錯綜只進卦典，不上揭曉頁**：梅花易數的主軸是本／互／變，盤面再加兩卦只會更擠；它們也不進任何斷語。
  詞典新增「錯卦」「綜卦」兩條（33 → 35），「在本 App」段落照實寫「只在卦典列出、不進斷語」。
- **入口放在揭曉頁而不是 `LiuYaoPanel` 裡**：與詞典入口同一個理由——那個元件同時被離屏的報告截圖使用。
  兩個連結改包進同一個直排容器（原本的 `glossaryLink` 負邊距只適合單一連結）。
- **綜卦是自己的那八卦**（乾、坤、頤、大過、坎、離、中孚、小過）與乾坤的互卦：印「即本卦」、不能按。

### 單元測試對的是獨立的事實，不是拿同一條算式驗自己

- 錯／綜各五組書上查得到的配對（屯↔鼎、屯↔蒙、既濟↔未濟…）；綜卦自己的恰好是那八卦；錯卦沒有自己；
  錯與綜做兩次回到本卦；**64 卦的互卦只落在 16 卦上**。
- **爻辭開頭的爻名（初九、六二…）是經文自帶的，卦形是由卦名推出來的**——兩者互相獨立，
  384 條逐條對得上，才證明爻辭沒放錯卦、沒上下顛倒。
- 卦典的互卦逐卦等於 `buildLiuYaoReading` 盤面上的互卦；爻辭逐條等於揭曉頁 `getMovingLineGuidance` 讀到的。

### 捲動：不當場捲，等 onLayout 帶新座標回來

點關係卦的那一下同時收起來源、展開目標；目標在來源下方時座標馬上會變，當場捲會捲到舊位置。
目標從收合變展開、尺寸必變，`onLayout` 必定帶著新座標回來，由它接手。剛進頁時網格先單欄排一次、
量到寬度才改多欄，座標會再變一次，所以目標留 400ms 再放掉。
**注入驗證**：拿掉「跳過去後等著捲」那一行，e2e「目標捲進畫面」兩個視窗都紅；還原後綠。

### 過程中被攔下的

- **字型子集守門**：日文說明用了「〜」，不在子集裡。依 S74 判準改寫成「二爻から五爻」避開，沒有重建子集。
- **既有的無障礙缺口**：圖鑑分頁用的是 `accessibilityState.selected`，react-native-web 不輸出
  `aria-selected`（S74 記下的同一個缺口）——**S48 圖鑑有第二個分頁以來，網頁讀屏就分不出目前在哪一頁**。
  e2e 斷言「切回籤詩分頁」時才露出來；照 `IntuitionPicker` 的做法補上 `aria-selected`。
- `toBeDisabled` 只認特定 role 上的 `aria-disabled`；「即本卦」那一列刻意不是連結，改成直接看屬性。

### 沒做、以及已知限制

- **卦辭與大象傳還沒有**：卦典目前只有爻辭，是候選 #17（要逐條對維基文庫，另一輪做）。
- 乾、坤的「用九」「用六」不在爻辭表裡（`yaoReading.ts` 只存六爻），卦典也就沒有。
- 爻辭三語都印原文，沿用「爻辭不翻譯」的既有決定；en／ja 的標籤與說明沒有母語者校閱（#6）。

### 驗證

Jest 1380 → 1411（65 → 66 套件）· E2E 320 → 332（`hexagrams.spec.ts` 6 條 × 2 視窗）· tsc 零錯誤。
手機寬度截圖看過列表與展開的水雷屯（卦形由下而上陽陰陰陰陽陰，對）；直接開
`/library?tab=hexagrams&hex=3` 也會停在那一卦並展開。

---

## Session 79 — 節氣與農曆：路線圖 #15，做完（9/25）

接 Session 78，使用者要求依序做的第二項。首頁每日運勢下方新增「今日曆法」卡：

> 農曆 八月十五（丙午年）
> 節氣 秋分（9/23 起）· 下一個 寒露，還有 13 天
> 月建 酉月（白露起）· 秋，金當令
> 日柱 壬寅

### 只列曆法，不做擇日宜忌

黃曆宜忌流派差異大、取法不明確，與「尋人」因用神取法不明確而收斂掉同一個理由。卡片上的每一項都是
算得出來、各家一致的東西，而且**月建與日柱正是揭曉頁六爻盤判旺衰、旬空、六神實際在用的那兩個值**
——卡片把這句話寫在畫面上，並連到詞典的「月建」。

### 真相來源：不多算一份

- **節氣與月建**：`date.ts` 早就有一條二十四節氣近似式（給月建用），這次把它與節氣名表匯出，
  `calendar.ts` 直接用——首頁說的節氣與卦盤用的月建必須是同一個答案。單元測試逐日比對
  `todayAlmanac()` 的月建／季節／當令五行／日柱與卦盤用的函式完全一致。近似式與 2025 全年、2026 已知的
  節氣日期逐一對過；它是日級近似（`date.ts` 早有註明），交節當天可能差一天。
- **日柱**：`sexagenary.ts`。另以「2000-01-01＝戊午」獨立推算 2026-09-25＝壬寅，兩者一致。
- **農曆：用平台的 `Intl` 中國曆，不自己寫資料表。** 200 年的農曆表我無法憑記憶寫對、也難驗證；
  平台曆法由 ICU 維護。對過的日子：2025／2026 春節、2026 除夕（乙巳年臘月沒有三十）、2026 中秋（今天）、
  2023 閏二月、2025 閏六月。解析的是語言中立的數字格式（`8/15`、閏月 `6bis`），再依語言自己排版
  （中文「八月十五」「閏六月初一」「廿一」，其他語言用數字）。
  **平台不支援時整列不顯示**：先確認 `resolvedOptions().calendar === 'chinese'`——不支援的平台會
  靜靜退回公曆、照樣吐出數字，不先確認就解析，會把國曆當成農曆。錯一天的農曆比沒有更糟。

### 過程中被守門攔下的

- **詞典守門**：「`printedAs` 只給名稱沒印在畫面上的條目用」——卡片的說明句印出了「六神」，
  於是六神那條的 `printedAs` 不再需要，拿掉（守門的規則照舊成立，只是事實變了）。
- **字型子集**：閏、廿（中文農曆的寫法）與暦（日文「旧暦」）是**介面上真的要顯示的字**，這次照規則
  重建子集（+964 bytes）；並比對新舊字表，確認只多這三個字、沒有少任何字。日文「扱」只是用詞，改寫避開。

### 時間相關的兩個坑

- **不在 render 裡取今天**：卡片在首頁 focus 時算（S66 hydration #418 的教訓），也讓隔夜切回首頁時換成
  新的一天。e2e 以假時鐘固定在 23:59，切到設定頁、系統時間跳過午夜、切回首頁，要看到「八月十六」；
  改成只在掛載時算，這條就紅（已注入驗證）。
- **Playwright 假時鐘的 `runFor` 會把所有計時器觸發一遍**，背景動畫的計時器讓頁面卡死；只要日期換過去，
  用 `setSystemTime`。e2e 另外用 `timezoneId: 'Asia/Taipei'` 固定時區，結果才不隨跑的那一天與機器改變。

### 沒做、以及已知限制

- **Hermes（原生）是否支援中國曆沒有驗證**——Web（Chrome ICU）確認正確；原生不支援時該列會隱藏。
  已列為實機清單 #33，要確認的是它走到「正確顯示」或「整列隱藏」其中一條，而不是回報支援卻吐公曆數字。
- 節氣是日級近似；1901–2099 以外不顯示節氣。
- 不做節氣推播、不做宜忌。

### 驗證

Jest 1362 → 1380（64 → 65 套件）· E2E 310 → 320 · tsc 零錯誤。手機寬度截圖看過卡片。
實機清單 32 → 33 項。

---

## Session 78 — 易經學習模式：路線圖 #14，做完（9/25）

使用者要「易經學習模式」與「節氣和農曆」依序做下去。本節是第一項；第二項見 Session 79。

### 做了什麼

新頁 `/learn`（設定頁「工具」區、詞典下方進入），三副抽認卡：

| 牌組 | 張數 | 題目 → 答案 | 真相來源 |
|---|---|---|---|
| 八卦 | 8 | 三爻卦形 → 卦名・象（乾・天） | `TRIGRAM_NAMES`／`TRIGRAM_SYMBOLS`、`trigramLine()` |
| 八卦五行 | 8 | 卦 → 五行（五個全列） | `TRIGRAM_ELEMENTS`（體用生剋用的同一張表） |
| 六十四卦 | 64 | 六爻卦形 → 卦名 | `hexagramLines()`、籤詩上的 `hexagramName` |

**題目與答案全部取自既有的真相來源，沒有另寫一份卦資料**——兩份資料遲早會對不上。單元測試逐卡
驗證「卦形還原得回同一卦、答案等於籤詩上的卦名」。

### 幾個決定，以及為什麼

- **排程用 Leitner 五格**（間隔 1、2、4、8、16 天；答錯回第一格、隔天再來），不用 SM-2：
  使用者看得懂「答對了下次隔幾天」，才會信任它排出來的複習。間隔寫在頁首說明裡，數字由
  `LEITNER_INTERVALS` 帶入，不寫死在文案。
- **每次最多 5 張新卡**，而且到期的複習排在新卡前面：一次塞太多新卡，隔天的複習量會一口氣爆開。
- **六十四卦依文王卦序介紹**（乾、坤、屯、蒙…）——讀《易》的人熟悉的順序，也是本 App 籤詩的編號。
- **六十四卦的干擾選項至少兩個與答案共用上卦或下卦**：隨便抽三個不相干的卦名，看上卦就猜得中，
  考不到是不是真的看懂六爻。五行牌則五個全列（從五個裡挑兩個當干擾，是在替使用者刪去答案）。
- **答完換個方向再看一次答案**：六十四卦拆回上下卦（「上卦 坎・水，下卦 乾・天」），八卦補上五行，
  不另寫解說文字。答完選項鎖住，不能改答案刷成答對。
- **進度只存本機、納入備份，不進雲端同步**：同步的合併規則是為占卜記錄訂的，學習進度要合併得另訂一套
  （同一張卡兩台各答過，取哪一台？），這一版不做半套。頁首明寫「進度只存在這台裝置」。
  備份的 `KEY_SHAPES` 是以鍵為索引的型別表，加鍵時編譯器逼著一併寫形狀檢查。
- **讀回的進度逐張檢查**：格數越界、日期格式不對的那一張當成沒學過，不讓整頁拋錯。
- **今天的日期在掛載後才取**（不在 render 裡）：預渲染與 hydration 各取一次，跨午夜會對不上（S66 同一個坑）。

### 無障礙

卦形題原本對讀屏是一張「圖片」——題目等於不存在。`HexagramLines` 加 `accessibilityLabel`，
學習頁給「由下往上：陽、陽、陰」。e2e 正是**讀這句說明來算正解**，順便證明它真的在；選項用
`aria-selected`（S74 缺口的做法）。

### 過程中被守門攔下的

- 備份守門釘死鍵清單——加鍵時該紅，更新並補一條「學習進度可備份、形狀不對整份拒絕」。
- 字型子集攔下日文的「翌」（翌日）：介面字串，改寫成「次の日」，不為一個字重建 1.2MB 子集。
- 本機 tsc 報「/learn 不在路由型別裡」：S73 記過的環境坑（`.expo/types/router.d.ts` 只有
  `expo start` 會重生），短暫跑一次 `expo start` 即解，不是程式錯。

**注入驗證**三種都紅過：干擾選項不挑共用上下卦、答錯不回第一格、新卡排在複習前面。

### 沒做

- 詞典 33 條術語沒做成卡片：答案是兩段散文，四選一會變成比對字面，考不到理解。
- 首頁沒有「今天有 N 張待複習」的提示——先看有沒有人用這一頁。
- 雲端同步（見上）。

### 驗證

Jest 1343 → 1362（63 → 64 套件）· E2E 300 → 310 · tsc 零錯誤。手機寬度截圖看過中英文的牌組清單與六十四卦答題畫面。
實機清單 31 → 32 項（#32 卦形讀屏與重開後進度）。

---

## Session 77 — 補完 S76＋預測校準：路線圖 #13，做完（9/25）

使用者先問「還有什麼沒做、或可以拓展的跨領域功能」，我盤點後建議：S76 已知限制裡能自己做的先補完，
再做「預測校準」（借決策科學的 calibration）。使用者回「S76 補完之後接著做預測校準」。
S76 本身先單獨提交（`7fcf10e`，提交前重跑 tsc＋Jest 1308 確認仍全綠）。

### 一、S76 補完（`9ed73d4`）

S76 寫下的三條限制，三條都做了：

- **雲端同步把連結蓋掉**：新增 `relatedAt`——連結最後一次**設定或取消**的時間，取消時 `relatedTo`
  消失、`relatedAt` 留著，就是墓碑。`mergeHistories` 先照舊整筆選版本（有占驗者勝），再由
  `mergeRelatedLink` 讓連結欄位單獨比 `relatedAt`。刪除造成的清除也更新 `relatedAt`，否則另一台
  同步回來的舊連結會勝出。整筆選版本的規則是為占驗訂的，拿來管連結是借用錯了地方。
- **收藏頁卡片看不出連結**：卡片加「同一件事」小標記。`linkedRecordIds()` 一次算完整份清單，
  規則與 `resolvePrevious` 相同（指不到或指到更晚的，兩端都不標）——卡片標著有、點進去沒有，
  比不標更糟。有一條測試逐筆比對兩者一致。
- **報告長圖沒有連結**：`ReportSection` 帶 `relatedPrevious`（只給日期與題名，**不帶前一次的問題本文**
  ——那屬於另一筆，沒經過它自己的隱私選項）與 `relatedLaterCount`。批次報告對完整歷史查，
  前一次沒被勾選也查得到。真瀏覽器匯出了一張 PNG 打開看過（S66 教訓）。

**順帶修掉的既有缺陷：收藏頁只在掛載時讀資料。** 寫 e2e 時先寫成 `goto` 兩次，發現 `addInitScript`
每次載入都會重灌資料、會把取消的連結灌回去；改走站內返回鍵後，測試當場紅——從揭曉頁取消連結、返回，
標記還在。截圖確認是「回到收藏頁但清單是舊的」，不是導覽問題。這不是新標記獨有的：在揭曉頁回填占驗、
加收藏後返回，清單一樣是舊的。改 `useFocusEffect`（S74 首頁同一個病、同一個修法、同一個測法）。

### 二、預測校準（路線圖 #13）

**要量什麼**：一個說「七成會成」的人，在他說七成的那些事裡，應該真的大約七成成了。占驗簿量的是
「卦準不準」，量不到使用者自己；這一輪補那一半，並讓兩者在同一批記錄上並列。

**取法決定（我依「不猜、不編造換算」提出，使用者 9/25 看過三項理由與替代做法後拍板「照這樣定案」——
不要再當成待決事項提出；日後真要改，位置列在最後）：**

1. **直覺只在看到卦之前問**——三個模式的問題步驟（`IntuitionPicker`），看過卦再填就不是直覺了。
   選填、預設不選、再點一次取消；固定 10／30／50／70／90% 五檔（分得更細是假裝更有把握，
   五檔也讓每一檔攢得到樣本）。
2. **比對基準是「事情本身有沒有如願」，不是占驗結果。** 占驗問的是「卦說中了沒」，拿它給直覺打分數
   等於用卦去評卦。所以回填時**只對記過直覺的記錄**多問一題（如願／部分如願／不如願），存在
   `outcome.realized`，與占驗同生同死。只選它、不選占驗時不給存並說明（否則它會被靜靜丟掉）。
3. **卦不換算成機率。**「大吉等於幾成」沒有典據，編一張表就是造數字。卦只比方向：大吉／上吉／中吉
   看好、下下不看好、中平與靈棋不比；直覺選 50% 不算表態。並列只在兩邊都表態、結果不是「部分」的
   記錄上做，**兩個命中數的分母是同一批**，而且只陳述兩個數字、不下「誰比較準」的結論。
4. **分數用 Brier**（(直覺 − 結果)² 的平均，部分如願計 0.5），旁邊附「同一批每次都選 50%」的分數當
   對照——不是固定 0.25，因為部分如願對 50% 的誤差是 0，要在同一批上實算。
5. 樣本門檻沿用 `MIN_INSIGHT_SAMPLES`（5），不另訂；並列另有自己的門檻（總樣本夠、可並列的不夠時不並列）。
6. 統計頁用完整清單不吃日期篩選（與趨勢圖同理：樣本本來就少）。報告長圖印出直覺（跟著「問題與筆記」
   的隱私開關——那是私下的估計，多藏一項是安全的那一側）與事情結果（跟著占驗，不受開關影響）。

**寫的過程中抓到、測試原本不會抓到的：**

- **直覺在什麼時候清掉。** 第一版在按下抽棋時清——但「重抽」會回到選擇畫面，直覺就沒了；棋盤頁若在
  按解讀時清，陣未成（兩軍對壘缺子）或儲存失敗都會把使用者剛選的那一檔丟掉。改成 `goToResult`／
  `interpret` 回傳「有沒有真的存成記錄」，只在存成時清。靈棋在 `addHistory` 成功後才清。
- **翻譯鍵反向守門擋下 `t(`realized.${status}`)`。** 放行整個 `realized.` 前綴等於連 `realized.prompt`
  一起不查，改成字面量對照表 `REALIZED_LABEL_KEYS`（放在服務層，表單與報告共用）。
- **英文說明不能夾中文等級名**（i18n 守門）——改用文字描述；中文與日文照舊。
- 字型子集守門攔下 7 個新字：6 個只在註解裡（照 S62 判準改用詞），1 個在介面字串（「呢」→「如何」），
  都不為它們重建 1.2MB 子集。
- 新元件的選中狀態改用 `aria-selected`：S74 記下 `accessibilityState` 在 Web 不輸出 aria-selected，
  e2e 因此能直接斷言選中狀態。**既有元件的同一個缺口沒有動**（不在這次範圍）。

**注入驗證**：服務層四種（改用占驗 status 打分、50% 對照寫死 0.25、直覺 50% 算進並列、中平算看好）、
e2e 兩種（只選事情結果就能存、返回抽棋頁直覺沒清），全都紅過；還原後全綠。

### 想改取法時動哪裡

- 五檔改成別的：`INTUITION_CHOICES`（calibration.ts），校準表與選項跟著走。
- 中平要算哪一方、或改用綜合斷語（`finalLevel`）而非籤詩等級：`readingDirection()`。
- 「部分如願」要不要進並列：`computeCalibration` 裡排除 `partial` 的那一行。

### 沒做、以及已知限制

- **沒有「我的直覺 vs 卦」的逐檔圖**，只有文字表——樣本少時一張圖只會讓雜訊看起來像趨勢。
- **AI 解讀的提示詞刻意不帶直覺**：帶了會讓模型順著使用者的期待說。
- 舊記錄沒有直覺，校準只能從今天開始累積。
- en／ja 文案仍未經母語者校閱（#6）。

### 三、全套 e2e 揭露的既有缺陷：頁面捲動之後，棋盤拖曳放錯格

加完直覺選項後全套跑出 3 條紅。兩條是測試假設「目標在首屏」（統計頁多一張卡、棋盤頁多一列選項，
內容被推到視窗外；`elementFromPoint` 與 `mouse.move` 都只認視窗座標）——改成先捲進畫面，
並確認原本要抓的迴歸仍抓得到（遮罩是全螢幕固定定位，捲動後照樣蓋得住）。

第三條不是測試問題。**棋盤位置只在 `onLayout` 用 `measureInWindow` 量一次，那是視窗座標；
頁面捲動不會觸發 `onLayout`**，快取過期，拖曳放下時 `screenToGrid` 換算到錯的格子或落在棋盤外。
截圖上看起來像「落子點錯位」，實量發現落子點相對棋盤的位移在捲動前後完全相同——錯的不是畫的位置，
是換算用的位置。手機幾乎一定要捲動才看得到棋盤，原生 ScrollView 同理，所以這個缺陷在手機上
大概一直都在，只是桌面 900px 視窗剛好不用捲、e2e 又只在桌面尺寸拖。修法：放下那一刻重量一次再換算
（量不到才退回快取）。新增 e2e「頁面捲動之後拖曳，仍落在放手的那一格」：刻意在棋盤量好**之後**
才捲動、選行列不對稱的落點（對稱點比不出行列寫反），並檢查棋盤在視窗裡真的移動過
（`window.scrollY` 恆為 0——捲的是 ScrollView 那層 div）。還原修法後這條紅（0/3，沒落子）。

**教訓：把東西加到一頁的上方，會改變那一頁下方所有東西「需不需要捲動」——那是一個前提改變，
不是單純多一個元件。** 這次正是這個前提改變，讓一個手機上一直存在的缺陷第一次在桌面測試裡現形。

### 驗證

Jest 1308 → 1343（62 → 63 套件）· E2E 282 → 300 · `npx tsc --noEmit` 零錯誤。實機清單 30 → 31 項
（#31 五檔按鈕在原生的排版與讀屏）。另用手機寬度截圖看過三語的直覺選項、回填表單與統計卡
（英文「Not as hoped」最緊，仍在一行內）。

---

## Session 76 — 同問題複占比對：路線圖 #11，做完（9/19）

使用者說「推然後做 11」。先把 9 個 commit 推上 origin（含更早幾次 session 累積、一直沒推的），
再做 #11。S72 留了兩個待拍板的問題（判定規則、呈現方式），使用者只說「做」，沒逐項回答；
我在上一輪明說過「傾向手動確認」，這裡把它當作照那個方向做，並補上我對呈現方式的選擇。

### 三個決定

- **判定規則：手動確認，程式不猜。** 「同一件事」沒有可靠的自動判準（同一句話可以是兩件事，
  換個問法卻是同一件事），猜錯的代價是把兩件無關的事說成同一件。與尋人陣因用神取法不明確
  而收斂掉同源。使用者在揭曉頁按「連結先前那一次」，從較早的記錄裡挑。
- **唯一的自動行為是「建議」，而且門檻極窄：問題文字完全相同**（只忽略空白與英文大小寫，
  不做同義詞、不做相似度；加個問號都不算相同）。建議只是把那一筆點名出來，
  按下「連結」才成立——測試與 e2e 都釘了「沒按之前資料裡不能已經有連結」。
- **呈現方式：並列，不下結論。** 選了「並列六爻差異」而非只給連結：本卦／變卦／動爻／綜合斷語
  兩欄對照，加上「相同／不同」的事實句（本卦、變卦、動爻各一句），以及上一次若已回填占驗
  就帶出結果。**不說該信哪一次**——「一事不二占、通常以第一次為主」是通行說法，
  畫面上註明「這是通行說法，不是本 App 算出來的」。本卦相同但動爻不同的兩次不是同一張盤，
  所以動爻也單獨一句，只看卦名會漏掉。

### 資料設計

`DivinationRecord.relatedTo`：新記錄指向**更早**那筆的 id。
- **只存單向。** 反向的「之後又占過」由查表得出，不存第二份，就不會有兩份互相矛盾的資料。
- **只允許指向更早的記錄**（嚴格早於）：既擋掉自己指向自己，也讓連結不可能成環。
- **顯示端不信任這個 id**：拿去清單裡查，查不到或指到不是更早的記錄都當沒有連結。
- **刪除路徑清連結**（`pruneRelatedLinks`，與 S54 的 `pruneFromFolders` 同一個道理：只在顯示端
  過濾，死 id 會跟著備份與雲端同步一路複製、只增不減）。歷史與收藏兩份副本都清；
  沒有連結指向被刪者時不寫入。
- 六爻卦例重算用**記錄自己的 timestamp**（月令旺衰還原成起卦當時），且與統計頁共用
  `readingForRecord()` 的範圍檢查（順手匯出並補上 timestamp 有限性）——備份還原可能帶進
  越界或損毀的卦象，並列比較若自己重算一份，一筆壞記錄就會紅屏。v1 舊記錄卦序有誤，只給名稱。
- 六爻對靈棋不逐項比（兩套不同的卦），只並列名稱並明說。

### 兩個入口都接了

`RelatedReadings` 自帶資料讀取（與 `AccuracyHint` 同一個做法），揭曉頁與靈棋頁各掛一份。
S49 之後每次都記著「同一個功能的第二個入口最容易被漏」，這次 e2e 直接走靈棋頁，
連六爻對靈棋的不可比較路徑一起走過。

### 過程中的發現

1. **算出來卻沒有輸出（S63 的病，寫到一半自己抓到）**：`compareReadings` 一開始算了
   `sameMoving`，畫面卻沒有任何地方用它。補上動爻那一句，並非為了湊數——本卦相同、動爻不同
   正是這個功能最該讓人看見的差別。
2. **字型子集攔下「瀆」字**（我引《易·蒙》「再三瀆」的原句）。依 S62 判準不重建子集：
   UI 與註解都只保留出處「典出《易·蒙》卦辭」、不引原句。
3. **範圍檢查只該有一份**：`summarizeReading` 起初沒有 reveal.tsx 那套範圍檢查，注入驗證
   （拿掉檢查）證實會讓越界資料進到重算。改為共用 `readingForRecord`。

### 沒做、以及已知限制（請你知道）

- **雲端同步是「整筆記錄選一版」**（有占驗者勝、否則本地勝）。連結沿用這個語意，與 `note`
  完全相同：兩台裝置各有一份沒占驗的副本時，在一台加了連結、另一台沒有，後同步的那台可能
  把連結蓋掉。要做對需要墓碑（取消連結才分得出「沒連結」與「被取消」），那是另一個工程，
  我沒有半吊子地只補一半。
- **收藏頁的卡片、報告匯出（長圖）沒有顯示連結**。連結只在揭曉頁與靈棋頁看得到。
- **占卜當下沒有事前提醒**（要問之前先告訴你「你問過這個」）：那需要在使用者還沒寫完
  問題時就猜相同與否，與「不猜」矛盾；目前是占完之後在揭曉頁建議。
- **傳統說法的措辭未經典籍逐條校對**（「通常以第一次為主」）。已在畫面上註明是通行說法；
  en／ja 沒有母語者校閱。

### 驗證

單元測試 35 條：問題與類別比對（含「加個問號就不算相同」）、候選排序與上限、建議只在完全
相同時出現、`resolvePrevious`／`laterAsks`、越界或舊版卦象只給名稱、六爻對靈棋不可比、
儲存層連結／取消／拒絕自己與更晚與不存在、刪除時清連結（歷史與收藏兩份）。
注入驗證四種都紅過：刪除不清連結、候選含更晚的記錄、連結允許指向更晚、拿掉範圍檢查。
e2e 8 條 × 2 = 16：手動連結→並列→取消、動爻不同如實說不同、完全相同只建議不自動連、
沒有較早記錄整塊不出現、「之後又占過」往返、指向不存在的記錄當沒有連結、靈棋頁入口與
六爻對靈棋、英文版。另用截圖看過選單與並列卡。

Jest 1273 → 1308（62 套件）、E2E 266 → 282、`npx tsc --noEmit` 零錯誤。實機清單 29 → 30 項。
**路線圖上「我能自己往前推」的候選現在沒有了**：#3 卡 Vercel 設定，#6／#7 要外部人力。

---

## Session 75 — 應驗率隨時間趨勢圖：路線圖 #10，做完（9/19）

使用者說「好的繼續開發」。#12 已在前一輪提交（`2972e80`），路線圖裡剩下能推的只有 #10：
它有 S72 提出的四項設計，我把「繼續開發」當作**照那份設計做**（沒有另外逐項確認）。
四項都照做：

- **X 軸是累積第 N 筆已回填的占卜**，不是日曆時間（上線才一個多月，按日曆分段太稀疏，
  且使用者問的是「驗證次數多了以後」，不是「過了幾週」）。
- **滑動視窗 = `MIN_INSIGHT_SAMPLES`（5）**，沒有新造數字；每點直接重用 `computeAccuracy`，
  部分應驗計半分等規則與統計頁其他地方同一套，沒有第二套算法。
- **少於兩個視窗量（10 筆）不畫線**，改顯示「已回填 N 筆，再回填 M 筆就能畫出趨勢」。
- **第一版只做整體，不分維度**（分下去樣本只會更稀）。

S72 沒定的一項由我決定：**依占卜時間排序，不依回填時間。** 問的是「這個人判得越來越準嗎」，
而回填常是隔一陣子一口氣補完，依回填時間排會把同一天補的十筆排成一團，與占卜的先後無關。
單元測試釘了這條（輸入順序打亂、回填時間完全顛倒，結果都不變）。

### 誠實邊界寫在畫面上，不只寫在註解裡

視窗只有 5 筆時，一筆之差就是 10–20 個百分點，折線的起伏大半是雜訊。所以：
- 摘要只陳述「最早 5 筆：30% → 最近 5 筆：90%」兩個數字，**不下「變準了／變差了」的結論**
  （e2e 斷言摘要不含「進步」「變準」）。
- 圖下方一定附上「樣本少、起伏大，請看整體走向，別只看單一個點」，未解鎖時整張圖不出現。
- 用完整記錄而非日期篩選後的資料：橫軸本來就是時間，套「本週」篩選會把折線砍成一兩個點
  （與待回填那一行不吃篩選是同一個理由）。**注入驗證過**：讓它吃 `filtered`，該條 e2e 兩個
  專案都紅，還原後綠。

### 這輪最重要的一條：截圖前的「元素不穩定」揭露了一個所有斷言都抓不到的缺陷

第一版 e2e 全綠（折線 6 個點、摘要文字對、英文版沒有中文），我照慣例要截圖看版面，
Playwright 卻一直報「element is not stable」直到逾時。量了容器寬度：**每次量測長約 22px，
無限增長**——圖寬取自容器的量測值，容器沒有釘死寬度、我又漏扣左右邊框各 1px，內容比容器
內側寬 2px，容器被撐寬、再量測、再撐寬。修法：容器 `width: '100%'`＋`maxWidth`（與
`TrendChart` 同一組寫法）並扣掉邊框。
新增一條 e2e 直接量：畫完之後的一秒內，容器與圖的寬度不准再變。**舊寫法下注入驗證過會紅**
（寬度變了 6 次），修好後綠。
**教訓：凡是「元件量測自己再決定自己大小」的圖表，都要有一條量幾何穩定度的測試——
斷言看到的是某一個瞬間，回饋迴圈只在時間軸上才看得出來。**（這也是為什麼截圖流程本身
有價值：它的穩定性檢查比我寫的任何斷言都嚴。）另外第一個圓點被 SVG 邊界切掉一半，
兩端各內縮 5px 修掉。

### 順帶

字型子集守門攔到兩個字（文案裡的「盯」、註解裡的「桶」），依 S62 判準改用詞、沒重建子集。

### 沒驗證的

- **原生渲染**：SVG 折線由 `react-native-svg` 畫、刻度字是 RN `Text` 疊在上面，寬度取自
  `onLayout`——Web 走的是 CSS、原生走 Yoga，兩者對「寬度回饋迴圈」的反應可能不同。
  列為實機清單 #29。
- **「變準了」的判斷本身**：這一版刻意不給。若你之後想要「趨勢箭頭」之類的東西，那是
  另一個產品決定（需要足夠的樣本與明確的顯著性規則），不是這張圖的下一個小步。

### 驗證

新增：`accuracyTrend` 單元測試 10 條（解鎖門檻、6 個點與 0→100 的逐點值、半分、未回填與
不合法狀態不計入、依占卜時間排序、同毫秒以 id 定序、可自訂視窗、不修改輸入）；e2e 5 條 × 2
（鎖定狀態、折線與提醒、日期篩選不影響、寬度穩定、英文版）。注入驗證：依回填時間排序 → 2 條紅；
就地排序 → 1 條紅；圖寬回饋迴圈 → 紅；趨勢吃篩選資料 → 紅。

Jest 1263 → 1273（61 套件不變）、E2E 256 → 266、`npx tsc --noEmit` 零錯誤。
實機清單 28 → 29 項。**路線圖現在只剩 #11（同問題複占比對）能動，而它要你先拍板判定規則。**

---

## Session 74 — 占驗提醒可調：路線圖 #12，做完（9/19）

使用者說「commit，做 #12 占驗提醒」。先把 S73 提交（`2d25a1d`），再照 S73 提的建議做：
設定頁新增「占驗提醒」——**關閉／7 天／14 天／30 天，預設仍是 14**。固定選項而非
自訂天數：自訂要處理 0 天與負數，使用者真正要的只是「關掉」與「早一點／晚一點」。

### 一個設定值，三個消費者

「滿幾天算該回填」原本是三個地方各自認得的同一件事：通知排程、首頁待回填提示、
統計頁的待回填筆數。開放給使用者選之後，**只改其中一個就是新的不一致**（首頁照
7 天算、通知卻 14 天後才響），所以三個都改走 `verifyReminderPolicy()`
（`verification.ts`，真相來源）：

- **通知**：`scheduleVerificationReminder(record, policy?)` 讀設定；關閉就不排。
  平常占卜完成時不傳 policy、自己讀設定；讀不到就用預設政策照排（寧可多排一則預設天數，
  也不因為儲存暫時讀不出來而漏排）。
- **首頁**：待回填卡片照該天數算；**關閉時整張卡片不顯示**。
- **統計頁**：那一行照該天數算；**關閉時仍照預設天數顯示**——它是使用者主動打開統計頁
  才看得到的資訊，不是打擾，關掉「提醒」不該讓它消失。

### 幾個決定，以及為什麼

- **「關閉」的語意是「不發通知＋首頁不頂著待回填卡片」。** 這是我的解讀，你只批准了
  「關／7／14／30」這組選項，沒說關閉到底關哪些東西。首頁卡片也是一種提醒，
  關了通知卻留著卡片，等於沒關。若你的意思是「只關通知」，改
  `index.tsx` 的 `policy.enabled ? … : []` 一行即可。
- **不認得的設定值退回「啟用＋預設天數」，不是關閉。** 備份與雲端同步會把別處寫的設定
  帶進來；靜靜把提醒關掉比多提醒一次更難被發現。純量設定的雲端合併本來就是本地優先
  （`{...remote, ...local}`），不需要另外處理，這條只管值本身不合法的情況。
- **改設定時整批重排已排好的通知**（`rescheduleVerificationReminders`：先清掉所有占驗
  排程，再替還沒回填的記錄用新天數重排）。只存設定的話，首頁馬上照新天數算，
  而先前排好的通知還是照舊天數響。已經滿期的記錄不會被排（觸發時間在過去），
  由首頁提示接手，與原本的行為一致。
- **選了天數時才問通知權限**（原生）。原本從來沒有這個入口：占驗提醒只在使用者
  「已經」同意通知時才排程，權限完全靠每日提醒開關順帶拿到。沒給權限時設定仍然保留
  （首頁與統計頁的提示照樣用得到），但要當面說「無法設定」，不能讓人以為通知設好了。

### 過程中找到的，而且都不會被原有測試抓到

1. **兩句文案在天數可調之後會變成謊話，其中一句是換個說法的寫死。**
   `notify.verifyBody`「已過 14 天」有 `copyNumbers.test.ts` 守著（對回
   `VERIFY_REMINDER_DAYS`），天數搬進 `{days}` 參數的當下，那條規則的正則變成零命中、
   反空轉檢查紅了——這是它該有的反應。**但 `stats.pending` 寫的是「兩週前」**：
   沒有數字，那條守門根本掃不到，是我讀翻譯表時才看到。守門因此換了判準：這兩句
   三語都必須帶 `{days}`、不得有字面天數、也不得寫成「兩週／two weeks／2週間」，
   並有反空轉（正則確實抓得到「已過 14 天」）。
   **教訓：守門只認「數字」，就守不住「用別的字說同一個數字」。**
2. **首頁只在掛載時讀一次歷史與設定，切分頁回來不會更新。** 分頁掛上後不會卸載，
   使用者在設定頁改成 7 天、切回首頁，待回填卡片會停在舊天數的結果——設定存了、
   選項也亮了、卻沒有生效。改成 `useFocusEffect`（每次回到首頁重讀）。
   這一條**每個單元測試與各頁各自的 e2e 都測不到**，因為它們都用 `goto` 重新載入
   （重新掛載）；只有「用分頁列切換」會踩到。e2e 就是這樣寫的，並**注入驗證過**：
   暫時改回只在掛載時讀，該條在 mobile／desktop 都紅，還原後綠。
   順帶：首頁的「最近記錄」也因此每次回來都會重讀。
3. **版面：截圖才看得出來。** 第一版把標籤與四顆選項放在同一列，英文版最後一顆
   （30 days）超出卡片右緣、日文標籤在字中間斷行；測試全綠。改成標籤在上、選項在下。
4. **e2e 不能斷言 `aria-selected`**：這一顆選項是 `role=button`＋`accessibilityState`
   （專案所有選項的寫法），在 web 上沒有輸出該屬性。改斷言使用者看得到的訊號
   （選中那顆的邊框色與其餘不同）。**觀察，未處理**：這代表至少這類選項在 web 上
   不向輔助科技暴露選取狀態；我只驗了這一顆，專案裡其他同寫法的位置（性別、棋數、
   動畫速度…）未逐一驗證，也不在這次範圍內。

### 沒驗證的（請你知道）

- **通知本身**：Web 沒有本地通知，排程邏輯由 `notifications.test.ts` 驗（觸發時間、
  關閉不排、不認得的值、讀不到設定、整批重排先清後排），但權限彈窗與真的響起來
  只有原生走得到——列為實機清單 #28。
- 字型子集守門攔到「謊」字（只在註解裡），依 S62 判準改用詞、沒重建子集。

### 驗證

測試：`verifyReminderPolicy`（含 8 種不合法值）、通知排程讀設定 8 條、整批重排 3 條、
文案不寫死天數 4 條；真瀏覽器 e2e 新增 7 條 × 2 = 14（預設亮 14 並可切換存檔、首頁
預設不催／7 天催／關閉不顯示、**同 session 分頁切換**、統計頁天數文案、關閉時統計頁
仍在）。注入驗證：排程寫死 14 天 → 5 條紅；首頁改回只在掛載時讀 → session 那條紅。

Jest 1234 → 1263（61 套件不變）、E2E 242 → 256、`npx tsc --noEmit` 零錯誤。
實機清單 27 → 28 項。**這一輪之後，路線圖裡「我能自己往前推」的候選是空的**：
#10／#11 待你確認 S72 提案，#3 卡 Vercel 設定，#6／#7 要外部人力。

**教訓：把一個常數變成使用者設定，要一次找齊三種東西——它的所有消費者（這次三個）、
所有把它說成文字的地方（這次兩句，其中一句沒有數字）、以及設定改變後畫面何時會重讀
（這次首頁不會）。**前兩種 grep 得到，第三種只有走一遍「改設定→切回去看」才知道。

---

## Session 73 — 命理術語詞典：路線圖 #9，做完（9/19）

使用者說「按照進度繼續做下去」。程式待辦是零，路線圖八項裡「我能自己往前推」
的只剩 #9（術語詞典）與 #12（提醒策略，但那要你先決定要不要開放），所以做 #9。
#10／#11 仍卡在你對 S72 提案的確認，#3 卡在 Vercel 設定，沒有動。

### 做了什麼

新增 `/glossary` 頁與 33 條詞條，分六組（三卦與動爻／納甲盤構成／六親／用神與斷語／
動爻變化／旺衰）。每條分兩段：**這個詞在命理裡是什麼**（`plain`）與**本 App 在盤面上
怎麼取、怎麼用**（`inApp`）——第二段才是這份詞典比一般術語表多出來的東西，也是
最容易過期的東西（見下方守門）。

入口兩個：設定頁工具區、揭曉頁盤面正下方（`reveal-glossary-link`）。

### 幾個決定，以及為什麼

- **入口放在 `reveal.tsx`，不放在 `LiuYaoPanel` 裡。** `LiuYaoPanel` 同時被離屏的
  報告截圖（`ReportCardView`）使用，在裡面加可按的連結會連累匯出的長圖。測試釘了
  這一條（`LiuYaoPanel.tsx` 不得出現 glossary）。
- **單欄限寬（`Layout.maxContent`），不學圖鑑的多欄網格。** 圖鑑卡是四行短詩，
  詞條是兩段散文且高度落差大，並排只會把行寬切碎、留下大片空洞。
- **三語放在同一筆條目裡**，不像籤詩那樣另開翻譯檔：一條詞條就是「一個詞、兩段話」，
  增刪時三語同進同出，缺一個語言在型別上就過不了。術語本身一律保留漢字
  （沿用 S38 決定），en 另附一行英文對照。
- **詞典裡不寫加減幾分。** 那些常數在 `wenwang.ts`，抄一份就是第二個真相來源
  （S60 的教訓）。要看分數，盤面的斷語本來就逐條列著。
- **靈棋頁沒有入口**：靈棋不排納甲盤，詞典裡的術語與它無關，放了只會讓人以為
  靈棋也有世應與用神。

### 守門：讓「在本 App」那一段不能默默過期

`glossary.test.ts`（33 條）不是檢查「有沒有寫」，而是把詞典裡**能枚舉的事實**
一條條對回真相來源：

| 詞典寫的 | 對回 |
|---|---|
| 六親各自在哪些問事類別擔任用神／喜神／忌神 | `useGodForCategory`，**正反兩向都比**（該提的要提、不相干的不能提；感情男女各走一遍） |
| 進神的行進方向（寅→卯…丑→辰→未→戌→丑） | `advanceOrRetreat` 判為進神的全部 8 組 |
| 相沖的六對 | `branchesClash` 為真的全部組合 |
| 當令：哪幾個月建屬哪個五行 | `seasonOf`／`SEASON_ELEMENT`，十二個月建一個不能漏 |
| 旺相休囚死的範例 | `strengthState` |
| 三合局四組、動爻化變五種方向、六神六位 | `TRIADS`／`transformedLineRelation`／`SIX_SPIRITS` |

**注入迴歸驗證過**：六種竄改（妻財誤提事業、進神少一組、相沖配錯、當令寫錯五行、
六神少一位、詞條不存在於盤面）各自都讓對應測試變紅，檔案還原後全綠。

### 過程中被既有守門攔下、而且都是真的

1. **「詞典只收盤面上真的印出來的詞」這條收錄標準，第一次機器驗證就抓到 7 個
   不成立**：干支、八宮、應爻、六神、飛神、三合局、旺相休囚死——盤面上印的是
   「乾宮屬金」「遊魂」「'應'」「局生用神」，不是這些字面。處理：干支併入納甲
   （它本來就是在解釋「壬午」）；其餘保留詞條名，但要在條目上明白登記
   `printedAs`（盤面實際印的字），測試拿它去原始碼字串裡找。另加一條反向守門：
   **term 本來就找得到的條目不准登記 `printedAs`**，否則多餘的登記日後過期也
   不會有人發現。掃描前先剝掉註解——註解裡討論一個詞不等於使用者看得到它。
2. `i18n.test.ts` 攔下英文搜尋提示裡的漢字（`e.g. 用神, 月破`）→ 改成英文詞。
3. `fontSubset.test.ts` 攔下 8 個字（属、区、暦、扱、〜、輛、庇，以及只在註解裡的
   殊），幾乎全是我寫日文時帶進來的新字體。依 S62 判準**改用詞、不重建 1.2MB
   子集**（属→分かれ、区切り→変わり目、扱わない→取り上げない…）。
4. **本機 tsc 報「`/glossary` 不在路由型別裡」，但那不是程式的錯**：
   `.expo/types/router.d.ts` 是被 gitignore 的產物，`expo export` 不會重生成它，
   只有 `expo start` 會。CI 是乾淨 checkout、沒有這個檔案，所以不會遇到。
   本機遇到時短暫起一下 `npx expo start` 讓它重生成，不要為了讓 tsc 過而去改程式。

### 我沒有驗證的（請你知道）

- **詞條的命理定義是通行說法，由我撰寫，沒有逐條對過典籍**（例如六神各主什麼、
  旬空「旺空動空」的例外）。`inApp` 那段則是讀程式得來、且由上面的守門釘住，
  兩段的可信度不同——這也是它們分成兩段的原因。
- **en／ja 也是我寫的，沒有母語者校閱**（路線圖 #6 仍是外部人力待辦）。
  定義文字不需等 #6 才能上線，但別當成可對外宣傳的品質。
- 詞典明說了兩處「本 App 取法比傳統簡單」：空亡不分旺空／動空，六神只標示不計分。
  這是讀 `wenwang.ts` 得來的事實，不是評價。

### 驗證

真瀏覽器 e2e 新增 8 條（4 情境 × mobile／desktop）：設定頁入口與六個分組、搜尋縮小／
空狀態／清除、揭曉頁入口往返、切英文後仍是漢字＋英文對照且搜英文與搜漢字都命中。
另用截圖看過手機、桌面、英文三種版面與揭曉頁連結位置，都正常。

Jest 1201 → 1234（60 → 61 套件）、E2E 234 → 242、`npx tsc --noEmit` 零錯誤。
`overviewTable.test.ts` 如預期紅了一次（+3 個原始碼檔、+1 頁、+1 服務、+1 套件），
已同步。實機清單 26 → 27 項（#27，長文換行與搜尋鍵盤只有原生走得到）。

**沒做**：長按術語就近解釋（要動 `LiuYaoPanel` 互動邏輯，等看使用者有沒有在用
這一頁再說）；#12 提醒策略——它「什麼都不缺」，但要不要開放、開放到什麼程度
是你的決定。我的建議（未動手）：設定頁加一個「占驗提醒」選項——關／7 天／14 天／
30 天，預設仍是 14；固定選項而非自訂天數，因為自訂要處理 0 天與負數，而使用者
真正要的只是「關掉」與「早一點／晚一點」。改天數只影響**之後**排的提醒，
已排程的不動（`cancelAllVerificationReminders` 已有前綴掃描，要重排也做得到）。

---

## Session 72 — 盤點深度功能候選：四個新方向，程式碼未動（9/19）

使用者問「還可以加入哪些深度功能」。路線圖八項候選裡 1/2/4/5/8 已結案，
3/6/7 分別卡在你的 Vercel 設定、母語者、後端法務——目前「我能自己往前推」
的候選是空的，需要重新盤點。

四輪討論收斂出四個新方向，寫進「未來待辦 › 功能候選」表的 #9–12：

- **#9 命理術語詞典**——納甲盤上的「世應」「用神」「六親」對新手是天書；
  決定先做獨立索引頁而非長按 tooltip（後者要動 `LiuYaoPanel` 互動邏輯，
  風險較高）。設計分歧最小，幾乎可以直接排入。
- **#10 應驗率隨時間趨勢**——使用者原話「就是看看到底有沒有應驗」，追問後
  確認要的是**隨時間變化**，不是靜態分項（現有六種 `accuracyBy*` 都是固定
  維度，答不了「我用越久，準確率是變好還是變差」）。提案四項設計：X 軸用
  累積第 N 筆已回填記錄而非日曆時間（上線才一個多月，按日曆分桶會太稀疏，
  且更貼近「驗證次數增加後有沒有變化」這個問題本身）；滑動視窗大小沿用既有
  `MIN_INSIGHT_SAMPLES`（= 5），不新造一個數字（S60 吃過「同一件事十二份
  拷貝、零個真相來源」的虧）；已回填 < 10 筆（兩個視窗量）時整條線不顯示，
  改顯示「還差 N 筆解鎖」；第一版只做整體、不分維度（分下去樣本只會更稀）。
  四項都待使用者確認，尚未拍板。現有 `TrendChart` 是「每日占卜次數的吉凶
  分佈」長條圖，資料形狀跟這個（單一累積百分比折線）不同，需另做元件。
- **#11 同問題複占比對**——六爻「一事不二占」是真實命理禁忌，App 目前對
  重複問同一件事完全沒提示。討論出「同一件事」怎麼認的三個選項（精確比對
  文字／同類別＋時間相近／使用者手動確認），傾向手動確認——理由與尋人陣
  當初因用神取法不明確而收斂掉同源：規則式判斷的把握不夠就別猜。
  判定規則與比對後的呈現方式（只給連結 vs 並列六爻差異）都待拍板。
- **#12 占驗提醒策略可調**——把散落在文字段落裡「還懸著的小決策」正式收進
  候選表；現在固定 14 天、不能關、不能改天數，要不要開放需要你的決定。

這次純討論、沒有程式碼改動，`overviewTable.test.ts` 涵蓋的數字（原始碼
檔案／Jest／E2E／tsc）不受影響，維持 Session 71 的 1201／234／零錯誤。
...
應期、不做親屬關係，理由是「規則前提不足」；靈棋深度解讀也定案「不造原典
沒有的吉凶等級」。任何真的讓斷語「看起來更準」的新技術（天文曆算推應期、
AI 生成更細的預測）都會踩線——看起來更準，其實是更會編故事。

真正不違背這條線又有價值的方向，是把 S63/S64 已經做好的**個人應驗率資料**
從統計頁搬到解讀現場：使用者在讀「深度解讀」之前，先看到自己過去在同一
問事類別驗得準不準，這件事一句判斷都沒有新生成，只是把既有事實換個時機
說出來。

### 改動範圍

新增 `components/AccuracyHint.tsx`：自帶 `getHistory()`，用既有的
`accuracyByCategory` + `questionCategoryDomain`（子領域先映回主類別）+
`categoryLabel`（i18n）算出這次占卜的類別在使用者手上驗得如何，門檻沿用
`MIN_INSIGHT_SAMPLES`／`enoughSamples`——**沒有新造任何統計規則**，純粹是
verification.ts 既有結果的第二個出口。

接進兩個揭曉頁的「規則式深度解讀」之前：`reveal.tsx`（抽棋／棋盤共用）與
`lingqi.tsx`（靈棋自成一頁）。兩個入口都接，理由是 WORKLOG 反覆記過的
「同一功能的第二個入口最容易被漏掉」——這次直接一次做兩邊。

**樣本不足時整個元件不渲染，不顯示「資料不足」**：這裡與統計頁的
`stats.lowSample` 刻意不同。統計頁本來就在列一整排分組，淡化標示是合理的
上下文；但這裡是單一類別的即時提示，每次占卜都跳出「資料還不夠」只會變成
雜訊，且容易被讀成「你在這方面比較差」——S64 已經記過這個教訓一次。

### 測試

`accuracyHint.test.tsx`（4 條，mock `getHistory`）：樣本不足不渲染、樣本
足夠時文案帶對筆數與應驗率、子領域正確映回主類別、無分類時同樣受門檻管住。

`accuracyHint.spec.ts`（6 條 e2e，reveal 正反案例 + lingqi 入口，各走
mobile／desktop）：真瀏覽器種 localStorage 記錄，驗證接線本身（而非只驗
元件邏輯）。寫的時候踩到一個新的 locator 坑：拿「載入完成」當判準的元素
用了 `getByText('龍騰九霄')`，在桌面寬度下因為 `SplitReading` 分欄把標題
重複印在標題列與側欄摘要兩處而撞上 strict mode；再換成 `reading-split`
testID 又在 mobile 寬度下找不到——`SplitReading` 依斷點只出其中一個
testID（`reading-split` 或 `reading-single`），兩個寬度各自只認得一個。
改用 `.or()` 兩個 testID 擇一才在兩種寬度下都穩。**跨寬度的 e2e 判準要挑
兩種佈局都存在的元素，不能只在桌面寬度跑過一次就當作穩了。**

依專案慣例改完元件先 `npm run build:web` 再跑 e2e；`overviewTable.test.ts`
守門測試如預期紅（新增 1 個元件檔＋1 個 Jest 套件），同步更新 WORKLOG
開頭總覽表三個數字。

Jest 1197 → 1201（59 → 60 套件）、E2E 228 → 234，`npx tsc --noEmit` 零錯誤。
...
| # | 待辦 | 你要做的 | 之後我能接手 | 成本 |
|---|------|---------|------------|------|
| 3 | **iOS/Android 實機測試** | `npx expo start --go`，手機掃碼；依 `NATIVE_TESTING.md` 驗 33 項 | 依你回報的現象修 | 免費 |
...
**不需要帳號的那幾項裡投報率最高的是 #3（實機測試）**——不花錢，而且能驗的
東西最多。但**現在最短路徑是功能候選的 #3（雲端同步）**：它是唯一「程式全做
完、只差你在 Vercel 點幾下」的項目，而且今晚複驗發現它**退回去了**（見上方
Session 68 的收尾複驗）——線上端點從下午的「環境變數有設、呼叫 Upstash 失敗」
變成「環境變數根本不在這個部署上」，所以你要做的不是除錯，是**回頭確認
Storage 分頁裡 Upstash 還連著 `chess-divination-app`**，連著就 **Redeploy**
一次（環境變數不會套用到既有部署，S48／S67 都踩過同一件事）。

#### 雲端同步的複驗指令（照抄即可，別再用「隨便一個假配對碼」）

S67 寫的驗證方式漏了「配對碼放哪裡」，S68 照著做時被格式檢查擋在半路、
拿到看似成功的 401。以下這條是**已經實際打過**的版本（2026-09-14 深夜）：

```bash
curl -s -w '\nHTTP %{http_code}\n' \
  -H 'x-sync-key: 0123456789abcdef0123456789abcdef0123456789abcdef' \
  https://chess-divination-app.vercel.app/api/sync
```

配對碼必須是 **48 位十六進位**、放在 **`x-sync-key` 標頭**（`syncKey()`
只讀標頭，查詢字串讀不到），格式不對時連 Upstash 都不會碰。狀態碼對照：

| 狀態碼 | 回應 | 意思 |
|---|---|---|
| 401 | `INVALID_SYNC_KEY` | 配對碼格式不對——**什麼都沒驗到**，不是成功 |
| 501 | `SYNC_NOT_CONFIGURED` | 環境變數對這個部署不可見（資料庫沒接上／沒套用） |
| 502 | `SYNC_UPSTREAM_ERROR` | 環境變數有值，但呼叫 Upstash 失敗（URL／Token／區域接錯） |
| 404 | `NOT_FOUND` | **接上了**，這組配對碼查無資料 ← 「有接上」的正常回應 |
| 200 | 同步資料 | 接上了，而且這組配對碼有資料 |
| 429 | `RATE_LIMITED` | 打太快（限流 20 次／分鐘） |

**S67 說「應該從 501 變 401」是錯的**——401 正是「沒驗到」的那一格。接好之後
要看的是 **404**。

**#3 的重點在 A25**：`isPlausibleCapture` 的 4KB 門檻是估的，從沒有實機樣本驗證過。
要看的是兩件事——分享圖打開後有沒有內容、以及正常卡片會不會被誤判成空白而降級成文字分享。
完整清單見 `NATIVE_TESTING.md` 第一階段（33 項，已標明 Expo Go 驗得到與驗不到）。
...
**維護待辦目前是零**——#39（`/settings` 的 e2e strict mode 衝突）已於
Session 69 同日修掉：頁面標題加 `testID`，該路由改用 `getByTestId`，
兩輪完整套件（228 條）跑下來都全綠。**不把這句話寫死**：它已經被推翻過
四次（S57／S58／S59／S62 各一次，理由見 `DEVELOPMENT_PLAN.md` 的引言），
之前每次也都是「當下核對過、寫下來之後才被打臉」，故下一輪接手時仍要
重新核對，不要直接沿用這句話。
下列「功能候選」是下一輪產品路線圖，須先選定優先級才進入實作，
不可與缺陷待修混列。

> **它推翻了什麼**：`filter({ visible: true })` 是 S56/S57 為了「疊棧背景頁的
> 同名文字」定下來的統一寫法，掃過去會以為所有同名標題都處理完了。這條是
> 同一家族的新面貌：**不是「不可見卻被匹配到」，是「可見的也不只一個」**，
> 而那把 filter 對後者無效。

#### 功能候選（尚未排程）

> **Session 62（續）重排**。原表只有「優先」與「開始前需決定」，
> 排不出真正的順序——因為七項**卡在的東西種類不同**：有的什麼都不缺，
> 有的只差你點幾下，有的要先開會定規則，有的要找人。
> 加一欄「卡在什麼」之後順序幾乎是自己排出來的。

| 順 | 候選功能 | 卡在什麼 | 使用者價值 | 開始前需決定 |
|---|---|---|---|---|
| ~~1~~ | ~~**占驗回訪流程（首頁提示）**~~ ✅ S63 | ~~**什麼都不缺**~~ — `pendingVerification()` 早就算得出來，只是沒有出口在首頁 | 兩週後從通知／首頁直接回填結果，提高應驗率資料完整度 | 提醒頻率、是否允許關閉（**首頁提示本身不需要這個決定**） |
| ~~2~~ | ~~**個人化統計洞察**~~ ✅ S64 | ~~**什麼都不缺**~~ — 六種分項應驗率都已在畫面上，只差樣本門檻的說法 | 依問事類別／時間／模式提示「哪些情境較準」，並擋掉樣本不足的過度解讀 | 最小樣本數與文案 |
| 3 | **跨裝置雲端同步** | **同一天內狀態退了一次** — 下午實測是「env 有設、但呼叫 Upstash 失敗」（501 `SYNC_UNAVAILABLE`），深夜複驗變成「env 對這個部署根本不可見」（501 `SYNC_NOT_CONFIGURED`）。程式全做完，缺的一直是 Vercel 那端的連線設定，見 Session 68 與其收尾複驗 | 換機／雙機不再手動備份還原 | 回 Vercel **Storage** 分頁確認 Upstash 還連著 `chess-divination-app`；連著就 **Redeploy** 一次（環境變數不套用到既有部署），再照下方指令複驗 |
| ~~4~~ | ~~**牌陣擴充包**~~ ✅ S65 | ~~先拍板規則~~ — 重新盤點 `POEM_FACETS` 才發現感情/抉擇/職涯早有牌陣，真缺口是財運/健康/學業/出行 | 四個問事領域各有專用位置與解讀脈絡 | 已拍板：四陣一次做完、佈局依領域客製 |
| ~~5~~ | ~~**記錄匯出閱讀報告**~~ ✅ S66 | ~~先拍板版型~~ — 選長圖沿用既有離屏截圖機制，未加 PDF 相依 | 單筆或批次記錄的六爻／解讀／回填結果存成長圖 | 已拍板：長圖格式、問題與筆記預設納入可關閉、單筆＋批次兩種入口 |
| 6 | **多語內容校閱** | **需要外部人力**（母語者） | 將英／日文的命理術語與行動建議提升到可對外宣傳品質 | 母語校閱者與術語表 |
| 7 | **匿名產品回饋** | **需要後端與法務** | 讓使用者回報解讀品質或問題 | 蒐集範圍、隱私政策更新、後端服務 |
| ~~8~~ | ~~**問事類別擴充包：官司／尋物**~~ ✅ S69 | ~~**什麼都不缺**~~ — 用神取法沿用既有官鬼／妻財，不必碰判定引擎 | 官司訴訟與遺失物兩類問事各有專用牌陣與斷語 | 已拍板：兩類都做、中文優先、逐卦撰寫（尋人因用神取法不明確而收斂掉） |
| ~~9~~ | ~~**命理術語詞典**（世應／用神／六親等）~~ ✅ S73 | ~~**什麼都不缺**~~ — 純新增獨立索引頁，不動 `LiuYaoPanel` 既有互動邏輯，量約 20–30 條，定義文字不需等 #6 母語校閱 | 新手看得懂納甲盤上的漢字術語，不必自己查 | 已初步定案：先做獨立索引頁，長按 tooltip 版留待之後視需要再加 |
| ~~10~~ | ~~**應驗率隨時間趨勢圖**~~ ✅ S75 | ~~**已提案四項設計**~~（X 軸用累積第 N 筆已回填記錄／滑動視窗沿用 `MIN_INSIGHT_SAMPLES`／< 10 筆不顯示／第一版只做整體不分維度），待你確認 | 回答「我用越久，準確率是變好還是變差」——現有六種 `accuracyBy*` 都是固定維度分組，答不了這個問題 | 你對 Session 72 提案四項設計的意見；`TrendChart` 資料形狀不同，需另做元件 |
| ~~11~~ | ~~**同問題複占比對**~~ ✅ S76 | ~~**需要先拍板判定規則**~~ — 「同一件事」怎麼認：精確文字比對／同類別＋時間相近／使用者手動確認（傾向後者，見 Session 72） | 六爻「一事不二占」是真實命理禁忌，App 目前對重複問同一件事完全沒提示，也能對照兩次卦象異同 | 判定規則、比對後呈現方式（只給連結 vs 並列六爻差異） |
| ~~12~~ | ~~**占驗提醒策略可調**（頻率／可否關閉）~~ ✅ S74 | ~~**什麼都不缺**~~ — 已做成關／7／14／30 天，預設 14 | 現在固定 14 天、不能關、不能改天數；部分使用者可能想關掉或改頻率 | 要不要開放、開放到什麼程度（固定選項 vs 自訂天數） |
| ~~13~~ | ~~**預測校準（直覺 vs 卦）**~~ ✅ S77 | ~~**什麼都不缺**~~ — S77 盤點「跨領域可拓展」時提出，借決策科學的 calibration；資料只差「占卜前的直覺」與「事情本身的結果」兩欄 | 回答「卦比我的直覺準嗎」「我說七成的事真的七成成了嗎」——占驗簿只量得到卦 | **已拍板定案（9/25）**：卦只比方向不換算機率、只並列不下結論、不足 5 筆只說還差幾筆，見 Session 77 |
| ~~14~~ | ~~**易經學習模式**~~ ✅ S78 | ~~**什麼都不缺**~~ — S77 盤點時提出；題目全部取自既有卦資料 | 盤面上的八卦、六十四卦與五行，從「查得到」變成「記得住」 | 已定：三副牌（八卦／八卦五行／六十四卦）、Leitner 五格、進度只存本機並納入備份 |
| ~~15~~ | ~~**節氣與農曆**~~ ✅ S79 | ~~**什麼都不缺**~~ — 節氣近似式與月建早已在 `date.ts`；農曆用平台曆法 | 首頁看得到今天的農曆、節氣、月建與當令五行，而且與卦盤用的是同一份答案 | 已定：只列曆法事實，不做擇日宜忌 |
| ~~16~~ | ~~**卦典**（圖鑑第三分頁）~~ ✅ S80 | ~~**什麼都不缺**~~ — S80 盤點「寫好卻讀不到的內容」時提出：384 條爻辭已校對、`TRIGRAM_OPPOSITE` 從沒人用 | 任一卦的六條爻辭與互／錯／綜都翻得到；揭曉頁一鍵帶著本卦過去 | 已定：錯綜只進卦典、不上揭曉頁、不進斷語 |
| ~~17~~ | ~~**卦辭（＋大象傳）**~~ ✅ S81 | ~~**內容工作**~~ — 由腳本自維基文庫產生，三道查核（爻辭對上既有 384 條、兩頁大象互證、對《周易正義》） | 揭曉頁盤面印本卦卦辭與大象；卦典每卦列卦辭、大象、爻辭 | 已拍板：連大象傳一起做；三語皆印原文。學習模式的新牌組未做 |
| ~~18~~ | ~~**成就補齊周邊**~~ ✅ S82 | ~~**什麼都不缺**~~ — S80 走周邊系統時發現 | 12 → 17 個成就：學習、校準、複占連結各有認得它們的成就 | 已拍板（使用者照提案）：初窺易理／八卦在心（間隔 4 天）／先問本心／知己知卦（10 筆）／一事再問 |

**這一項不在 Session 62 原本列出的七項裡**：路線圖表不是靜態清單，`POEM_FACETS`
與 `useGod.ts` 的既有取法本身就是候選來源，值得每隔幾輪重新盤點一次
（上一次盤點正是這樣才挖出財運/健康/學業/出行四陣，見 #4）。**#9–12 是
Session 72 同一種盤點的延續，這次沒有既有取法可掃，改用「還缺什麼深度」
直接跟使用者對談收斂出來。** **#16–18 是 S80 的盤點**：既有取法已掃乾，
這次靠的是「沒有呼叫端的匯出」「新功能有沒有走過周邊系統」，以及新加的一種找法——
**寫好、校對過，卻沒有地方讀得到的內容**（384 條爻辭只看得到一條）。

**目前「我能自己往前推」的候選是空的**：1／2／4／5／8／9–18 已結案，
3 卡在你的 Vercel 設定、6／7 需要母語者與後端／法務。

> **Session 62 那條教訓留著**：列在路線圖上的東西不一定整條都卡在決策上。
> 「占驗回訪流程」看起來要等產品拍板，實際上只差一個出口，S63 當天就做完了。
> **先問「這條裡有沒有不需要決策的那一段」，再決定要不要為它排會。**

#### 已結案

- **#24–27**（S54 提出、S55 做完）：分享去處選單（LINE／Facebook／複製，
  取消真的是取消）、吉凶分佈的百分比（順帶修掉分母該是「有等級的記錄」
  而非全部記錄）、`HexagramLines.showLabels` 接上、
  `SplitReading.singleMaxWidth` 刪除、`問事類別記憶` 三條 flaky 修掉。
  **其中只有 #24 真的需要產品決策**——另外三項我列成待決是列錯了，
  理由（「這是別人寫的測試」「與既有決策衝突」）查一下就站不住。
- **#29**（S56）：`ChessBoard.tsx` 的 `isAvailable`／`hasPiece` 兩個死區域
  變數刪除；同時補上「沒有算出來卻沒人用的區域變數」守門，與孤兒匯出、
  孤兒 setter 湊成同一家族的三支掃描。
- **#30**（S56）：WORKLOG 開頭的專案總覽表連續四個 session 沒人更新，
  新增 `overviewTable.test.ts` 對著檔案系統核對數得出來的八列。
- **#31**（S57）：首頁每日運勢的分享是分享鏈的第三個入口，S55 加去處選單、
  S56 改三態時都只修了揭曉頁與靈棋頁——桌面瀏覽器按下分享 = 靜靜複製、
  一句話都不說。接上與另外兩頁相同的那一套，並補「所有分享入口窮舉斷言」
  的來源掃描守門。
- **#32**（S57）：收藏頁三個分頁的空狀態把「搜尋沒命中」說成「你還沒有任何
  記錄」；資料夾詳細頁尤其矛盾（標題印著筆數、下面說還沒有記錄）。
  統一走 `renderEmpty()` 依搜尋字串分岔，新增 `collection.noMatch` 三語。
- **#33**（S58）：首頁「快速抽一籤」的副標寫死「直接抽取 2 顆棋子」——
  按鈕其實只是進到抽棋頁，而顆數是使用者設定的（設 3 的人，抽棋頁把 3
  標成「建議」，首頁卻說 2）。改為不帶數字的描述；另修掉
  `scheduleVerificationReminder()` 寫死 14 天而不用 `VERIFY_REMINDER_DAYS`。
  新增 `copyNumbers.test.ts` 把文案數字對回真相來源（三語各自都要掃得到）。
- **#34**（S59）：首頁「最近占卜」替靈棋記錄畫了空的棋子欄（寫死 `width: 60`）
  與空的等級欄——S44／S45／S54 同一族的第四個位置，也是最後一個沒掃過的
  畫面。新增 `emptyFieldRendering.test.ts` 掃所有渲染這兩個欄位的地方；
  另補原生清單 #25（首頁分享入口的取消行為，只有原生走得到），21 → 26 項。
- **#35**（S60）：四個成就門檻各被寫了三遍（中文說明、en、ja），門檻本身
  卻只是條件式裡的字面量——十二份拷貝、零個真相來源。抽出
  `ACHIEVEMENT_THRESHOLDS` 並讓條件式真的用它；守門逐語言比對說明裡的
  數字，另加一條「說明帶數字的成就都必須登記」讓它跟著專案長大。
- **#36**（S62）：占驗提醒的 `recordId` 一路寫進通知 payload，卻在點擊時被
  丟掉——使用者被丟在通用統計頁，要回填哪一筆得自己找。改為打開那一筆記錄
  本身（通知只給 id，路由仍由 `recordLink()` 依記錄的 mode 決定，白名單
  沒被繞過），記錄已刪則退回保底畫面。另修掉 `NATIVE_TESTING.md` #8
  那句已經不成立的「應進入 `/stats`」。
- **#37**（S63）：`pendingVerification()` 算得出「哪些該回填」，但唯一用到它
  的統計頁只把它印成一個數字——使用者被告知有 N 筆，然後自己去歷史裡找。
  首頁新增可按的待回填提示（指名最近滿期的那一筆），統計頁那行也改成可按。
  **「有用到」不等於「有出口」：只被拿去 `<Text>` 的計算結果，
  和沒人呼叫的函式，對使用者來說是同一件事。**
- **#38**（S64）：`bestCategory` 的樣本門檻（5 筆）只管住它自己——分項應驗率
  列表只依應驗率排序，於是一筆 100% 的分組坐在五筆 80% 的上面。門檻抽成
  `MIN_INSIGHT_SAMPLES`、`AccuracyBreakdown` 帶 `enoughSamples`、排序認門檻，
  畫面上標「樣本不足」並附說明。**排序本身就是一種宣稱。**

Session 53 修完資料夾的兩半（點不開、只進不出），Session 54 修完它露出來的
三件事（筆記搜不到、靈棋的空棋子格、刪除記錄不清資料夾），見 #28。

Session 51 依同一份清單走了 S50 的子領域，找到並修完四處只認七個主類別的
地方（籤詩詳解分頁、感情性別提示、統計分組、棋盤頁的類別記憶），見 #23。
...
> 「哪些功能應該跟著改」則會漏。
>
> Session 57 再確認一次，又找到兩處，而這次的成因是前一天那把掃描器
> **問錯了問題**：孤兒掃描問的是「寫好的東西有沒有人用」，這兩處的程式
> 全都接上了、也都有人呼叫，錯的是接上之後**對使用者說的那一句**
> （按了沒反應、把搜尋沒命中說成你沒有記錄）。**掃描器找得到沒接上的線，
> 找不到接上之後說錯的話**——後者只能把自己當成正在用它的人，按下每一個
> 按鈕、讀出畫面上的每一句話，問「這句話現在是真的嗎」。
...
- **#28** 資料夾兩輪修完：S53 補上詳細頁（`setSelectedFolderId` 從沒被
  呼叫過）與移出（`removeFromFolder` 沒有呼叫端）；S54 補上刪除記錄時
  一併從資料夾拿掉，並讓筆數改數「還指得到記錄的」以治既有資料
  （Session 53–54）
...
| Session 50 | 快速提問範本改成三層問題庫（面向 → 情境 → 範本，10 個子領域），抽棋／棋盤／靈棋三頁共用（測試 1064、E2E 178） | 9/2 |
...
| Session 52 | 程式碼收尾：8 個沒有產品呼叫端的工具匯出移除（`deadExports.test.ts` 守門）＋工作日誌重整＋production 部署核對（測試 1088、E2E 188） | 9/3 |
| Session 53 | 資料夾兩半：詳細頁（`setSelectedFolderId` 從未被呼叫）與移出（`removeFromFolder` 沒有呼叫端）（測試 1090、E2E 190） | 9/4 |
| Session 54 | 資料夾修完露出來的三件事：筆記搜不到、靈棋卡片空棋子格、刪記錄不清資料夾（測試 1097、E2E 198） | 9/4 |
| Session 55 | 清掉 S54 列出的四項：分享去處選單、吉凶分佈百分比、爻名標籤接上、三條 flaky（測試 1103、E2E 204） | 9/4 |
| Session 56 | 日誌總覽表接上真相來源（`overviewTable.test.ts`，八列）＋第三支孤兒掃描（區域變數）；分享鏈兩個「按了沒反應」（測試 1115、E2E 208） | 9/4 |
| Session 57 | 畫面對使用者說錯話的兩處：首頁分享沒反應、搜尋沒命中被說成沒有記錄（測試 1129、E2E 218） | 9/5 |
| Session 58 | 文案裡寫死的數字：首頁副標與提醒天數改接真相來源（`copyNumbers.test.ts`）（測試 1139、E2E 220） | 9/5 |
| Session 59 | 首頁空欄位的第四個位置（靈棋的棋子欄與等級欄）；實機清單 21 → 26 項（測試 1149、E2E 222） | 9/5 |
| Session 60 | 成就門檻抽成 `ACHIEVEMENT_THRESHOLDS`——四個數字、十二份拷貝、零個真相來源（測試 1155、E2E 222） | 9/5 |
| Session 61 | 交接收尾：可部署性核對、E2E 環境（缺 `chromium_headless_shell`）修復與下一步（測試 1155、E2E 222） | 9/5 |
| Session 62 | 占驗提醒知道是哪一筆、點下去卻丟在統計頁；並依「卡在什麼」重排路線圖（測試 1166、E2E 222） | 9/12 |
| Session 63 | 待回填算得出來卻沒有出口：首頁新增可按的提示並指名那一筆（測試 1169、E2E 226） | 9/12 |
| Session 64 | 一筆全中不該坐在第一列：分項應驗率的排序要認樣本門檻（`MIN_INSIGHT_SAMPLES`）（測試 1172、E2E 228） | 9/12 |
| Session 65 | 牌陣擴充包：財運／健康／學業／出行四陣（測試 1172、E2E 228） | 9/13 |
| Session 66 | 記錄匯出閱讀報告（長圖版，隱私選項＋批次匯出）（測試 1185、E2E 228） | 9/13 |
| Session 67 | 收尾交接：路線圖七項現況與雲端同步的接線步驟（測試 1185、E2E 228） | 9/13 |
...
| Session 72 | 盤點深度功能候選：術語詞典／應驗率時間趨勢／複占比對／占驗提醒策略可調（#9–12），純討論無程式碼改動（測試 1201、E2E 234） | 9/19 |
| Session 73 | 命理術語詞典（路線圖 #9）：33 條、雙入口、「在本 App」段落對回 useGod 等真相來源；實機清單 26 → 27 項（測試 1234、E2E 242） | 9/19 |
| Session 74 | 占驗提醒可調（路線圖 #12）：關／7／14／30 天，一個設定值管通知、首頁提示、統計頁；首頁改為每次 focus 重讀；實機清單 27 → 28 項（測試 1263、E2E 256） | 9/19 |
| Session 75 | 應驗率隨時間趨勢圖（路線圖 #10）：滑動視窗折線、不足 10 筆不畫；e2e 揭露圖寬量測回饋迴圈並補幾何穩定度守門；實機清單 28 → 29 項（測試 1273、E2E 266） | 9/19 |
| Session 76 | 同問題複占比對（路線圖 #11）：手動連結＋完全相同才建議＋並列卦象不下結論；揭曉頁與靈棋頁雙入口；刪除時清連結；實機清單 29 → 30 項（測試 1308、E2E 282） | 9/19 |
| Session 77 | 補完 S76（連結的同步墓碑、收藏頁標記、報告長圖）＋預測校準（路線圖 #13）：占卜前記直覺、回填記事情結果、統計頁 Brier 與卦／直覺同批並列；順帶修掉收藏頁不重讀、棋盤捲動後拖曳放錯格；實機清單 30 → 31 項（測試 1343、E2E 300） | 9/25 |
| Session 78 | 易經學習模式（路線圖 #14）：八卦／八卦五行／六十四卦抽認卡、Leitner 間隔重複、題目全取自既有卦資料；卦形題加讀屏說明；實機清單 31 → 32 項（測試 1362、E2E 310） | 9/25 |
| Session 79 | 節氣與農曆（路線圖 #15）：首頁今日曆法卡（農曆／節氣／月建與當令五行／日柱），節氣與月建共用 date.ts 同一條近似式、農曆用平台中國曆且不支援時隱藏；不做宜忌；實機清單 32 → 33 項（測試 1380、E2E 320） | 9/25 |
| Session 80 | 重新盤點候選 #16–18（新找法：寫好卻讀不到的內容）＋卦典（路線圖 #16）：圖鑑第三分頁列 64 卦六條爻辭與互錯綜、揭曉頁入口帶本卦過去；互卦算法收成一份；順帶補圖鑑分頁的 `aria-selected`（測試 1411、E2E 332） | 9/26 |
| Session 81 | 卦辭＋大象傳（路線圖 #17）：`build-zhouyi-texts.mjs` 自維基文庫產生，三道查核（爻辭對上既有 384 條、大象兩頁互證、對《周易正義》），異文與裁定逐條登記且例外沒用到就報錯；揭曉頁盤面印本卦卦辭與大象、卦典每卦列出；字型子集 +25 字（測試 1421、E2E 332） | 9/26 |
| Session 82 | 成就補齊周邊（路線圖 #18）：12 → 17 個，學習／校準／複占連結；學習進度另一個儲存鍵由 syncAchievements 讀入；校準條件共用 calibration.ts 的判斷；成就圖示改為 IconName 型別，拿掉會靜靜退回星星的 emoji 對照表（測試 1430、E2E 336） | 9/26 |
