...
| 15 | **`summarizeReading()` 是死碼** | 🟢 低 | 註解寫「供分享與離線解讀使用」，但全專案只有測試呼叫它（Session 38 掃樣板字串時發現）。它產出的 6 條中文樣板因此也沒翻。要嘛接上分享流程讓註解成真、要嘛連同 4 則測試一起刪；兩條路都不該順手做，需先確認當初是否真的打算用在分享文字 |
...
| 14 | ~~**六爻面板命理散文是否翻譯（Session 35 提出）**~~ ✅ | 🟢 已完成 | Session 38 結案。原估「需另立術語表」是把三堆混在一起算：384 條《周易》爻辭原典、148 條資料值字面量、57 條專案自撰散文。**只有第三堆該翻，且已翻完**（術語保留漢字、只翻連接文、英文首見加註——因為納甲盤上就印著同樣的字，譯掉會對不起來）。前兩堆維持原文，理由與邊界釘在 `divinationProse.test.ts` |
