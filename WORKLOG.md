...
| 項目 | 數值 |
|------|------|
| 原始碼檔案 | 115 個（`src` 的 .ts/.tsx，不含測試） |
| Git Commits | 167 次 ※ |
| Jest 測試 | 1380 個 ※ · 65 套件 · 全部通過 |
| E2E 測試 | 320 個 ※ · Playwright · mobile + desktop |
| TypeScript | 零錯誤 |
| 頁面 | 17 個 |
| 元件 | 30 個 |
| Hooks | 12 個 |
...
**目前候選裡「我能自己往前推」的是空的**：1／2／4／5／8／9／10／11／12／13／14／15 已結案，
剩 3（卡在你的 Vercel 設定）、6／7（需要母語者與後端／法務）。
下一個動作是處理 #3 雲端同步接線，或盤點新的候選（見下方「路線圖表不是靜態清單」）。
