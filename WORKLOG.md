...
> 尚未處理：`playClickSound` 仍然沒有呼叫端。要嘛全 App 的按鈕都接上
> 點擊聲（這是產品決策，不是補接線），要嘛連同 WAV 一起刪掉（同
> Session 36 清孤兒字型）。**留著不做決定，就是留一份沒人聽過的資產。**
...
Jest +10：成就接線守門 4、音效接線守門 4、占驗提醒標題 2。
...
完整清單見 `NATIVE_TESTING.md` 第一階段（14 項，已標明 Expo Go 驗得到與驗不到）。
其中 #13 拖曳落子與 #14 棋子池側置是 Session 39 新增的。

### 🟢 不需外部資源

**#22 待產品決策：`playClickSound` 沒有呼叫端。**六個音效裡唯一一個
從未被任何畫面用過（`playShakeSound` 已於 Session 49 接上靈棋擲卦）。
兩條路二選一：全 App 按鈕接上點擊聲，或連同 `assets/sounds/click.wav`
與 `sound.ts`／`sound.web.ts` 的實作一起刪掉（同 Session 36 清孤兒字型）。
留著不決定，就是留一份沒人聽過的資產。
...
| Session 49 | 靈棋擲完不算數的三處：連續天數與成就重算只掛在 reveal 頁（靈棋不走那一頁）、靈棋全程無音效（`playShakeSound` 從未有呼叫端）、占驗提醒印中文原題而非 `recordTitle()`；三處都補來源掃描守門，e2e 刻意不開會自我補算的成就頁（測試 1056、E2E 178） | 9/2 |
