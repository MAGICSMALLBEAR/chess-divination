...
| 項目 | 數值 |
|------|------|
| 原始碼檔案 | 105 個（`src` 的 .ts/.tsx，不含測試） |
| Git Commits | 156 次 ※ |
| Jest 測試 | 1263 個 ※ · 61 套件 · 全部通過 |
| E2E 測試 | 256 個 ※ · Playwright · mobile + desktop |
| TypeScript | 零錯誤 |
| 頁面 | 16 個 |
| 元件 | 25 個 |
| Hooks | 12 個 |
...
| # | 待辦 | 你要做的 | 之後我能接手 | 成本 |
|---|------|---------|------------|------|
| 3 | **iOS/Android 實機測試** | `npx expo start --go`，手機掃碼；依 `NATIVE_TESTING.md` 驗 28 項 | 依你回報的現象修 | 免費 |
...
完整清單見 `NATIVE_TESTING.md` 第一階段（28 項，已標明 Expo Go 驗得到與驗不到）。
...
| 10 | **應驗率隨時間趨勢圖** | **已提案四項設計**（X 軸用累積第 N 筆已回填記錄／滑動視窗沿用 `MIN_INSIGHT_SAMPLES`／< 10 筆不顯示／第一版只做整體不分維度），待你確認 | 回答「我用越久，準確率是變好還是變差」——現有六種 `accuracyBy*` 都是固定維度分組，答不了這個問題 | 你對 Session 72 提案四項設計的意見；`TrendChart` 資料形狀不同，需另做元件 |
...
**目前候選裡「我能自己往前推」的是空的**：1／2／4／5／8／9／12 已結案，3 卡在你的
Vercel 設定，6／7 需要母語者與後端／法務，10／11 則卡在你對 Session 72 提案的
確認／拍板。下一個動作是回覆 #10／#11 的開放式問題，或先處理 #3 雲端同步接線。
