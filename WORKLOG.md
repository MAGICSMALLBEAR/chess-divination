...
**維護待辦為零**：目前沒有已知程式缺陷等待修復。下列「功能候選」是下一輪
產品路線圖，須先選定優先級才進入實作；不可與缺陷待修混列。
...
| 3 | **跨裝置雲端同步** | **你已動手，但接錯了** — S67 之後你已接上 Upstash（env 有設），但線上端點實測顯示呼叫 Upstash 本身失敗；已回報你檢查 Vercel／Upstash 的連線設定，見 Session 68 | 換機／雙機不再手動備份還原 | 在 Vercel 確認 Upstash 資料庫真的連到 `chess-divination-app`、URL／Token 對應正確且環境涵蓋 Production |
...
**建議下一個程式迭代選 P0「占驗回訪流程」**：現有占驗簿、待回填判定與提醒
基礎都已存在，能把既有功能串成完整閉環；但仍須先由產品決定提醒策略。

> **Session 62 更新**：這條的**第一段已經做完了，而且它根本不需要產品決策**
> ——那不是新功能，是既有功能斷在半路（提醒知道是哪一筆，點下去卻只到通用
> 統計頁）。剩下真正需要你拍板的只有**提醒策略**：頻率（目前固定 14 天一次）、
> 要不要讓使用者關掉或改天數、以及要不要在首頁做「待回填」的提示區。
> **列在路線圖上的東西不一定整條都卡在決策上，先把不需要決策的那一段做掉。**
