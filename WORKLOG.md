...
**我做的取法決定（使用者沒有逐項拍板，照「不猜、不編造換算」做，想改的位置列在最後）：**
...
| ~~13~~ | ~~**預測校準（直覺 vs 卦）**~~ ✅ S77 | ~~**什麼都不缺**~~ — S77 盤點「跨領域可拓展」時提出，借決策科學的 calibration；資料只差「占卜前的直覺」與「事情本身的結果」兩欄 | 回答「卦比我的直覺準嗎」「我說七成的事真的七成成了嗎」——占驗簿只量得到卦 | 取法我依「不猜、不換算」自訂（卦只比方向、分母同一批），見 Session 77「想改取法時動哪裡」 |
