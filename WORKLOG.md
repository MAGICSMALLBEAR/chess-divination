...
| # | 待辦 | 你要做的 | 之後我能接手 | 成本 |
|---|------|---------|------------|------|
| 3 | **iOS/Android 實機測試** | `npx expo start --go`，手機掃碼；依 `NATIVE_TESTING.md` 驗 26 項 | 依你回報的現象修 | 免費 |
...
完整清單見 `NATIVE_TESTING.md` 第一階段（26 項，已標明 Expo Go 驗得到與驗不到）。
...
| 9 | **命理術語詞典**（世應／用神／六親等） | **什麼都不缺** — 純新增獨立索引頁，不動 `LiuYaoPanel` 既有互動邏輯，量約 20–30 條，定義文字不需等 #6 母語校閱 | 新手看得懂納甲盤上的漢字術語，不必自己查 | 已初步定案：先做獨立索引頁，長按 tooltip 版留待之後視需要再加 |
...
**目前候選裡「我能自己往前推」的只剩 #9（術語詞典）與 #12（提醒策略，
若你決定要開放）**：1／2／4／5／8 已結案，3 卡在你的 Vercel 設定，6／7
需要母語者與後端／法務，10／11 則卡在你對 Session 72 提案的確認／拍板。
下一個動作是回覆 #9–12 的開放式問題，或先處理 #3 雲端同步接線。
