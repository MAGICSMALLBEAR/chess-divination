...
| 項目 | 數值 |
|------|------|
| 原始碼檔案 | 108 個（`src` 的 .ts/.tsx，不含測試） |
| Git Commits | 156 次 ※ |
| Jest 測試 | 1308 個 ※ · 62 套件 · 全部通過 |
| E2E 測試 | 282 個 ※ · Playwright · mobile + desktop |
| TypeScript | 零錯誤 |
| 頁面 | 16 個 |
| 元件 | 27 個 |
| Hooks | 12 個 |
...
| # | 待辦 | 你要做的 | 之後我能接手 | 成本 |
|---|------|---------|------------|------|
| 3 | **iOS/Android 實機測試** | `npx expo start --go`，手機掃碼；依 `NATIVE_TESTING.md` 驗 30 項 | 依你回報的現象修 | 免費 |
...
完整清單見 `NATIVE_TESTING.md` 第一階段（30 項，已標明 Expo Go 驗得到與驗不到）。
...
**目前候選裡「我能自己往前推」的是空的**：1／2／4／5／8／9／10／11／12 已結案，
