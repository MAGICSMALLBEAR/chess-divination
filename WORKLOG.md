...
| Git Commits | 156 次 ※ |
| Jest 測試 | 1234 個 ※ · 61 套件 · 全部通過 |
| E2E 測試 | 242 個 ※ · Playwright · mobile + desktop |
| TypeScript | 零錯誤 |
| 頁面 | 16 個 |
| 元件 | 25 個 |
| Hooks | 12 個 |
...
| # | 待辦 | 你要做的 | 之後我能接手 | 成本 |
|---|------|---------|------------|------|
| 3 | **iOS/Android 實機測試** | `npx expo start --go`，手機掃碼；依 `NATIVE_TESTING.md` 驗 27 項 | 依你回報的現象修 | 免費 |
...
完整清單見 `NATIVE_TESTING.md` 第一階段（27 項，已標明 Expo Go 驗得到與驗不到）。
...
| 12 | **占驗提醒策略可調**（頻率／可否關閉） | **什麼都不缺** — 提醒本來就在跑，只差你決定要不要開放調整 | 現在固定 14 天、不能關、不能改天數；部分使用者可能想關掉或改頻率 | 要不要開放、開放到什麼程度（固定選項 vs 自訂天數） |
...
**目前候選裡「我能自己往前推」的只剩 #12（提醒策略，若你決定要開放）**：
1／2／4／5／8／9 已結案，3 卡在你的 Vercel 設定，6／7 需要母語者與後端／法務，
10／11 則卡在你對 Session 72 提案的確認／拍板。下一個動作是回覆 #10–12 的
開放式問題，或先處理 #3 雲端同步接線。
