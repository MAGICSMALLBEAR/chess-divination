...
| 3 | **跨裝置雲端同步** | **只差你動手一次** — 程式已完成且會誠實回報未啟用；線上端點實測 501（伺服器沒接資料庫）。接線步驟已交給你，見 Session 67 | 換機／雙機不再手動備份還原 | 在 Vercel Marketplace 接 Upstash Redis → redeploy；資料保留與費用上限 |
