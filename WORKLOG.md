...
| # | 待辦 | 你要做的 | 之後我能接手 | 成本 |
|---|------|---------|------------|------|
| 1 | **AI 解讀上線** | Vercel 專案設定加入 `DEEPSEEK_API_KEY` | 驗證 `api/interpret` 不再回 501、實測解讀品質 | 約 2 分鐘 |
...
**#1 投報率最高**——兩分鐘的設定，讓「AI 深度解讀」這個主打功能從降級狀態變成真的可用。

**#3 的重點在 A25**：`isPlausibleCapture` 的 4KB 門檻是估的，從沒有實機樣本驗證過。
要看的是兩件事——分享圖打開後有沒有內容、以及正常卡片會不會被誤判成空白而降級成文字分享。
完整清單見 `NATIVE_TESTING.md` 第一階段（12 項，已標明 Expo Go 驗得到與驗不到）。

### 🟢 不需外部資源（我可以直接做）

| # | 待辦 | 優先度 | 備註 |
|---|------|--------|------|
| 15 | **`summarizeReading()` 是死碼** | 🟢 低 | 註解寫「供分享與離線解讀使用」，但全專案只有測試呼叫它（Session 38 掃樣板字串時發現）。它產出的 6 條中文樣板因此也沒翻。**要先確認原意**：接上分享流程讓註解成真，或連同 4 則測試一起刪。有測試掛著，不該順手刪 |
| 16 | **棋盤頁：棋子池移到棋盤右邊** | 🟢 低 | Session 37 判定棋盤頁不該套雙欄（內容不足一屏、側欄會截斷類別列），但桌面上仍有實質問題：挑棋子與落子之間要上下移動視線。真正的解法是把棋子池移到棋盤右側——需把 tray 從 `ChessBoard` 拆出來，而拖曳定位邏輯（`screenToGrid`）在裡面，屬於**重構而非版面切換**，要另立範圍 |
